...
export function OverlayDesigner({ value, onChange, onReset }: { value: OverlayPreferences; onChange: (patch: OverlayPreferencesPatch) => void; onReset: () => void }): JSX.Element {
  ...
  return <section className="settings-service-card overlay-preferences-card overlay-designer-card">
    ...
    <section className="designer-section designer-display-section">
      <div className="designer-section-heading"><div><h3>显示方式</h3><p>运行时悬浮窗默认无外投影；文字模式适合放在 IDE 或网页上方。</p></div></div>
      <div className="designer-fields-grid">
        <label className="designer-select-field"><span>显示模式</span><select data-testid="overlay-appearance-mode" value={value.appearance.mode} onChange={(event) => setAppearance({ mode: event.target.value as OverlayPreferences["appearance"]["mode"] })}><option value="glass">玻璃卡片</option><option value="translucent">半透明</option><option value="text_only">纯文字</option><option value="custom">自定义</option></select></label>
        <label className="designer-select-field"><span>文字阴影</span><select value={value.appearance.textShadow} onChange={(event) => setAppearance({ textShadow: event.target.value as OverlayPreferences["appearance"]["textShadow"] })}><option value="none">无</option><option value="soft">柔和</option><option value="medium">增强</option></select></label>
        <label className="designer-select-field"><span>文字描边</span><select value={String(value.appearance.textOutline)} onChange={(event) => setAppearance({ textOutline: Number(event.target.value) as OverlayPreferences["appearance"]["textOutline"] })}><option value="0">无</option><option value="0.5">细</option><option value="1">增强</option></select></label>
      </div>
      <div className="designer-toggle-grid">
        <label className="designer-check-row"><input type="checkbox" checked={value.behavior.followLatestQuestion} onChange={(event) => setBehavior({ followLatestQuestion: event.target.checked })} /><span><strong>问题自动跟随最新</strong><small>手动上滚后暂停，可点“回到最新”恢复</small></span></label>
        <label className="designer-check-row"><input type="checkbox" checked={value.behavior.followLatestAnswer} onChange={(event) => setBehavior({ followLatestAnswer: event.target.checked })} /><span><strong>回答自动跟随最新</strong><small>新问题或新回答开始时自动回到尾部</small></span></label>
      </div>
    </section>
  </section>;
}
