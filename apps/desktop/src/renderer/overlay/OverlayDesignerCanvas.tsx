...
import { boundsForPanel, clampDesignerPosition, clampDesignerRect, resizeDesignerRectFromPointer, type DesignerCanvas, type DesignerLayout, type DesignerPanel, type DesignerRect, type ResizeHandle } from "./overlay-designer";
...
function PreviewWindow({ panel, rect, canvas, constraints, selected, onSelect, onMove, onResize, onCommit, orientation, children }: { panel: DesignerPanel; rect: DesignerRect; canvas: DesignerCanvas; constraints: ReturnType<typeof boundsForPanel>; selected: boolean; onSelect: () => void; onMove: (rect: DesignerRect) => void; onResize: (rect: DesignerRect) => void; onCommit: () => void; orientation?: OverlayPreferences["interview"]["controlBar"]["orientation"]; children: JSX.Element }): JSX.Element {
  ...
  const beginDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const move = (next: PointerEvent) => { const frame = frameRef.current?.parentElement; const start = startRef.current; if (!frame || !start) return; onMove(clampDesignerPosition({ ...start.rect, x: start.rect.x + (next.clientX - start.clientX) * canvas.width / Math.max(1, frame.clientWidth), y: start.rect.y + (next.clientY - start.clientY) * canvas.height / Math.max(1, frame.clientHeight) }, canvas, constraints)); };
...
export function OverlayDesignerCanvas({ value, mode = "interview", onChange, displays = [], activeDisplay }: { value: OverlayPreferences; mode?: OverlayDesignerMode; onChange: (patch: OverlayPreferencesPatch) => void; displays?: OverlayDisplayInfo[]; activeDisplay?: OverlayDisplayInfo }): JSX.Element {
  ...
  const source = mode === "writtenTest" ? value.writtenTest : value.interview;
  const geometryMode = mode === "writtenTest" ? "written_test" : "interview";
  const geometryPreset = source.layoutPreset;
  ...
  const handleDisplayChange = (displayId: number): void => {
    ...
    if (!selectedDisplay || selectedDisplay.id < 0) return;
    setSelectedDisplayId(selectedDisplay.id);
    setScreenSize({ width: selectedDisplay.workArea.width, height: selectedDisplay.workArea.height });
  ...
  const selectedBounds = boundsForPanel({ panel: selected, mode, preset: geometryPreset });
