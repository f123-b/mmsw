...
function CatalogModelSelect({ label, value, models, category, onChange, optional = false }: { label: string; value: string; models: DiscoveredModel[]; category: ModelCategory; onChange: (value: string) => void; optional?: boolean }): JSX.Element {
  ...
  return <label className="clean-field model-catalog-field"><span>{label}<small>{options.length} 个目录建议 · 可手填</small></span><input aria-label={label} list={listId} value={value} onChange={(event) => onChange(event.target.value)} placeholder={optional ? "不配置（留空）" : "选择或输入模型 ID"} /><datalist id={listId}>{options.map((model) => <option value={model.id} key={model.id}>{model.name}</option>)}</datalist><small className="model-capability-hint">目录分类仅供筛选；请按供应商说明确认所选模型的能力。</small></label>;
}
...
function AboutSettings(): JSX.Element {
  return <div className="settings-content-stack" data-testid="settings-about-page"><section className="settings-section-card about-settings"><span className="page-kicker">INTERVIEW COPILOT</span><h2>关于</h2><p>面向 Windows 面试场景的低干扰桌面辅助工具。</p><dl><div><dt>版本</dt><dd>0.1.9</dd></div><div><dt>渲染架构</dt><dd>Electron · React · TypeScript</dd></div><div><dt>本地数据</dt><dd>SQLite + OS secure storage</dd></div></dl></section></div>;
}
