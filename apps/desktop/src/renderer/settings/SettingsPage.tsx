import { ASR_PRESETS, normalizeAsrSettings, usesHttpAsr, type ProviderSettings } from "@interview-copilot/shared";
...
interface SettingsPageProps {
  ...
  llm: {
    apiProtocol: import("@interview-copilot/shared").LlmApiProtocol;
    onApiProtocolChange: (value: import("@interview-copilot/shared").LlmApiProtocol) => void;
    providerName: string;
    baseUrl: string;
    apiKey: string;
    model: string;
  ...
  asr: {
    protocol: NonNullable<ProviderSettings["asrProtocol"]>;
    onProtocolChange: (value: NonNullable<ProviderSettings["asrProtocol"]>) => void;
    appId: string;
  ...
  onChooseLlmPreset: (preset: "deepseek" | "qwen" | "custom" | "openai" | "claude" | "gemini") => void;
...
type ModelPickerOption = Pick<DiscoveredModel, "id" | "name">;

const MODEL_PICKER_PLACEHOLDER = "__model_picker_placeholder__";
const MODEL_PICKER_CLEAR = "__model_picker_clear__";

/**
 * Keep the catalog browser independent from the editable model ID.
 * Native datalists filter their options by the current input value, which
 * made a populated model field look like it only had one available model.
 * A select is deliberately used here so opening the menu always shows the
 * complete catalog while the adjacent input still accepts custom IDs.
 */
function ModelPicker({ label, value, options, onChange, optional = false, hint = "下拉菜单始终展示完整目录；分类仅供筛选，请按供应商说明确认所选模型的能力。" }: { label: string; value: string; options: ModelPickerOption[]; onChange: (value: string) => void; optional?: boolean; hint?: string }): JSX.Element {
  const uniqueOptions = Array.from(new Map(options.filter((option) => option.id.trim()).map((option) => [option.id, option])).values());
  return <label className="clean-field model-catalog-field"><span>{label}<small>{uniqueOptions.length} 个目录建议 · 可手填</small></span><div className="model-picker-row"><select aria-label={`${label}目录建议`} className="model-picker-select" value={MODEL_PICKER_PLACEHOLDER} onChange={(event) => { const selected = event.target.value; if (selected === MODEL_PICKER_CLEAR) { onChange(""); } else if (selected !== MODEL_PICKER_PLACEHOLDER) { onChange(selected); } }}><option value={MODEL_PICKER_PLACEHOLDER} disabled>从完整目录选择…</option>{optional && <option value={MODEL_PICKER_CLEAR}>清除当前模型（留空）</option>}{uniqueOptions.map((option) => <option value={option.id} key={option.id}>{option.name && option.name !== option.id ? `${option.name} · ${option.id}` : option.id}</option>)}</select><input aria-label={label} className="model-picker-input" value={value} onChange={(event) => onChange(event.target.value)} placeholder={optional ? "模型 ID（可留空）" : "模型 ID（也可手动输入）"} /></div><small className="model-capability-hint">{hint}</small></label>;
}
...
function CatalogModelSelect({ label, value, models, category, onChange, optional = false }: { label: string; value: string; models: DiscoveredModel[]; category: ModelCategory; onChange: (value: string) => void; optional?: boolean }): JSX.Element {
  ...
  return <ModelPicker label={label} value={value} options={options} onChange={onChange} optional={optional} />;
}
...
function ModelSettings({ props }: { props: SettingsPageProps }): JSX.Element {
  ...
  return <div className="settings-content-stack" data-testid="settings-models-page">
    ...
    <section className="settings-section-card"><h2>主流模型接入</h2><p>选择接口模板 → 填写供应商提供的模型 ID 和 API Key → 保存并测试连接。已有配置仍保留；切换模板不会自动激活。</p><div className="provider-preset-row"><button onClick={() => props.onChooseLlmPreset("openai")}>OpenAI / Responses</button><button onClick={() => props.onChooseLlmPreset("claude")}>Claude / Messages</button><button onClick={() => props.onChooseLlmPreset("gemini")}>Gemini / OpenAI 兼容</button></div><p className="page-note">DeepSeek、千问、Kimi、豆包、智谱及兼容网关使用 Chat Completions；路径请按厂商文档填写。文本、视觉、推理能力取决于具体模型，不是所有模型都能截图识题。未列出的模型 ID 可手动填写。</p></section>
    <section className="settings-section-card" data-testid="settings-llm-provider"><div className="settings-section-heading"><div><h2>模型与 API</h2><p>从 Provider 获取真实模型目录，并按能力分配到回答任务。</p></div><span className={`settings-status-dot ${selected?.hasApiKey ? "ready" : ""}`}>{selected?.hasApiKey ? "已配置" : "未配置"}</span></div><div className="provider-preset-row" role="group" aria-label="LLM 供应商"><button className={providerPreset === "deepseek" ? "selected" : ""} onClick={() => props.onChooseLlmPreset("deepseek")}><strong>DeepSeek</strong><small>官方 API</small></button><button className={providerPreset === "qwen" ? "selected" : ""} onClick={() => props.onChooseLlmPreset("qwen")}><strong>阿里云百炼 / 千问</strong><small>文本与视觉</small></button><button className={providerPreset === "custom" ? "selected" : ""} onClick={() => props.onChooseLlmPreset("custom")}><strong>OpenAI 兼容</strong><small>vLLM / LM Studio / Ollama / Gateway</small></button></div><div className="settings-form-grid"><label className="clean-field"><span>API 协议</span><select aria-label="API 协议" value={props.llm.apiProtocol} onChange={event => props.llm.onApiProtocolChange(event.target.value as typeof props.llm.apiProtocol)}><option value="openai-chat">OpenAI Chat Completions / 国内兼容接口</option><option value="openai-responses">OpenAI Responses</option><option value="anthropic-messages">Claude Messages</option></select></label><label className="clean-field"><span>供应商名称</span><input value={props.llm.providerName} onChange={(event) => props.llm.onProviderNameChange(event.target.value)} /></label><label className="clean-field"><span>Base URL（填写完整 API 前缀）</span><input value={props.llm.baseUrl} onChange={(event) => props.llm.onBaseUrlChange(event.target.value)} /></label><label className="clean-field"><span>API Key <em className="configured-label">{selected?.hasApiKey ? "已保存 · 输入新值即可替换" : "仅保存在系统安全存储"}</em></span><input type="password" autoComplete="off" value={props.llm.apiKey} onChange={(event) => props.llm.onApiKeyChange(event.target.value)} placeholder={selected?.hasApiKey ? "输入新的 API Key" : "输入 API Key"} /></label><div className="settings-form-actions"><button className="dark-pill" onClick={() => void props.onSaveLlmProfile()}>{props.modelCatalogLoading.llm ? "保存中…" : "保存并启用"}</button><button className="outline-pill" disabled={props.modelCatalogLoading.llm} onClick={() => props.onFetchModels("llm")}>{props.modelCatalogLoading.llm ? "获取中…" : "获取模型"}</button><button className="outline-pill" onClick={() => props.onTestProvider("llm")}>测试连接</button></div></div><p className="provider-feedback">{props.providerTests.llm ?? "保存后将自动作为面试和笔试使用的当前模型配置"}</p></section>
    ...
    <div className="settings-sticky-actions"><button className="dark-pill" onClick={() => void props.onSaveLlmProfile()}>保存并启用模型配置</button></div>
  </div>;
}

function asrEndpointPreview(props: SettingsPageProps): string {
  try {
    const settings: ProviderSettings = { providerType: props.asr.providerType, providerName: ASR_PRESETS[props.asr.providerType].name, baseUrl: props.asr.baseUrl, model: props.asr.model, asrProtocol: props.asr.protocol, apiKey: "", timeoutMs: 30000, maxRetries: 2 };
    return `${usesHttpAsr(settings) ? "分段转写" : "实时识别"} · ${normalizeAsrSettings(settings).baseUrl}`;
  } catch { return "请填写完整的服务地址"; }
}
...
function AboutSettings(): JSX.Element {
  return <div className="settings-content-stack" data-testid="settings-about-page"><section className="settings-section-card about-settings"><span className="page-kicker">INTERVIEW COPILOT</span><h2>关于</h2><p>面向 Windows 面试场景的低干扰桌面辅助工具。</p><dl><div><dt>版本</dt><dd>0.1.12</dd></div><div><dt>渲染架构</dt><dd>Electron · React · TypeScript</dd></div><div><dt>本地数据</dt><dd>SQLite + OS secure storage</dd></div></dl></section></div>;
}
