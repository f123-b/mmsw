...
import type { InterviewDirectionSelection, InterviewTerminologyPreview, Profile } from "@interview-copilot/shared";
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose(); }}>
    <section className="setup-modal interview-setup-modal" role="dialog" aria-modal="true" aria-labelledby="setup-title">
      ...
      <div className="setup-modal-body">
        ...
        <div className="setup-section"><DirectionSelector value={props.directionSelection} onChange={props.onDirectionSelectionChange} compact /><div className="direction-preview-summary">{props.directionPreviewLoading ? "正在计算本轮有效词典…" : preview ? <><span>领域：{[...preview.primaryDomains, ...preview.secondaryDomains].slice(0, 5).join(" · ") || "自动"}</span><span>有效术语 {preview.lexiconSize} 项</span></> : <span>未选择方向时保持当前自动识别</span>}</div><label className="setup-save-default"><input type="checkbox" checked={saveAsProfileDefault} onChange={(event) => setSaveAsProfileDefault(event.target.checked)} /><span>保存为当前档案默认方向</span></label></div>
        <div className="setup-control-grid setup-section"><label className="clean-field"><span>回答模式</span><select value={props.answerMode} onChange={(event) => props.onAnswerModeChange(event.target.value as "FAST" | "NORMAL" | "DEEP")}><option value="FAST">FAST · 优先低延迟</option><option value="NORMAL">NORMAL · 平衡</option><option value="DEEP">DEEP · 深度</option></select></label><label className="clean-field"><span>自动回答</span><select value={props.automationMode} onChange={(event) => props.onAutomationModeChange(event.target.value as "AUTO" | "MANUAL")}><option value="AUTO">AUTO · 自动回答</option><option value="MANUAL">MANUAL · 手动触发</option></select></label></div>
