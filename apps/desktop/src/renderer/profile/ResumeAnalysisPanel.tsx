...
import type { ProjectRecord, ResumeProjectLinkRecord } from "../../main/database";
...
interface ResumeAnalysisPanelProps {
  ...
  onSaveLink: (resumeProjectId: string, projectId: string) => void;
...
export function ResumeAnalysisPanel({ material, analysis, running, onAnalyze, projects, links, onSaveLink, onCreateProject }: ResumeAnalysisPanelProps): JSX.Element {
  ...
  return <section className="profile-subsection resume-analysis-panel">
    ...
    {current && <div className="resume-analysis-summary"><div className="detail-metrics"><span>教育<strong>{current.education.length}</strong></span><span>工作/实习<strong>{current.workExperience.length + current.internships.length}</strong></span><span>项目<strong>{current.projects.length}</strong></span><span>技能<strong>{current.skills.length}</strong></span></div><div className="resume-analysis-projects">{current.projects.map((project) => { const link = links.find((item) => item.resumeProjectId === project.id); return <article className="profile-project-card" key={project.id}><div className="profile-project-heading"><strong>{project.name}</strong><span>{link?.confirmed ? "已确认关联" : link ? `建议关联 · ${Math.round(link.confidence * 100)}%` : "未关联"}</span></div><p>{project.description || "项目描述待补充"}</p><small>证据：{project.evidence.rawExcerpt}</small><div className="detail-actions"><select value={link?.projectId ?? ""} onChange={(event) => { if (event.target.value) onSaveLink(project.id, event.target.value); }}><option value="">选择已有项目</option>{projects.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select><button className="text-button" onClick={() => onCreateProject(project.id, project.name)}>新建并关联</button></div></article>; })}{current.projects.length === 0 && <p className="page-note">当前 Resume 未找到满足严格标题条件的项目。</p>}</div>{current.warnings.map((warning) => <small className="page-note" key={warning}>{warning}</small>)}</div>}
  </section>;
}
