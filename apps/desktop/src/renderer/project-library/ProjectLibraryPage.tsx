...
function sourceRoleLabel(value: unknown): string {
  ...
  return ({ overview: "项目说明", code: "源码", resume: "简历经历", responsibility: "职责说明", debug: "问题排查", test: "测试与指标", architecture: "架构设计", reference: "参考资料", other: "项目资料" } as Record<string, string>)[role] ?? "项目资料";
}
...
function ProjectAnalysisNotice({ model, onRebuild, rebuilding }: { model: ProjectLibraryPageViewModel; onRebuild: () => void; rebuilding: boolean }): JSX.Element | null {
  if (model.analysisStatus === "ready" && !rebuilding) return null;
  const analyzing = model.analysisStatus === "analyzing" || rebuilding;
  const title = analyzing ? `正在分析 ${model.status.sourceCount} 份项目资料…` : model.analysisStatus === "failed" ? "项目分析失败" : "已上传项目资料，等待生成项目技术知识";
  const description = analyzing ? "读取资料 → 整理技术事实 → 提取关键参数 → 整理技术决策 → 生成问题链" : model.analysisStatus === "failed" ? "已保留原有项目知识，可以重新运行一次完整分析。" : "完成后会自动生成 Project Facts、参数、决策和问题链。";
  return <section className={`project-analysis-notice ${analyzing ? "is-analyzing" : model.analysisStatus === "failed" ? "is-failed" : ""}`} role="status"><div><strong>{title}</strong><p>{description}</p>{analyzing && <div className="project-analysis-steps"><span>读取资料</span><span>整理技术事实</span><span>提取关键参数</span><span>整理技术决策</span><span>生成问题链</span></div>}</div>{!analyzing && <button className="outline-pill" onClick={onRebuild}>分析项目</button>}</section>;
}
...
function ProjectOverview({ model, onSection, onSelectParameter, onSelectDecision, onSelectProblem, onOpenSources, onOpenQuestions, onRebuild, rebuilding, onAddSource, onNextAction }: { model: ProjectLibraryPageViewModel; onSection: (section: ProjectSection) => void; onSelectParameter: (fact: ProjectFact) => void; onSelectDecision: (fact: ProjectFact) => void; onSelectProblem: (problem: ProjectProblem) => void; onOpenSources: () => void; onOpenQuestions: () => void; onRebuild: () => void; rebuilding: boolean; onAddSource: () => void; onNextAction: (type: ProjectLibraryPageViewModel["nextActions"][number]["type"]) => void }): JSX.Element {
  ...
  return <div className="project-overview-layout-v5"><main className="project-overview-main-v5">
    <ProjectAnalysisNotice model={model} onRebuild={onRebuild} rebuilding={rebuilding} />
