...
function sourceRoleLabel(value: unknown): string {
  ...
  return ({ overview: "项目说明", code: "源码", resume: "简历经历", responsibility: "职责说明", debug: "排查记录", test: "测试与指标", architecture: "架构设计", reference: "参考资料", other: "项目资料" } as Record<string, string>)[role] ?? "项目资料";
}
...
function ProjectHeader(props: {
  ...
  return <header className="project-header-v5">
    <div className="project-header-main"><div className="project-header-kicker">项目库 <span>/</span> 项目详情</div><div className="project-title-line"><h1>{props.project.name}</h1><button className="project-icon-button" onClick={props.onEdit} aria-label="编辑项目设置">✎</button></div><p className="project-header-summary">{props.summary}</p><span className="project-ownership-chip">{OwnershipLabel({ mode })}</span></div>
...
function ProjectTabs({ section, advancedOpen, onSection, onToggleAdvanced }: { section: ProjectSection; advancedOpen: boolean; onSection: (section: ProjectSection) => void; onToggleAdvanced: () => void }): JSX.Element {
  const tabs: Array<[ProjectSection, string]> = [["overview", "概览"], ["parameters", "关键参数"], ["architecture", "技术架构"], ["decisions", "决策与 Why"], ["problems", "问题排查"], ["questions", "面试题"], ["sources", "资料证据"]];
...
function TechnologyRows({ model }: { model: ProjectLibraryPageViewModel }): JSX.Element {
  ...
  return <div className="project-technology-rows">{model.technologies.map((group) => {
    const items = group.items.slice(0, Math.max(0, 20 - shown));
    shown += items.length;
    return <div className="project-technology-row" key={group.category}><strong>{group.label}</strong><span>{items.join(" · ") || "待补充"}{group.items.length > items.length && <button className="project-inline-link" onClick={() => undefined}>+ {group.items.length - items.length} more</button>}</span></div>;
  })}{total === 0 && <p className="project-muted">还没有可靠技术信息。</p>}</div>;
}

function ParameterTable({ parameters, limit, onSelect, emptyAction }: { parameters: ProjectFact[]; limit?: number; onSelect: (fact: ProjectFact) => void; emptyAction?: ReactNode }): JSX.Element {
  const visible = limit ? parameters.slice(0, limit) : parameters;
  return <div className="project-table-wrap"><table className="project-data-table"><thead><tr><th>参数</th><th>当前值</th><th>状态</th></tr></thead><tbody>{visible.map((fact) => <tr key={fact.id} tabIndex={0} onClick={() => onSelect(fact)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(fact); } }}><td><button className="project-table-link" onClick={(event) => { event.stopPropagation(); onSelect(fact); }}>{fact.title}</button></td><td>{factValue(fact)}</td><td><span className={`project-status-text ${statusLabel(fact) === "有冲突" ? "warning" : "success"}`}>{statusLabel(fact)}</span></td></tr>)}</tbody></table>{visible.length === 0 && <EmptyState title="关键参数" description="还没有可靠关键参数。上传配置文件或手动补充参数，可以减少面试时的回答错误。" action={emptyAction} />}</div>;
}

function DecisionList({ decisions, onSelect, emptyAction }: { decisions: ProjectFact[]; onSelect: (fact: ProjectFact) => void; emptyAction?: ReactNode }): JSX.Element {
  return <div className="project-decision-list">{decisions.slice(0, 3).map((fact) => <button className="project-decision-row" key={fact.id} onClick={() => onSelect(fact)}><span className="project-decision-question">为什么{fact.title || "采用这个方案"}？</span><span>{cleanText(fact.content)}</span><small>{evidenceCount(fact)} 个来源 <b>→</b></small></button>)}{decisions.length === 0 && <EmptyState title="还没有形成 Why 事实" description="补充技术方案的取舍和原因，让面试回答更完整。" action={emptyAction} />}</div>;
}
...
function ProjectOverview({ model, onSection, onSelectParameter, onSelectDecision, onSelectProblem, onOpenSources, onOpenQuestions, onRebuild, rebuilding, onAddSource, onNextAction }: { model: ProjectLibraryPageViewModel; onSection: (section: ProjectSection) => void; onSelectParameter: (fact: ProjectFact) => void; onSelectDecision: (fact: ProjectFact) => void; onSelectProblem: (problem: ProjectProblem) => void; onOpenSources: () => void; onOpenQuestions: () => void; onRebuild: () => void; rebuilding: boolean; onAddSource: () => void; onNextAction: (type: ProjectLibraryPageViewModel["nextActions"][number]["type"]) => void }): JSX.Element {
  ...
  return <div className="project-overview-layout-v5"><main className="project-overview-main-v5">
    ...
    <section className="project-section-v5"><SectionHeader title="关键参数" description="只显示最常用的 6 项" action={<button className="project-text-link" onClick={() => onSection("parameters")}>查看全部参数 →</button>} /><ParameterTable parameters={model.parameters} limit={6} onSelect={onSelectParameter} emptyAction={<button className="outline-pill" onClick={() => onSection("parameters")}>补充参数</button>} /></section>
    <section className="project-section-v5"><SectionHeader title="技术决策" description="用 Why 复习取舍和原因" action={<button className="project-text-link" onClick={() => onSection("decisions")}>查看全部 Why →</button>} /><DecisionList decisions={model.decisions} onSelect={onSelectDecision} emptyAction={<button className="outline-pill" onClick={() => onSection("decisions")}>查看可能决策</button>} /></section>
...
function ProjectAgentRuntime({ model, messages, agentInput, onInput, onSend, sending, onRetry, onSettings }: { model: ProjectLibraryPageViewModel; messages: ProjectAgentMessage[]; agentInput: string; onInput: (value: string) => void; onSend: () => void; sending: boolean; onRetry: (messageId: string) => void; onSettings: () => void }): JSX.Element {
  ...
  return <div className="project-agent-runtime"><div className="project-agent-conversation">{visible.length ? visible.map((message) => { const failed = message.role === "assistant" && message.status === "failed"; return <article className={`project-agent-message ${message.role} ${failed ? "failed" : ""}`} key={message.id}><strong>{message.role === "user" ? "我" : "项目 Agent"}{failed ? " · 生成失败" : ""}</strong><p>{message.content || (message.status === "streaming" ? "正在分析项目资料…" : failed ? chatFailureText(message.errorCode, message.model) : "未返回内容")}</p>{failed && <div className="project-agent-recovery"><button className="outline-pill" disabled={sending} onClick={() => onRetry(message.id)}>重新生成</button><button className="outline-pill" onClick={onSettings}>检查模型设置</button></div>}</article>; }) : <div className="project-empty-state project-agent-empty"><strong>从资料到可信项目库</strong><p>可以直接问“哪些地方冲突”“我的职责还缺什么证据”，也可以让 Agent 提议补充事实。</p></div>}</div><div className="project-agent-composer"><textarea value={agentInput} onChange={(event) => onInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); onSend(); } }} placeholder="告诉 Agent 你的真实职责、实现细节，或让它检查资料冲突…" aria-label="项目 Agent 输入" /><button className="dark-pill" disabled={!agentInput.trim() || sending} onClick={onSend}>{sending ? "分析中…" : "发送"}</button></div></div>;
}
...
function ProjectParameters({ model, search, onSearch, category, onCategory, onSelect }: { model: ProjectLibraryPageViewModel; search: string; onSearch: (value: string) => void; category: string; onCategory: (value: string) => void; onSelect: (fact: ProjectFact) => void }): JSX.Element {
  ...
  return <section className="project-detail-page-v5"><div className="project-detail-intro"><span className="project-eyebrow">PROJECT DETAIL</span><h2>关键参数</h2><p>把面试中最容易被追问的数值集中在一张可读表里。</p></div><div className="project-filter-row"><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="搜索参数..." aria-label="搜索参数" /><select value={category} onChange={(event) => onCategory(event.target.value)} aria-label="参数分类"><option>全部</option>{categories.map((item) => <option key={item}>{item}</option>)}</select></div><ParameterTable parameters={visible} onSelect={onSelect} /></section>;
}

function ProjectArchitecture({ model }: { model: ProjectLibraryPageViewModel }): JSX.Element {
  return <section className="project-detail-page-v5"><div className="project-detail-intro"><span className="project-eyebrow">PROJECT DETAIL</span><h2>技术架构</h2><p>用层级关系复习系统，不引入复杂图形。</p></div><div className="project-architecture-layout"><div className="project-architecture-tree">{model.technologies.map((group) => <section key={group.category}><h3>{group.label}</h3><ul>{group.items.map((item) => <li key={item}>{item}</li>)}</ul></section>)}{model.modules.length > 0 && <section><h3>核心模块</h3><ul>{model.modules.slice(0, 8).map((module) => <li key={module.id}><strong>{module.moduleName}</strong>{module.description && <small>{module.description}</small>}</li>)}</ul></section>}{model.technologies.length === 0 && model.modules.length === 0 && <EmptyState title="还没有架构信息" description="上传架构说明或源码后重新分析。" />}</div><aside className="project-context-aside"><h3>相关技术</h3><p>{model.technologies.flatMap((group) => group.items).slice(0, 10).join(" · ") || "待补充"}</p><h3>相关参数</h3><p>{model.parameters.slice(0, 5).map((fact) => fact.title).join(" · ") || "待补充"}</p><h3>相关资料</h3><p>{model.status.sourceCount} 份项目资料</p></aside></div></section>;
}

function ProjectDecisions({ model, onSelect }: { model: ProjectLibraryPageViewModel; onSelect: (fact: ProjectFact) => void }): JSX.Element {
  return <section className="project-detail-page-v5"><div className="project-detail-intro"><span className="project-eyebrow">PROJECT DETAIL</span><h2>决策与 Why</h2><p>每条决策都回答“为什么这样做”，并保留可追溯证据。</p></div><DecisionList decisions={model.decisions} onSelect={onSelect} /></section>;
}
...
function ProjectQuestions({ model, onSelect }: { model: ProjectLibraryPageViewModel; onSelect: (question: ProjectInterviewQuestion) => void }): JSX.Element {
  return <section className="project-detail-page-v5"><div className="project-detail-intro"><span className="project-eyebrow">PROJECT DETAIL</span><h2>面试题</h2><p>只展示题目、分类和准备状态；关联事实与证据在详情抽屉中查看。</p></div><div className="project-question-list-v5">{model.questions.map((question) => <button key={question.id} onClick={() => onSelect(question)}><div><strong>{question.question}</strong><small>{question.keywords.slice(0, 3).join(" · ") || "项目复习"}</small></div><span>{question.stale ? "需更新" : "已准备"}</span></button>)}{model.questions.length === 0 && <EmptyState title="还没有项目面试题" description="确认项目事实后重新生成项目题。" />}</div></section>;
}

function ProjectSources({ facts, sources, onSelect, onOpenSources }: { facts: ProjectFact[]; sources: SourceRecord[]; onSelect: (source: SourceRecord) => void; onOpenSources: () => void }): JSX.Element {
  return <section className="project-detail-page-v5"><div className="project-detail-intro"><span className="project-eyebrow">PROJECT DETAIL</span><h2>资料证据</h2><p>{sources.length} 份资料 · 点击文件查看绑定角色和支持的信息。</p></div><div className="project-source-list-v5">{sources.map((source) => { const sourceId = String(source.sourceId ?? source.id ?? ""); const supported = facts.filter((fact) => fact.sourceIds.includes(sourceId)).length; return <button key={String(source.id ?? source.sourceId)} onClick={() => onSelect(source)}><span className="project-source-file-icon">▤</span><span className="project-source-copy"><strong>{String(source.title ?? source.sourceId ?? "项目资料")}</strong><small>{sourceRoleLabel(source.sourceRole)} · {String(source.status ?? "已就绪")}</small></span><span className="project-source-count">{supported} 条信息</span><span className="project-source-chevron">→</span></button>; })}{sources.length === 0 && <EmptyState title="还没有绑定资料" description="添加 README、代码、排查记录或测试报告，让项目复习有依据。" action={<button className="dark-pill" onClick={onOpenSources}>添加资料</button>} />}</div>{sources.length > 0 && <p className="project-muted project-source-footnote">资料绑定保持在当前项目上下文中，解除绑定不会删除原始文件。</p>}</section>;
}
...
function ProjectDrawerContent({ drawer, model, sourceTitle, onClose, onResolveConflict, onReviewFact, onUpdateProject, selectedProject, onUnassignSource }: { drawer: DrawerState; model: ProjectLibraryDisplayModel; sourceTitle: (id: string) => string; onClose: () => void; onResolveConflict: (group: ProjectConflictGroup, fact: ProjectFact, keepBoth?: boolean, variantContexts?: Record<string, string>) => void; onReviewFact: (factId: string, status: "active" | "pending_review" | "rejected" | "conflicting") => void; onUpdateProject: (projectId: string, input: { ownershipMode?: OwnershipMode; ownershipNote?: string }) => Promise<void>; selectedProject: ProjectMemoryProject; onUnassignSource: (source: SourceRecord) => void }): JSX.Element {
  ...
  if (drawer.kind === "question") return <Drawer title={drawer.question.question} eyebrow="面试题" onClose={onClose}><h3>推荐回答要点</h3><ul className="project-bullet-list">{drawer.question.answerPoints.map((point) => <li key={point}>{point}</li>)}</ul><p className="project-drawer-meta-line">{drawer.question.factIds?.length ?? 0} 条关联信息 · {drawer.question.sourceIds.length} 个来源</p></Drawer>;
...
export function ProjectLibraryPage(props: ProjectLibraryPageProps): JSX.Element {
  ...
  const model = useMemo(() => selectedProject ? deriveProjectLibraryViewModel({ project: selectedProject, facts: projectFacts, modules: props.memory.modules, problems: props.memory.problems, questions: props.memory.interviewQuestions, sourceCount: sources.length, staleFactCount: staleFacts.length, completeness }) : undefined, [selectedProject, projectFacts, props.memory.modules, props.memory.problems, props.memory.interviewQuestions, sources.length, staleFacts.length, completeness]);
  ...
  const openAddSource = (): void => setSourceDialog(true);
  const sourceRoleOptions = [["overview", "项目说明 / README"], ["code", "源码 / 代码压缩包"], ["resume", "简历项目经历"], ["responsibility", "个人职责说明"], ["debug", "问题排查记录"], ["test", "测试与指标"], ["architecture", "架构设计"], ["reference", "参考资料"], ["other", "其他"]];
  ...
  return <section className="project-library-v5">
    ...
    <div className="project-workspace-grid"><aside className="project-local-sidebar"><div className="project-local-sidebar-heading"><span>我的项目</span><button onClick={() => setCreateProjectOpen(true)} aria-label="添加项目">＋</button></div><div className="project-local-list">{props.memory.projects.slice(0, 5).map((project) => <button key={project.id} className={project.id === model.project.id ? "selected" : ""} onClick={() => selectProject(project.id)}><span className="project-local-dot" /><strong title={project.name}>{project.name}</strong></button>)}{props.memory.projects.length > 5 && <button className="project-local-more" onClick={() => undefined}>查看全部项目</button>}</div></aside><div className="project-workspace-content">{projectLoading && <ProjectLoadingSkeleton />}<ProjectHeader project={model.project} summary={model.summary} sourceCount={model.status.sourceCount} sourceCoverage={model.completeness.sourceCoverageScore} updatedAt={sources.reduce((latest, source) => Math.max(latest, Number(source.updatedAt ?? 0)), 0)} familiarity={model.familiarity} onEdit={() => setDrawer({ kind: "settings" })} /><ProjectTabs section={section} advancedOpen={advancedOpen} onSection={selectSection} onToggleAdvanced={() => setAdvancedOpen((value) => !value)} />{section === "overview" && <ProjectOverview model={model} onSection={selectSection} onSelectParameter={(fact) => setDrawer({ kind: "parameter", fact })} onSelectDecision={(fact) => setDrawer({ kind: "decision", fact })} onSelectProblem={selectProblem} onOpenSources={() => selectSection("sources")} onOpenQuestions={() => selectSection("questions")} onRebuild={() => props.onRebuild(model.project.id)} rebuilding={props.rebuilding} onAddSource={openAddSource} onNextAction={(type) => { if (type === "missing_parameters") selectSection("parameters"); else if (type === "missing_decisions") selectSection("decisions"); else if (type === "conflict") { setAdvancedSection("conflicts"); selectSection("advanced"); } else openAddSource(); }} />}{section === "parameters" && <ProjectParameters model={model} search={parameterSearch} onSearch={setParameterSearch} category={parameterCategory} onCategory={setParameterCategory} onSelect={(fact) => setDrawer({ kind: "parameter", fact })} />}{section === "architecture" && <ProjectArchitecture model={model} />}{section === "decisions" && <ProjectDecisions model={model} onSelect={(fact) => setDrawer({ kind: "decision", fact })} />}{section === "problems" && <ProjectProblems model={model} selectedId={selectedProblemId} onSelect={(problem) => setSelectedProblemId(problem.id)} />}{section === "questions" && <ProjectQuestions model={model} onSelect={(question) => setDrawer({ kind: "question", question })} />}{section === "sources" && <ProjectSources facts={projectFacts} sources={sources} onSelect={(source) => setDrawer({ kind: "source", source })} onOpenSources={openAddSource} />}{section === "advanced" && <ProjectAdvanced model={displayModel} advancedSection={advancedSection} onAdvancedSection={setAdvancedSection} onSelectFact={(fact) => setDrawer({ kind: "fact", fact })} onSelectConflict={(group) => setDrawer({ kind: "conflict", group })} onReviewFact={(factId, status) => void props.onReviewFact(factId, status)} />}
      {section === "overview" && model.project.ownershipMode !== "reference" && <div className="project-responsibility-inline"><label htmlFor="project-responsibility-v5">{normalizeProjectOwnershipMode(model.project.ownershipMode) === "personal" ? "补充项目边界（可选）" : "补充我的负责范围"}</label><div><textarea id="project-responsibility-v5" rows={2} value={responsibilityInput} onChange={(event) => setResponsibilityInput(event.target.value)} placeholder="补充你真实负责的模块、实现细节和边界" /><button className="outline-pill" disabled={!responsibilityInput.trim()} onClick={async () => { await props.onAddResponsibility(model.project.id, responsibilityInput.trim()); setResponsibilityInput(""); }}>保存范围</button></div></div>}
      {section === "overview" && <div className="project-agent-runtime-card"><header><div><span className="project-eyebrow">PROJECT AGENT</span><h2>项目资料整理助手</h2><p>问我哪里有冲突、还缺什么，或者准备一段真实的项目介绍。</p></div><span className="project-agent-scope">当前项目 · {model.project.name}</span></header><div className="project-agent-quick-actions"><button onClick={() => setAgentInput("检查当前项目资料中的冲突、缺失和不确定项。")}>检查冲突与缺口</button><button onClick={() => setAgentInput("根据现有资料整理一份可以直接用于面试的项目介绍，并标出没有证据的说法。")}>整理面试项目介绍</button><button onClick={() => setAgentInput("基于已确认信息生成这个项目最可能被追问的问题和答题要点。")}>生成追问题库</button></div><ProjectAgentRuntime model={model} messages={currentMessages} agentInput={agentInput} onInput={setAgentInput} onSend={() => void sendAgent()} sending={props.agentSending} onRetry={(messageId) => void props.onRetryAgent(messageId)} onSettings={props.onOpenSettings} /></div>}
    </div></div>
    ...
    {sourceDialog && <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="modal-card project-create-modal"><h2>添加项目资料</h2><p className="page-note">资料会直接绑定到“{model.project.name}”，不会自动创建新项目。</p><label className="clean-field"><span>资料类型</span><select value={sourceRole} onChange={(event) => setSourceRole(event.target.value)}>{sourceRoleOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label className="dark-pill upload-project-action">选择文件<input type="file" accept=".zip,.txt,.md,.pdf,.docx" onChange={(event) => { const file = event.target.files?.[0]; if (file) { void props.onUploadProject(file, model.project.id, sourceRole); setSourceDialog(false); } event.target.value = ""; }} /></label><button className="outline-pill" onClick={() => setSourceDialog(false)}>取消</button></div></div>}
  </section>;
}
