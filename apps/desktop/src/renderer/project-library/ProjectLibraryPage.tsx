...
function ProjectAnalysisNotice({ model, onRebuild, rebuilding }: { model: ProjectLibraryPageViewModel; onRebuild: () => void; rebuilding: boolean }): JSX.Element | null {
  if ((model.analysisStatus === "ready" || model.understanding?.status === "completed") && !rebuilding) return null;
  const analyzing = model.analysisStatus === "analyzing" || rebuilding;
  const title = analyzing ? `正在理解 ${model.status.sourceCount} 份项目资料…` : model.analysisStatus === "failed" ? "项目分析失败" : "已上传项目资料，等待生成项目技术知识";
  const description = analyzing ? "建立 Repo Map → 识别核心模块 → 分析运行流程 → 核对参数 → Grounding" : model.analysisStatus === "failed" ? "已保留原有项目知识，可以重新运行一次完整分析。" : "完成后会生成项目理解、Project Facts、参数、决策和问题链。";
  return <section className={`project-analysis-notice ${analyzing ? "is-analyzing" : model.analysisStatus === "failed" ? "is-failed" : ""}`} role="status"><div><strong>{title}</strong><p>{description}</p>{analyzing && <div className="project-analysis-steps"><span>Repo Map</span><span>核心模块</span><span>运行流程</span><span>参数语义</span><span>Grounding</span></div>}</div>{!analyzing && <button className="outline-pill" onClick={onRebuild}>分析项目</button>}</section>;
}
