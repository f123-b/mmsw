...
import { ProjectFileGuide, ProjectQuickStart } from "./ProjectQuickStart";
...
type ProjectSection = "overview" | "introduction" | "questions" | "sources" | "test" | "parameters" | "architecture" | "decisions" | "problems" | "advanced";
...
interface ProjectLibraryPageProps {
  ...
  onUpdateProject: (projectId: string, input: { ownershipMode?: OwnershipMode; ownershipNote?: string; projectIntroduction?: string }) => Promise<void>;
...
function ProjectTabs({ section, onSection }: { section: ProjectSection; advancedOpen: boolean; onSection: (section: ProjectSection) => void; onToggleAdvanced: () => void }): JSX.Element {
  const tabs: Array<[ProjectSection, string]> = [["overview", "概览"], ["introduction", "项目介绍"], ["sources", "项目文档"], ["questions", "项目题库"], ["test", "测试对话"]];
  return <nav className="project-tabs-v5" aria-label="项目详情导航">{tabs.map(([value, label]) => <button key={value} className={section === value ? "active" : ""} aria-selected={section === value} onClick={() => onSection(value)}>{label}</button>)}</nav>;
}
...
function ProjectIntroductionEditor({ project, onSave }: { project: ProjectMemoryProject; onSave: (text: string) => Promise<void> }): JSX.Element {
  const [text, setText] = useState(project.projectIntroduction ?? "");
  const [saving, setSaving] = useState(false);
  useEffect(() => setText(project.projectIntroduction ?? ""), [project.id, project.projectIntroduction]);
  return <section className="project-introduction-editor">
    <SectionHeader title="项目介绍" description="预先写好一段可以直接口述的介绍。面试官问“介绍一下这个项目”“讲讲这个项目”等问题时，会立即展示这里的内容。" />
    <textarea value={text} onChange={(event) => setText(event.target.value)} rows={14} placeholder="建议按：项目背景与目标 → 系统架构 → 核心实现 → 难点与结果，写成 1～3 分钟可直接口述的内容。" />
    <div className="detail-actions"><button className="dark-pill" disabled={!text.trim() || saving} onClick={async () => { setSaving(true); try { await onSave(text.trim()); } finally { setSaving(false); } }}>{saving ? "保存中…" : "保存项目介绍"}</button><span className="project-muted">保存后立即用于实时面试和测试对话。</span></div>
  </section>;
}
  ...
  return <section className="project-library-v5">
    ...
    <ProjectQuickStart key={model.project.id} hasProject sourceCount={sources.length} trustedFacts={model.status.trustedFacts} onCreate={() => setCreateProjectOpen(true)} onUpload={openAddSource} onReview={() => { setAdvancedSection("review"); selectSection("advanced"); }} onQuestions={() => selectSection("questions")} />
     <div className="project-workspace-grid"><div className="project-workspace-content">{projectLoading && <ProjectLoadingSkeleton />}<ProjectHeader project={model.project} summary={model.summary} sourceCount={model.status.sourceCount} sourceCoverage={model.completeness.sourceCoverageScore} updatedAt={latestSourceUpdatedAt} familiarity={model.familiarity} onEdit={() => setDrawer({ kind: "settings" })} onRename={() => void props.onRenameProject?.(model.project.id, model.project.name)} onDelete={() => void props.onDeleteProject?.(model.project.id, model.project.name)} /><ProjectTabs section={section} advancedOpen={advancedOpen} onSection={selectSection} onToggleAdvanced={() => setAdvancedOpen((value) => !value)} />{section === "overview" && <ProjectOverview model={model} onSection={selectSection} onSelectParameter={(fact) => setDrawer({ kind: "parameter", fact })} onSelectDecision={(fact) => setDrawer({ kind: "decision", fact })} onSelectProblem={selectProblem} onOpenSources={() => selectSection("sources")} onOpenQuestions={() => selectSection("questions")} onRebuild={() => props.onRebuild(model.project.id)} rebuilding={analysisRebuilding} analysisJob={analysisJob} onCancelAnalysis={() => void props.onCancelAnalysis(model.project.id, analysisJob?.id)} onRetryAnalysis={() => void props.onRetryAnalysis(model.project.id)} onAddSource={openAddSource} onNextAction={(type) => { if (type === "analyze_sources") props.onRebuild(model.project.id); else openAddSource(); }} />}{section === "introduction" && <ProjectIntroductionEditor project={model.project} onSave={(text) => props.onUpdateProject(model.project.id, { projectIntroduction: text })} />}{section === "questions" && <ProjectQuestionBankPanel key={model.project.id} facts={[...projectFacts, ...staleFacts]} sourceTitle={sourceTitle} profileId={props.profileId} projectId={model.project.id} projectName={model.project.name} onImport={(file) => props.onImportProjectQuestionBank(model.project.id, file)} onGenerate={() => props.onGenerateProjectQa(model.project.id)} />}{section === "sources" && <ProjectSources facts={projectFacts} sources={sources} analysisStatus={model.analysisStatus} onSelect={(source) => setDrawer({ kind: "source", source })} onOpenSources={openAddSource} />}
      ...
      {section === "test" && <div className="project-agent-runtime-card"><header><div><span className="project-eyebrow">PROJECT QA TEST</span><h2>项目回答测试</h2><p>像面试官一样提问，检查当前项目文档、项目介绍和题库能否给出准确回答。</p></div><span className="project-agent-scope">当前项目 · {model.project.name}</span></header><div className="project-agent-quick-actions"><button onClick={() => setAgentInput("请介绍一下这个项目")}>测试项目介绍</button><button onClick={() => setAgentInput("这个项目的系统架构和核心模块是什么？")}>测试项目架构</button><button onClick={() => setAgentInput("项目中遇到的主要问题和解决方法是什么？")}>测试问题排查</button></div><ProjectAgentRuntime model={model} messages={currentMessages} agentInput={agentInput} onInput={setAgentInput} onSend={() => void sendAgent()} sending={props.agentSending} onRetry={(messageId) => void props.onRetryAgent(messageId)} onSettings={props.onOpenSettings} /></div>}
    </div></div>
    ...
    {sourceDialog && <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="modal-card project-create-modal project-import-modal"><h2>上传项目文档</h2><p className="page-note">每个项目只需要一份完整项目文档；项目题库请在“项目题库”页上传。支持 MD、TXT、DOCX、可读取文字的 PDF 或源码 ZIP。</p><label className="dark-pill upload-project-action">选择项目文档<input type="file" accept=".zip,.txt,.md,.pdf,.docx" disabled={sourceImporting} onChange={(event) => { const files = Array.from(event.target.files ?? []).slice(0, 1); setSelectedFiles(files); setSelectedSourceRoles(files.map(() => "overview")); event.target.value = ""; }} /></label>{selectedFiles.length > 0 && <div className="project-import-preview"><strong>已选择项目文档</strong><div className="project-import-preview-row"><span><b>{selectedFiles[0]?.name}</b><small>将作为当前项目的主文档导入并分析</small></span></div></div>}<div className="detail-actions"><button className="dark-pill" disabled={selectedFiles.length === 0 || sourceImporting} onClick={() => void startProjectImport()}>{sourceImporting ? "导入并分析中…" : "导入并分析"}</button><button className="outline-pill" disabled={sourceImporting} onClick={closeSourceDialog}>取消</button></div></div></div>}
  </section>;
}
