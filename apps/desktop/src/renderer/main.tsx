  ...
  return (
    <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: "48px", background: "#0b1020", color: "#f4f7ff", fontFamily: "system-ui, sans-serif" }}>
      <section style={{ maxWidth: "560px", border: "1px solid #394463", borderRadius: "16px", padding: "32px", background: "#151d33", boxShadow: "0 18px 60px #0006" }}>
        ...
        <h1 style={{ margin: "0 0 16px", fontSize: "28px" }}>有招 启动失败</h1>
...
const isOverlayWindow = overlayWindowMode === "overlay-question" || overlayWindowMode === "overlay-answer" || overlayWindowMode === "overlay-script" || overlayWindowMode === "overlay-control" || overlayWindowMode === "overlay-transient";
if (isOverlayWindow) {
...
if (rootElement) {
  const mount = async () => {
    ...
    if (overlayWindowMode === "overlay-script") { const module = await import("./overlay-script"); void module; return; }
  ...
  document.body.textContent = "有招 启动失败：根节点不存在。";
}
