...
import { ANSWER_DESIGNER_BOUNDS, QUESTION_DESIGNER_BOUNDS, clampDesignerRect, resizeDesignerRect, type DesignerCanvas, type DesignerLayout, type DesignerPanel, type DesignerRect, type ResizeHandle } from "./overlay/overlay-designer";
...
function designerLayoutFromPreferences(value: OverlayPreferences): DesignerLayout {
  return {
    question: { x: value.questionWindow.x ?? 120, y: value.questionWindow.y ?? 180, width: value.questionWindow.width, height: value.questionWindow.height },
    answer: { x: value.answerWindow.x ?? 570, y: value.answerWindow.y ?? 180, width: value.answerWindow.width, height: value.answerWindow.height },
...
function PreviewWindow({ panel, rect, canvas, selected, onSelect, onMove, onResize, onCommit, orientation, children }: { panel: DesignerPanel; rect: DesignerRect; canvas: DesignerCanvas; selected: boolean; onSelect: () => void; onMove: (rect: DesignerRect) => void; onResize: (handle: ResizeHandle, delta: { x: number; y: number }) => void; onCommit: () => void; orientation?: OverlayControlBarOrientation; children: JSX.Element }): JSX.Element {
  ...
  const beginDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const end = () => { window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", end); startRef.current = undefined; onCommit(); };
    window.addEventListener("pointermove", move); window.addEventListener("pointerup", end, { once: true });
  };
  const beginResize = (handle: ResizeHandle, event: React.PointerEvent<HTMLDivElement>) => {
    event.stopPropagation(); onSelect(); startRef.current = { clientX: event.clientX, clientY: event.clientY, rect: { ...rect }, handle };
...
function DesktopPreview({ value, onChange }: { value: OverlayPreferences; onChange: (patch: OverlayPreferencesPatch) => void }): JSX.Element {
  ...
  const [layout, setLayout] = useState<DesignerLayout>(() => designerLayoutFromPreferences(value));
  const [screenSize, setScreenSize] = useState<DesignerCanvas>(DESIGNER_CANVAS);
  const draggingRef = useRef(false);
  const layoutRef = useRef(layout);
  useEffect(() => { setBackground(value.previewBackground); }, [value.previewBackground]);
  useEffect(() => { if (!draggingRef.current) { const next = designerLayoutFromPreferences(value); layoutRef.current = next; setLayout(next); } }, [value.questionWindow.x, value.questionWindow.y, value.questionWindow.width, value.questionWindow.height, value.answerWindow.x, value.answerWindow.y, value.answerWindow.width, value.answerWindow.height, value.controlBar.x, value.controlBar.y, value.controlBar.width, value.controlBar.height, value.controlBar.orientation]);
  ...
  const resizePanel = (panel: DesignerPanel, handle: ResizeHandle, delta: { x: number; y: number }) => { const rect = layoutRef.current[panel]; const bounds = panel === "question" ? QUESTION_DESIGNER_BOUNDS : panel === "answer" ? ANSWER_DESIGNER_BOUNDS : { minimumWidth: 120, maximumWidth: 2_000, minimumHeight: 36, maximumHeight: 240 }; updateLayout({ ...layoutRef.current, [panel]: resizeDesignerRect(rect, handle, delta, screenSize, bounds) }); };
  const frameChildren = (panel: DesignerPanel): JSX.Element => panel === "question" ? <><span className="preview-window-kicker">QUESTION NAVIGATOR</span><strong>CAN 总线是什么？</strong><small>点击或拖动阅读问题组</small></> : panel === "answer" ? <><span className="preview-window-kicker">ANSWER READER</span><strong>AI 回答</strong><p>CAN 采用基于 ID 的逐位仲裁，显性位会覆盖隐性位。</p><p>拖动边缘可以调整阅读窗大小。</p></> : <><span className="preview-control-dot">◈</span><span>00:24</span><span>回答中</span><button type="button" onClick={() => setSelected("controlBar")}>设置</button></>;
  return <div className="desktop-preview-shell">
    <div className="desktop-preview-heading"><div><span className="page-kicker">DESKTOP PREVIEW</span><strong>桌面效果预览</strong><small>拖动窗口或八向调整点，释放后同步设置；预览不会读取或修改真实面试数据。</small></div><select aria-label="预览屏幕比例" value={`${screenSize.width}x${screenSize.height}`} onChange={(event) => { const [width, height] = event.target.value.split("x").map(Number); if (width && height) setScreenSize({ width, height }); }}><option value="1920x1080">Current Display · 1920×1080</option><option value="2560x1440">2560×1440</option></select></div>
    ...
    <div className="desktop-preview-frame" style={previewBackgroundStyle(background, value.previewCustomColor)} data-preview-background={background}>
      <div className="desktop-preview-browser-chrome"><span /><span /><span /><small>{screenSize.width} × {screenSize.height} · simulated display</small></div>
      <div className="desktop-preview-canvas" style={{ aspectRatio: `${screenSize.width} / ${screenSize.height}` }}>
    ...
    <div className="desktop-preview-legend"><span><i className="preview-legend-question" />问题导航</span><span><i className="preview-legend-answer" />答案阅读器</span><span><i className="preview-legend-toolbar" />控制栏</span><small>Preview ↔ Settings 双向同步 · 逻辑坐标 {screenSize.width}×{screenSize.height}</small></div>
  </div>;
}
...
function DesignerWindowSection({ kind, title, value, onChange, onReset }: { kind: "question" | "answer"; title: string; value: OverlayPreferences["questionWindow"] | OverlayPreferences["answerWindow"]; onChange: (patch: Partial<OverlayPreferences["questionWindow"]>) => void; onReset: () => void }): JSX.Element {
  ...
  return <div className="designer-section-card"><div className="designer-section-card-heading"><div><strong>{title}</strong><small>{isQuestion ? "Question Navigator" : "Answer Reader"} · 独立配置</small></div><button type="button" className="text-button" onClick={onReset}>重置本区</button></div><div className="designer-fields-grid designer-geometry-fields"><DesignerNumberField label="X" value={value.x ?? 0} min={-2000} max={4000} suffix="px" onChange={(next) => onChange({ x: next })} /><DesignerNumberField label="Y" value={value.y ?? 0} min={-2000} max={3000} suffix="px" onChange={(next) => onChange({ y: next })} /><DesignerNumberField label="宽度" value={value.width} min={minWidth} max={maxWidth} suffix="px" onChange={(next) => onChange({ width: next })} /><DesignerNumberField label="高度" value={value.height} min={minHeight} max={1_200} suffix="px" onChange={(next) => onChange({ height: next })} /></div><div className="designer-fields-grid"><DesignerNumberField label="字体大小" value={value.fontSize} min={10} max={isQuestion ? 32 : 40} suffix="px" onChange={(next) => onChange({ fontSize: next })} /><label className="designer-select-field"><span>字体粗细</span><select value={value.fontWeight} onChange={(event) => onChange({ fontWeight: Number(event.target.value) as OverlayFontWeight })}><option value="400">400 · Regular</option><option value="500">500 · Medium</option><option value="600">600 · Semibold</option></select></label><DesignerNumberField label="行高" value={value.lineHeight} min={1.1} max={2.2} step={0.05} onChange={(next) => onChange({ lineHeight: next })} /><DesignerNumberField label={isQuestion ? "Item 间距" : "Paragraph 间距"} value={isQuestion ? value.itemGap : value.paragraphGap} min={isQuestion ? 2 : 0} max={isQuestion ? 24 : 32} suffix="px" onChange={(next) => onChange(isQuestion ? { itemGap: next } : { paragraphGap: next })} /></div><div className="designer-fields-grid designer-appearance-fields"><DesignerOpacityField label="背景透明度" value={value.backgroundOpacity} onChange={(next) => onChange({ backgroundOpacity: next })} /><DesignerOpacityField label="文字透明度" value={value.textOpacity} onChange={(next) => onChange({ textOpacity: next })} /><DesignerOpacityField label="边框透明度" value={value.borderOpacity} onChange={(next) => onChange({ borderOpacity: next })} /><label className="designer-color-field"><span>背景 / 文字颜色</span><div><input type="color" value={value.backgroundColor} onChange={(event) => onChange({ backgroundColor: event.target.value })} /><input type="color" value={value.textColor} onChange={(event) => onChange({ textColor: event.target.value })} /></div></label></div></div>;
}
...
function OverlayPreferencesPanel({ value: persistedValue, onChange, onReset }: { value: OverlayPreferences; onChange: (patch: OverlayPreferencesPatch) => void; onReset: () => void }): JSX.Element {
  ...
  const resetLayout = () => { void window.interviewCopilot.overlay.resetLayout(); commit({ layoutPreset: "standard", questionWindow: { x: 120, y: 180, width: 430, height: 500 }, answerWindow: { x: 570, y: 180, width: 680, height: 500 }, controlBar: { x: 620, y: 24, width: 680, height: 50, positionMode: "top_center" } }); };
  ...
  return <section className="settings-service-card overlay-preferences-card overlay-designer-card">
    <header className="overlay-designer-header"><div><span className="step-number">04</span><div><span className="page-kicker">OVERLAY DESIGNER</span><h2>悬浮窗设计器</h2><p>右侧预览模拟真实桌面效果；拖动和调整大小会在释放后同步到真实 Overlay。</p></div></div><div className="overlay-designer-header-actions"><button className="outline-pill" onClick={onReset}>恢复全部默认</button><button className="outline-pill" onClick={undoLayout} disabled={!lastLayout.current}>撤销布局</button>{!value.behavior.liveApply && <><button className="outline-pill" onClick={cancelDraft} disabled={!draftDirty}>取消修改</button><button className="dark-pill" onClick={applyDraft} disabled={!draftDirty}>应用到真实窗口</button></>}</div></header>
    <div className="overlay-designer-layout"><div className="overlay-designer-controls">
      <section className="designer-section"><div className="designer-section-heading"><div><span>01</span><div><h3>Layout</h3><p>布局、窗口坐标、Control Bar 和吸附。</p></div></div><button className="text-button" onClick={resetLayout}>重置布局</button></div><label className="designer-select-field designer-preset-field"><span>布局预设</span><select value={value.layoutPreset} onChange={(event) => { const preset = event.target.value as OverlayPreferences["layoutPreset"]; commit({ layoutPreset: preset, ...(preset === "transparent" ? { appearance: { ...value.appearance, mode: "text_only" } } : {}) }); }}><option value="compact">Compact</option><option value="standard">Standard</option><option value="wide">Wide</option><option value="dual_screen">Dual screen</option><option value="transparent">Transparent · Text-only</option><option value="custom">Custom</option></select></label><DesignerWindowSection kind="question" title="问题导航窗口" value={value.questionWindow} onChange={updateQuestion} onReset={resetQuestion} /><DesignerWindowSection kind="answer" title="答案阅读器窗口" value={value.answerWindow} onChange={updateAnswer} onReset={resetAnswer} /><div className="designer-section-card"><div className="designer-section-card-heading"><div><strong>Control Bar</strong><small>顶部/侧边控制栏自由定位</small></div><button type="button" className="text-button" onClick={() => updateControl({ positionMode: "top_center", orientation: "horizontal" })}>重置</button></div><div className="designer-fields-grid designer-geometry-fields"><DesignerNumberField label="X" value={value.controlBar.x ?? 620} min={-2000} max={4000} suffix="px" onChange={(next) => updateControl({ x: next, positionMode: "custom" })} /><DesignerNumberField label="Y" value={value.controlBar.y ?? 24} min={-2000} max={3000} suffix="px" onChange={(next) => updateControl({ y: next, positionMode: "custom" })} /><DesignerNumberField label="宽度" value={value.controlBar.width} min={120} max={2_000} suffix="px" onChange={(next) => updateControl({ width: next })} /><DesignerNumberField label="高度" value={value.controlBar.height} min={36} max={240} suffix="px" onChange={(next) => updateControl({ height: next })} /></div><div className="designer-fields-grid"><label className="designer-select-field"><span>位置</span><select value={value.controlBar.positionMode} onChange={(event) => updateControl({ positionMode: event.target.value as OverlayControlBarPositionMode })}><option value="top_left">Top left</option><option value="top_center">Top center</option><option value="top_right">Top right</option><option value="bottom_left">Bottom left</option><option value="bottom_center">Bottom center</option><option value="bottom_right">Bottom right</option><option value="custom">Custom</option></select></label><label className="designer-select-field"><span>方向</span><select value={value.controlBar.orientation} onChange={(event) => updateControl({ orientation: event.target.value as OverlayControlBarOrientation })}><option value="horizontal">Horizontal</option><option value="vertical">Vertical</option></select></label></div><div className="designer-fields-grid"><DesignerNumberField label="字号" value={value.controlBar.fontSize} min={9} max={24} suffix="px" onChange={(next) => updateControl({ fontSize: next })} /><label className="designer-select-field"><span>字重</span><select value={value.controlBar.fontWeight} onChange={(event) => updateControl({ fontWeight: Number(event.target.value) as OverlayFontWeight })}><option value="400">400</option><option value="500">500</option><option value="600">600</option></select></label><DesignerOpacityField label="背景透明度" value={value.controlBar.backgroundOpacity} onChange={(next) => updateControl({ backgroundOpacity: next })} /><DesignerOpacityField label="文字透明度" value={value.controlBar.textOpacity} onChange={(next) => updateControl({ textOpacity: next })} /><DesignerOpacityField label="边框透明度" value={value.controlBar.borderOpacity} onChange={(next) => updateControl({ borderOpacity: next })} /></div><div className="designer-fields-grid"><label className="designer-color-field"><span>背景颜色</span><input type="color" value={value.controlBar.backgroundColor} onChange={(event) => updateControl({ backgroundColor: event.target.value })} /></label><label className="designer-color-field"><span>文字颜色</span><input type="color" value={value.controlBar.textColor} onChange={(event) => updateControl({ textColor: event.target.value })} /></label></div></div><label className="designer-check-row"><input type="checkbox" checked={value.behavior.snapEnabled} onChange={(event) => updateBehavior({ snapEnabled: event.target.checked })} /><span><strong>启用位置吸附</strong><small>屏幕边缘、窗口边缘和 Control Bar；按住 Alt 临时关闭，阈值 {value.behavior.snapThreshold}px。</small></span></label><DesignerNumberField label="吸附阈值" value={value.behavior.snapThreshold} min={8} max={16} suffix="px" onChange={(next) => updateBehavior({ snapThreshold: next })} /><button className="dark-pill designer-edit-real-button" onClick={() => void window.interviewCopilot.overlay.enterLayoutEditMode()}>编辑真实窗口</button></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>02</span><div><h3>Question Navigator</h3><p>只显示问题组和追问，不重新判断问题。</p></div></div><button className="text-button" onClick={resetQuestion}>重置本区</button></div><label className="designer-check-row"><input type="checkbox" checked={value.showTranscript} onChange={(event) => commit({ showTranscript: event.target.checked })} /><span><strong>启用问题导航</strong><small>保留 Question Group 的只读视图</small></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.showQuestionStatus} onChange={(event) => updateBehavior({ showQuestionStatus: event.target.checked })} /><span><strong>显示状态</strong></span></label></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>03</span><div><h3>Answer Reader</h3><p>答案栈只改变显示方式，不重新生成或清空回答。</p></div></div><button className="text-button" onClick={resetAnswer}>重置本区</button></div><label className="designer-check-row"><input type="checkbox" checked={value.showAnswer} onChange={(event) => commit({ showAnswer: event.target.checked })} /><span><strong>启用答案阅读器</strong><small>保留当前回答与历史答案</small></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.showAnswerStatus} onChange={(event) => updateBehavior({ showAnswerStatus: event.target.checked })} /><span><strong>显示状态</strong></span></label></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>04</span><div><h3>Interaction</h3><p>点击、穿透、滚轮和临时恢复交互。</p></div></div></div><div className="designer-fields-grid"><label className="designer-select-field"><span>Mouse Interaction Mode</span><select value={value.behavior.interactionMode} onChange={(event) => updateBehavior({ interactionMode: event.target.value as OverlayPreferences["behavior"]["interactionMode"] })}><option value="interactive">Interactive</option><option value="click_through">Click-through</option><option value="full_passthrough">Full passthrough</option></select></label><label className="designer-select-field"><span>Wheel Routing</span><select value={value.behavior.wheelRouting} onChange={(event) => updateBehavior({ wheelRouting: event.target.value as OverlayPreferences["behavior"]["wheelRouting"] })}><option value="overlay_under_cursor">Overlay under cursor</option><option value="underlying_app">Underlying app</option><option value="dual">Dual · Advanced</option></select></label><label className="designer-select-field"><span>Temporary Modifier</span><select value={value.behavior.temporaryInteractionModifier} onChange={(event) => updateBehavior({ temporaryInteractionModifier: event.target.value as OverlayPreferences["behavior"]["temporaryInteractionModifier"] })}><option value="ctrl">Ctrl</option><option value="alt">Alt</option><option value="shift">Shift</option><option value="ctrl_shift">Ctrl + Shift</option></select></label></div><div className="designer-toggle-grid"><label className="designer-check-row"><input type="checkbox" checked={value.behavior.alwaysOnTop} onChange={(event) => updateBehavior({ alwaysOnTop: event.target.checked })} /><span><strong>Always on top</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.lockLayout} onChange={(event) => updateBehavior({ lockLayout: event.target.checked })} /><span><strong>Lock layout</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.followLatestQuestion} onChange={(event) => updateBehavior({ followLatestQuestion: event.target.checked })} /><span><strong>Follow latest question</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.followLatestAnswer} onChange={(event) => updateBehavior({ followLatestAnswer: event.target.checked })} /><span><strong>Follow latest answer</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.rememberPosition} onChange={(event) => updateBehavior({ rememberPosition: event.target.checked })} /><span><strong>Remember position</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.rememberSize} onChange={(event) => updateBehavior({ rememberSize: event.target.checked })} /><span><strong>Remember size</strong></span></label></div><details className="designer-advanced-details"><summary>截图与中键</summary><div className="designer-toggle-grid"><label className="designer-check-row"><input type="checkbox" checked={value.screenshot.middleMouseEnabled} onChange={(event) => updateScreenshot({ middleMouseEnabled: event.target.checked })} /><span><strong>Middle Mouse Screenshot</strong><small>手动面试和笔试均保持独立 Hook</small></span></label><label className="designer-check-row"><input type="checkbox" checked={value.screenshot.enabledInManualInterview} onChange={(event) => updateScreenshot({ enabledInManualInterview: event.target.checked })} /><span><strong>实时面试启用</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.screenshot.enabledInExamMode} onChange={(event) => updateScreenshot({ enabledInExamMode: event.target.checked })} /><span><strong>笔试启用</strong></span></label></div><label className="designer-select-field"><span>截图区域</span><select value={value.screenshot.captureMode} onChange={(event) => updateScreenshot({ captureMode: event.target.value as OverlayPreferences["screenshot"]["captureMode"] })}><option value="current_display">Current display</option><option value="full_screen">Full screen</option><option value="fixed_region">Fixed region</option><option value="last_region">Last region</option><option value="interactive">Interactive</option></select></label>{value.screenshot.captureMode === "fixed_region" && <div className="designer-fields-grid designer-region-fields"><DesignerNumberField label="左" value={fixedRegion.x} min={0} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, x: next } })} /><DesignerNumberField label="上" value={fixedRegion.y} min={0} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, y: next } })} /><DesignerNumberField label="宽" value={fixedRegion.width} min={1} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, width: next } })} /><DesignerNumberField label="高" value={fixedRegion.height} min={1} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, height: next } })} /></div>}</details></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>05</span><div><h3>Appearance</h3><p>背景、文字、边框分离；Text-only 不隐藏文字。</p></div></div></div><div className="designer-appearance-mode-grid">{([ ["glass", "Glass", "70–85% background · blur 12–24"], ["translucent", "Translucent", "25–50% background · blur 4–12"], ["text_only", "Text-only", "0 background · text 100%"], ["custom", "Custom", "Use per-window values"] ] as const).map(([mode, label, description]) => <button key={mode} type="button" className={value.appearance.mode === mode ? "selected" : ""} onClick={() => commit({ appearance: { ...value.appearance, mode } })}><strong>{label}</strong><small>{description}</small></button>)}</div><div className="designer-fields-grid"><DesignerNumberField label="Blur" value={value.appearance.blur} min={0} max={40} suffix="px" onChange={(next) => commit({ appearance: { ...value.appearance, blur: next } })} /><DesignerNumberField label="Radius" value={value.appearance.radius} min={0} max={32} suffix="px" onChange={(next) => commit({ appearance: { ...value.appearance, radius: next } })} /><label className="designer-select-field"><span>Text Shadow</span><select value={value.appearance.textShadow} onChange={(event) => commit({ appearance: { ...value.appearance, textShadow: event.target.value as OverlayPreferences["appearance"]["textShadow"] } })}><option value="none">None</option><option value="soft">Soft · 推荐</option><option value="medium">Medium</option></select></label><label className="designer-select-field"><span>Text Outline</span><select value={value.appearance.textOutline} onChange={(event) => commit({ appearance: { ...value.appearance, textOutline: Number(event.target.value) as 0 | 0.5 | 1 } })}><option value="0">0 px</option><option value="0.5">0.5 px</option><option value="1">1 px</option></select></label></div><div className="designer-toggle-grid"><label className="designer-check-row"><input type="checkbox" checked={value.appearance.border} onChange={(event) => commit({ appearance: { ...value.appearance, border: event.target.checked } })} /><span><strong>Border</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.appearance.shadow} onChange={(event) => commit({ appearance: { ...value.appearance, shadow: event.target.checked } })} /><span><strong>Shadow</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.liveApply} onChange={(event) => updateBehavior({ liveApply: event.target.checked })} /><span><strong>Live Apply</strong><small>关闭后预览继续实时更新，点击“应用到真实窗口”才持久化</small></span></label></div></section>
    </div><aside className="overlay-settings-preview-sticky"><DesktopPreview value={value} onChange={commit} /></aside></div>
  </section>;
}
