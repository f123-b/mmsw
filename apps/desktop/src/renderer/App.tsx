...
export function App(): JSX.Element {
  ...
  const uploadKnowledgeFile = async (file: File, documentType: KnowledgeDocumentTypeOption = "auto") => {
    ...
    try {
      const imported = await window.interviewCopilot.knowledge.ingest({ knowledgeBaseId, filename: file.name, mimeType: file.type || "application/octet-stream", documentType, bytes: new Uint8Array(await file.arrayBuffer()) }) as { status?: string; error?: string };
