...
import { QUESTION_BANK_TYPE_LABELS, QUESTION_BANK_TYPES, isFactEligible, isFactReviewRequired, isFactUserActionRequired, listConflictGroups, validateLlmModelConfiguration, type ProjectConflictGroup } from "@interview-copilot/shared";
...
function ProjectLibraryManager(props: ProjectLibraryManagerProps): JSX.Element {
  ...
  const projectFacts = factFilter === "stale" ? staleProjectFacts : allProjectFacts.filter((fact) => !isConflictFact(fact) && (factFilter === "action" ? isFactUserActionRequired(fact) : factFilter === "review" ? isFactReviewRequired(fact) : factFilter === "conflict" ? false : factFilter === "responsibility" ? fact.type === "responsibility" : true));
...
export function App(): JSX.Element {
  ...
  const createProjectMemory = async () => {
    if (!profileId) return;
    const name = await requestDialog({ kind: "form", title: "新建项目", label: "项目名称", defaultValue: "新项目", required: true, confirmLabel: "创建" });
