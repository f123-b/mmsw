import { ASR_PRESETS, normalizeAsrSettings } from "@interview-copilot/shared";
...
import type { Profile, WrittenTestSession, WrittenTestSessionDetail } from "@interview-copilot/shared";
...
import { Bell, Check, ChevronRight, Command, Search, Settings2, X } from "lucide-react";
...
function SpeechScriptPage({ script, onUpload, onClear }: { script?: SpeechScript; onUpload: (file: File) => Promise<void>; onClear: () => Promise<void> }): JSX.Element {
  return <section className="simple-page speech-script-page">
    <div className="page-heading"><div><span className="page-kicker">SPEECH SCRIPT</span><h1>上传演讲稿</h1><p className="page-note">面试或会议时，在悬浮控制栏里显示、隐藏和滚动阅读。演讲稿与题目、回答窗口相互独立。</p></div><label className="dark-pill upload-project-action">{script ? "替换演讲稿" : "选择演讲稿"}<input type="file" accept=".txt,.md,.markdown,.html,.htm,.pdf,.docx,.pptx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void onUpload(file); event.target.value = ""; }} /></label></div>
    {script ? <article className="speech-script-card"><div className="speech-script-card-heading"><div><strong>{script.filename}</strong><small>最近更新 {new Date(script.updatedAt).toLocaleString("zh-CN")}</small></div><button className="text-button danger-text" onClick={() => void onClear()}>移除</button></div><pre className="speech-script-preview">{script.text}</pre></article> : <div className="knowledge-empty speech-script-empty"><strong>还没有演讲稿</strong><span>支持 TXT、Markdown、HTML、PDF、DOCX 和 PPTX；上传后会保存在本机。</span></div>}
    <div className="speech-script-help"><strong>面试中怎么用</strong><span>开始面试后，点击控制栏的“演讲稿”按钮即可显示或隐藏；滚轮会在窗口下滚动，按住 Ctrl 可临时进入交互模式。</span></div>
  </section>;
}
...
interface QuestionBankPageProps {
  ...
  onBulkDelete: (questionIds: string[]) => Promise<number>;
...
function QuestionBankPage(props: QuestionBankPageProps): JSX.Element {
  ...
  const toggleSelectVisible = () => setSelectedIds((current) => allVisibleSelected ? current.filter((id) => !visibleQuestions.some((item) => item.id === id)) : [...new Set([...current, ...visibleQuestions.map((item) => item.id)])]);
  const bulkDelete = async () => {
    if (!selectedIds.length) { props.onNotice("请先选择题目"); return; }
    const count = await props.onBulkDelete(selectedIds);
    setSelectedIds([]);
    if (selectedId && selectedIds.includes(selectedId)) resetForm();
    props.onNotice(`已删除 ${count} 道题目`);
    const [nextRows, nextTotal] = await Promise.all([props.onList(listOptions()), props.onCount(listOptions())]);
  ...
  return <section className="simple-page question-bank-page">
    ...
    <div className="question-bank-bulk-toolbar"><span>{selectedIds.length ? `已选择 ${selectedIds.length} 道` : "可勾选题目进行批量操作"}</span><button className="text-button" onClick={toggleSelectVisible} disabled={!visibleQuestions.length}>{allVisibleSelected ? "取消本页全选" : `全选本页（${visibleQuestions.length}）`}</button><button className="text-button" onClick={() => void bulkUpdate({ verified: true, stale: false })} disabled={!selectedIds.length}>标记已验证</button><button className="text-button" onClick={() => void bulkUpdate({ stale: true })} disabled={!selectedIds.length}>标记待复核</button><button className="text-button" onClick={() => void bulkUpdate({ status: "archived" })} disabled={!selectedIds.length}>归档</button><button className="text-button danger-text" onClick={() => void bulkDelete()} disabled={!selectedIds.length}>删除选中（{selectedIds.length}）</button><button className="text-button" onClick={() => setSelectedIds([])} disabled={!selectedIds.length}>清除选择</button></div>
...
function asrProviderLabel(providerType: AsrProviderType): string { return ASR_PRESETS[providerType].name; }
function asrDefaultModel(providerType: AsrProviderType): string { return ASR_PRESETS[providerType].models[0]; }
...
const EMPTY_RENDER_STATE: AudioRenderState = {
  ...
  hudState: { running: false, panelVisible: false, transcriptVisible: false, answerVisible: false, scriptVisible: false, transientLayer: "none", shareMode: false, topBarVisible: false, mouseMode: "passthrough", mode: "HIDDEN" },
  sessionState: "IDLE",
  ...
  automationMode: "AUTO",
  answerMode: "NORMAL",
  writtenTest: { running: false, answerMode: "NORMAL", screenshotStatus: "IDLE", questionCount: 0, screenshotCount: 0 },
...
const useAudioStore = create<AudioStore>((set) => ({
  ...
  setWrittenTestState: (writtenTest) => {
    ...
    answerThreadStore.reset();
    questionGroupsById.clear();
    set((current) => ({ writtenTest, runtimePhases: { ...initialRuntimePhaseState, automationMode: current.automationMode, sessionPhase: writtenTest.running ? "LISTENING" : "IDLE" }, answerText: "", answerStreaming: false, answerId: undefined, answerHistory: [], questionGroups: [], activeQuestionGroupId: undefined, activeAnswerGroupId: undefined, answerThreads: [], question: undefined, remoteTranscript: { source: "remote", final: [] }, micTranscript: { source: "mic", final: [] } }));
  },
...
function OverlayView(): JSX.Element {
  ...
  return (
    <main className="overlay-shell">
      <div className="overlay-bar"><span>有招 · {answerMode}</span><StatusPill state={state} /></div>
...
function tencentLabel(value: TencentValidationStatus): string {
  return value === "verified" ? "已验证" : value === "failed" ? "失败" : value === "stale" ? "需要重新验证" : "未验证";
}
...
function CaptureProtectionSettings({ status, validation = { desktopShare: "unverified", windowShare: "unverified" }, onToggle, onValidate = () => undefined }: { status: CaptureProtectionState; validation?: TencentValidationState; onToggle: (enabled: boolean) => void; onValidate?: (mode: "desktopShare" | "windowShare", status: TencentValidationStatus) => void }): JSX.Element {
  ...
  const protectedWindows = status.windows.filter((item) => item.protectionActive).length;
  const startValidation = () => { onToggle(true); recordValidation("desktopShare", "unverified"); recordValidation("windowShare", "unverified"); setValidationMode(true); };
  return <section className="capture-protection-settings"><div className="settings-subheading"><div><h2>共享保护 <small>Capture Protection V3</small></h2><p>每个敏感窗口都在首次显示前单独验证；任一失败会立即隐藏全部悬浮窗。</p></div><label className="switch-control"><input type="checkbox" checked={status.requested} disabled={!status.supported} onChange={(event) => onToggle(event.target.checked)} /><span /></label></div><div className="capture-protection-meta"><strong className={status.health === "PROTECTED" ? "enabled" : status.health === "DISABLED" ? "disabled" : status.health === "UNSUPPORTED" ? "unsupported" : "failed"}>总体状态：{status.health}</strong><span>范围：{status.scope === "application" ? "整个应用" : "仅悬浮窗"}</span></div><div className="capture-protection-layers"><div><span>窗口保护</span><strong>{protectedWindows} / {status.windows.length} verified</strong></div>{status.windows.map((item) => <div key={item.panel}><span>{item.panel}</span><strong>{item.protectionActive ? `✓ ${item.affinityName}` : `✕ ${item.lastError ?? "未保护"}`}</strong></div>)}<div><span>Window Capture</span><strong>{verificationLabel(status.windowCaptureVerified)}</strong></div><div><span>Display Capture</span><strong>{verificationLabel(status.displayCaptureVerified)}</strong></div><div><span>Tencent Meeting · Desktop Share</span><strong>{tencentLabel(validationState.desktopShare)}</strong></div><div><span>Tencent Meeting · Window Share</span><strong>{tencentLabel(validationState.windowShare)}</strong></div></div><button className="outline-pill tencent-validation-button" disabled={!status.supported} onClick={startValidation}>开始腾讯会议共享验证</button>{validationMode && <div className="tencent-validation-panel"><h3>VALIDATION MODE · 腾讯会议共享验证</h3><p>{status.health === "PROTECTED" ? "保护已通过。请让另一台设备上的参与者观察以下标记，依次测试桌面共享与窗口共享。" : "正在等待所有窗口达到 PROTECTED；在此之前不要开始共享。"}</p><div className="test-marker-label">CAPTURE PROTECTION V3 · TEST MARKER</div>{(["desktopShare", "windowShare"] as const).map((mode) => <div className="tencent-validation-row" key={mode}><span>{mode === "desktopShare" ? "共享整个桌面" : "共享窗口"}：{tencentLabel(validationState[mode])}</span><button className="text-button" disabled={status.health !== "PROTECTED"} onClick={() => recordValidation(mode, "verified")}>远端看不到</button><button className="text-button danger-text" disabled={status.health !== "PROTECTED"} onClick={() => recordValidation(mode, "failed")}>远端可以看到</button></div>)}<button className="outline-pill" onClick={() => setValidationMode(false)}>结束验证</button></div>}<p className="capture-protection-warning">{status.health === "PROTECTED" ? "本地 Windows Capture Protection 已通过；第三方会议软件共享仍需单独验证。" : "本地保护未通过，敏感悬浮窗已按 fail-closed 规则隐藏。"}</p>{status.lastError && <p className="capture-protection-error">共享保护失败：{status.lastError}</p>}</section>;
}

function HistoryPage({ records, search, onSearch, detail, metrics, onSelect, onDelete, onExport, writtenRecords, writtenDetail, onWrittenSelect, onWrittenDelete }: {
  ...
  writtenRecords?: WrittenTestSession[];
  writtenDetail?: WrittenTestSessionDetail;
  onWrittenSelect?: (id: string) => Promise<void>;
  onWrittenDelete?: (id: string) => Promise<void>;
}): JSX.Element {
  const [kind, setKind] = useState<"interview" | "written">("interview");
  if (kind === "written") return <WrittenHistorySection records={writtenRecords ?? []} detail={writtenDetail} onSelect={onWrittenSelect ?? (async () => undefined)} onDelete={onWrittenDelete ?? (async () => undefined)} onBack={() => setKind("interview")} />;
  ...
  return <section className="simple-page history-page">
    <div className="page-heading"><div><span className="page-kicker">INTERVIEW HISTORY</span><h1>面试记录</h1><p className="page-note">搜索、查看和删除历史记录；删除会同时清理这场面试的转写、问题和回答。</p></div><div className="detail-actions"><button className="outline-pill selected" onClick={() => setKind("interview")}>面试</button><button className="outline-pill" onClick={() => setKind("written")}>笔试（{writtenRecords?.length ?? 0}）</button><input className="inline-search" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="搜索日期、档案或状态" /></div></div>
...
function WrittenHistorySection({ records, detail, onSelect, onDelete, onBack }: { records: WrittenTestSession[]; detail?: WrittenTestSessionDetail; onSelect: (id: string) => Promise<void>; onDelete: (id: string) => Promise<void>; onBack: () => void }): JSX.Element {
  const [images, setImages] = useState<Record<string, string>>({});
  const [lightboxIndex, setLightboxIndex] = useState<number>();
  useEffect(() => {
    let disposed = false;
    if (!detail) { setImages({}); return () => { disposed = true; }; }
    void Promise.all(detail.screenshots.map(async (shot) => [shot.id, await window.interviewCopilot.writtenTestHistory.getImage(detail.session.id, shot.id)] as const)).then((entries) => { if (!disposed) setImages(Object.fromEntries(entries.filter((entry): entry is [string, string] => Boolean(entry[1])))); }).catch(() => undefined);
    return () => { disposed = true; };
  }, [detail?.session.id, detail?.screenshots.length]);
  const shots = detail?.screenshots ?? [];
  return <section className="simple-page history-page">
    <div className="page-heading"><div><span className="page-kicker">WRITTEN TEST HISTORY</span><h1>笔试记录</h1><p className="page-note">每次截图、题型、结构化答案和原图都保存在本地。</p></div><button className="outline-pill" onClick={onBack}>返回面试记录</button></div>
    <div className="history-layout"><div className="clean-list history-record-list">{records.map((record) => <div className={`clean-list-row history-record-row ${detail?.session.id === record.id ? "selected" : ""}`} key={record.id}><button className="row-main-button" onClick={() => void onSelect(record.id)}><strong>{new Date(record.startedAt).toLocaleString()}</strong><small>{record.status === "COMPLETED" ? "已完成" : record.status === "ABORTED" ? "已中断" : "进行中"} · {record.questionCount} 题 · {record.screenshotCount} 张截图 · {record.profileId}</small></button><button className="history-delete-button" onClick={() => void onDelete(record.id)}>删除</button></div>)}{records.length === 0 && <div className="knowledge-empty"><strong>还没有笔试记录</strong><span>进入笔试模式并完成一次截图后，记录会显示在这里。</span></div>}</div>{detail ? <div className="detail-sheet history-detail-sheet"><div className="history-detail-actions"><span>{detail.session.title} · {detail.session.id.slice(0, 12)}</span><button className="outline-pill danger-outline" onClick={() => void onDelete(detail.session.id)}>删除本次笔试</button></div><div className="detail-metrics"><span>题目 <strong>{detail.questions.length}</strong></span><span>截图 <strong>{detail.screenshots.length}</strong></span><span>已结构化 <strong>{detail.questions.filter((question) => question.answer).length}</strong></span></div><div className="written-history-gallery">{shots.map((shot, index) => <button key={shot.id} className="written-history-thumb" onClick={() => setLightboxIndex(index)}>{images[shot.id] ? <img src={images[shot.id]} alt={`第 ${index + 1} 张原图`} /> : <span>加载中…</span>}<small>{String(index + 1).padStart(3, "0")}</small></button>)}</div><div className="history-timeline">{detail.questions.map((question) => <article className="history-entry history-entry-ai" key={question.id}><div className="history-entry-heading"><b>第 {question.sequence} 题 · {question.questionType}</b><time>{new Date(question.createdAt).toLocaleTimeString()}</time></div><div className="history-entry-text">{question.normalizedQuestion || question.rawQuestionText}</div>{question.answerText ? <pre className="written-explanation">{question.answerText}</pre> : <p>尚无完整答案，请核对原图。</p>}</article>)}</div></div> : <div className="detail-sheet history-empty-detail"><strong>选择一场笔试</strong><span>这里会展示题目时间线、原图缩略图和答案详情。</span></div>}</div>
    {lightboxIndex !== undefined && shots[lightboxIndex] && <div className="written-lightbox" role="dialog" aria-label="查看原始截图" onClick={() => setLightboxIndex(undefined)}><button onClick={(event) => { event.stopPropagation(); setLightboxIndex((current) => current === undefined ? 0 : Math.max(0, current - 1)); }}>‹</button>{images[shots[lightboxIndex].id] && <img src={images[shots[lightboxIndex].id]} alt="笔试原始截图大图" />}
      <button onClick={(event) => { event.stopPropagation(); setLightboxIndex((current) => current === undefined ? 0 : Math.min(shots.length - 1, current + 1)); }}>›</button></div>}
  </section>;
}
...
export function App(): JSX.Element {
  ...
  const [llmApiProtocol, setLlmApiProtocol] = useState<import("@interview-copilot/shared").LlmApiProtocol>("openai-chat");
  ...
  const [asrProtocol, setAsrProtocol] = useState<"auto" | "qwen-http" | "qwen-realtime" | "dashscope-streaming" | "openai-transcription">("auto");
  ...
  const [captureProtection, setCaptureProtection] = useState<CaptureProtectionState>({ platform: "win32", supported: false, requested: true, scope: "application", health: "UNSUPPORTED", windows: [], fullyProtected: false, partiallyProtected: false, apiApplied: false, failClosed: true, osFlagApplied: false, enabled: true, applied: false, externalCaptureVerified: null, displayCaptureVerified: null, windowCaptureVerified: null });
  const [overlayPreferences, setOverlayPreferences] = useState<OverlayPreferences>(DEFAULT_OVERLAY_PREFERENCES);
  const [speechScript, setSpeechScript] = useState<SpeechScript>();
  ...
  const [writtenHistoryRecords, setWrittenHistoryRecords] = useState<WrittenTestSession[]>([]);
  const [writtenHistoryDetail, setWrittenHistoryDetail] = useState<WrittenTestSessionDetail>();
  ...
  const applyLlmSettings = (settings: PublicProviderSettings): void => {
    ...
    setLlmApiProtocol(settings.apiProtocol ?? (/api\.anthropic\.com/.test(settings.baseUrl) ? "anthropic-messages" : /\/responses\/?$/.test(settings.baseUrl) ? "openai-responses" : "openai-chat"));
  ...
  const applyProviderSettings = (settings: ProviderCenterPublicConfig | undefined): void => {
    ...
    setAsrModel(settings.asr.model); setAsrProtocol(settings.asr.asrProtocol ?? "auto"); setAsrAppId(settings.asr.asrAppId ?? "");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setSpeechScript(initialSpeechScript);
        ...
        setWrittenHistoryRecords(await window.interviewCopilot.writtenTestHistory.list());
    ...
    const cleanups = [
      ...
      window.interviewCopilot.events.onSpeechScript(setSpeechScript),
      ...
      window.interviewCopilot.events.onWrittenTestState((state) => { store.setWrittenTestState(state); if (!state.running) void window.interviewCopilot.writtenTestHistory.list().then(setWrittenHistoryRecords); }),
      window.interviewCopilot.events.onWrittenTestDocument((payload) => { void window.interviewCopilot.writtenTestHistory.list().then(setWrittenHistoryRecords); if (writtenHistoryDetail?.session.id === payload.question.sessionId) void window.interviewCopilot.writtenTestHistory.get(payload.question.sessionId).then((detail) => { if (detail) setWrittenHistoryDetail(detail); }); }),
  ...
  const saveLlmProfile = async (): Promise<{ config: ProviderCenterPublicConfig; profileId: string }> => {
    ...
    let next = await window.interviewCopilot.settings.saveLlmProfile(input);
    ...
    // Saving/testing a profile means the user expects the interview runtime to
    // use it. Older builds tested the selected profile but kept an empty
    // default profile active, so the test passed while interview startup failed.
    if (savedProfileId !== next.activeLlmProfileId) {
      const activated = await window.interviewCopilot.settings.activateLlmProfile(savedProfileId);
      if (!activated) throw new Error("模型配置保存后未能启用");
      next = activated;
    }
  ...
  const importProjectQuestionBank = async (projectId: string, file: File): Promise<ProjectQuestionBankImportReport | undefined> => {
    ...
    try {
      ...
      await refreshProjectState(profileId);
      store.setNotice(`项目题库已导入：识别 ${report.recognizedQuestions} 题，新增答案 ${report.importedAnswers} 条，请核对后确认；原有确认状态保持不变`);
      return report;
    } catch (error) {
  ...
  const generateProjectQuestionBank = async (projectId: string): Promise<ProjectQaGenerationResult | undefined> => {
    try {
      ...
      await refreshProjectState(profileId);
      store.setNotice(`已生成 ${result.generated} 条项目问题，答案卡均待确认${result.rejected ? `；拦截 ${result.rejected} 条证据不足或资料已变化的候选答案` : ""}${result.excludedFactCount ? `；排除 ${result.excludedFactCount} 条不可用事实` : ""}`);
      return result;
    } catch (error) {
  ...
  const updateProjectOwnership = async (projectId: string, input: { ownershipMode?: "personal" | "team" | "partial" | "reference"; ownershipNote?: string; projectIntroduction?: string }) => {
    const updated = await window.interviewCopilot.projects.update(projectId, input);
    if (updated) {
      await refreshProjectState(profileId);
      store.setNotice(input.projectIntroduction !== undefined ? "项目介绍已保存，实时面试会优先直接展示" : "项目归属边界已保存");
    }
  };
  ...
  const saveSelfIntroduction = async (text: string, targetDurationSeconds: number, language: string) => {
    if (!profileId) return;
    try {
      const saved = await window.interviewCopilot.selfIntroduction.save({ profileId, resumeHash: "", text, source: "manual", approved: true, targetDurationSeconds, language });
      if (saved) setSelfIntroduction(saved);
      store.setNotice(text.trim() ? "自我介绍已保存，面试时将按原稿展示" : "已清空自我介绍，面试时将根据简历生成");
  ...
  const uploadSelfIntroduction = async (file: File) => {
    ...
    try {
      ...
      if (saved) setSelfIntroduction(saved);
      store.setNotice("自我介绍稿件已导入，面试时将按原稿展示");
  ...
  const uploadSpeechScript = async (file: File) => {
    if (!file) return;
    try {
      const saved = await window.interviewCopilot.speechScript.upload({ filename: file.name, mimeType: file.type || "application/octet-stream", bytes: new Uint8Array(await file.arrayBuffer()) });
      setSpeechScript(saved);
      store.setNotice(`演讲稿已上传：${file.name}`);
    } catch (error) {
      store.setNotice(`演讲稿上传失败：${userFacingError(error)}`);
    }
  };
  const clearSpeechScript = async () => {
    const confirmed = await requestDialog({ kind: "confirm", title: "移除当前演讲稿？", description: "移除后不会影响面试记录和其他资料。", confirmLabel: "移除演讲稿" });
    if (confirmed !== true) return;
    await window.interviewCopilot.speechScript.clear();
    setSpeechScript(undefined);
    store.setNotice("演讲稿已移除");
  };
  ...
  const testProvider = async (section: "llm" | "asr" | "embedding") => {
    ...
    try {
      ...
      if (section === "asr") await window.interviewCopilot.settings.update("asr", { providerName: asrProviderLabel(asrProviderType), providerType: asrProviderType, asrProtocol, asrAppId, baseUrl: asrBaseUrl.trim(), model: asrModel.trim() || asrDefaultModel(asrProviderType), language: asrLanguage, apiKey: asrApiKey || undefined, timeoutMs: asrProviderType === "assemblyai" ? 60_000 : 30_000, maxRetries: 2 });
  ...
  const fetchProviderModels = async (section: "llm" | "asr" | "embedding") => {
    ...
    try {
      ...
      if (section === "llm") {
        ...
        // Catalogs are incomplete and capability categories are heuristic.
        // Preserve every manual ID; a refresh must not clear working models.
        if (!llmModel.trim()) setLlmModel(first("general") || first("fast") || first("reasoning"));
        if (!fastModel.trim()) setFastModel(first("fast") || first("general"));
        if (!normalModel.trim()) setNormalModel(first("general") || first("fast"));
        if (!deepModel.trim()) setDeepModel(first("reasoning") || first("general"));
  ...
  const chooseLlmPreset = (preset: "deepseek" | "qwen" | "custom" | "openai" | "claude" | "gemini") => {
    setLlmApiProtocol(preset === "openai" ? "openai-responses" : preset === "claude" ? "anthropic-messages" : "openai-chat");
    setModelCatalogs((current) => ({ ...current, llm: undefined }));
    setProviderTests((current) => ({ ...current, llm: "供应商已更改 · 请保存配置并获取模型" }));
    if (preset !== "custom") {
      setLlmProfileId(""); setLlmApiKey(""); setLlmProfileName(preset);
      setFallbackModel(""); setQuestionRecognitionModel(""); setProfileBuilderModel(""); setProjectAnalyzerModel(""); setQuestionBankModel(""); setChatModel(""); setPostInterviewModel(""); setPreparationModel("");
    }
    if (["openai", "claude", "gemini"].includes(preset)) {
      // A different endpoint gets a new profile; never send a saved vendor key
      // to another vendor accidentally, nor inherit incompatible task IDs.
      setLlmProfileId(""); setLlmApiKey(""); setLlmProfileName(preset);
      setLlmProviderName(preset); setLlmBaseUrl(preset === "openai" ? "https://api.openai.com/v1" : preset === "claude" ? "https://api.anthropic.com/v1" : "https://generativelanguage.googleapis.com/v1beta/openai");
      setLlmModel(""); setFastModel(""); setNormalModel(""); setDeepModel(""); setVisionModel("");
      setFallbackModel(""); setQuestionRecognitionModel(""); setProfileBuilderModel(""); setProjectAnalyzerModel(""); setQuestionBankModel(""); setChatModel(""); setPostInterviewModel(""); setPreparationModel("");
  ...
  const specialPageContent = page === "knowledge"
    ...
    : page === "speech-script"
      ? <SpeechScriptPage script={speechScript} onUpload={uploadSpeechScript} onClear={clearSpeechScript} />
  ...
  const modernPageContent = specialPageContent ?? (() => {
   ...
   if (String(page) === "personal-memory") return <><PersonalMemoryPage memory={projectMemory} stats={projectMemoryStats} rebuilding={projectMemoryRunning} onRebuild={() => void rebuildProjectMemory()} /><MemoryGovernancePanel memory={projectMemory} facts={projectFacts} jobTargets={jobTargets} analysisRuns={knowledgeAnalysisRuns} retrievalRuns={retrievalRuns} onVerifyFact={verifyProjectFact} /></>;
    ...
    if (page === "history") return <HistoryPage records={historyRecords} writtenRecords={writtenHistoryRecords} writtenDetail={writtenHistoryDetail} search={historySearch} onSearch={setHistorySearch} detail={historyDetail} metrics={historyMetrics} onSelect={async (recordId) => { const [metrics, detail] = await Promise.all([window.interviewCopilot.history.analyze(recordId), window.interviewCopilot.history.get(recordId)]); if (metrics) setHistoryMetrics({ id: recordId, ...metrics }); if (detail) setHistoryDetail(detail as HistoryDetail); }} onWrittenSelect={async (sessionId) => { const detail = await window.interviewCopilot.writtenTestHistory.get(sessionId); if (detail) setWrittenHistoryDetail(detail); }} onExport={async (recordId) => { try { const result = await window.interviewCopilot.history.export(recordId); if (!result.canceled && result.path) store.setNotice(`面试记录已导出：${result.path}`); } catch (error) { store.setNotice(`导出失败：${userFacingError(error)}`); } }} onDelete={async (recordId) => { const confirmed = await requestDialog({ kind: "confirm", title: "删除这场面试记录？", description: "这会同时删除该场面试的转写、识别问题和 AI 回答，操作无法撤销。", confirmLabel: "删除记录" }); if (confirmed !== true) return; await window.interviewCopilot.history.delete(recordId); setHistoryRecords((current) => current.filter((record) => record.id !== recordId)); if (historyDetail?.interview.id === recordId) { setHistoryDetail(undefined); setHistoryMetrics(undefined); } store.setNotice("面试记录已删除"); }} onWrittenDelete={async (sessionId) => { const confirmed = await requestDialog({ kind: "confirm", title: "删除这次笔试记录？", description: "题目、原始截图、缩略图和结构化答案会一并删除，操作无法撤销。", confirmLabel: "删除笔试记录" }); if (confirmed !== true) return; await window.interviewCopilot.writtenTestHistory.delete(sessionId); setWrittenHistoryRecords((current) => current.filter((record) => record.id !== sessionId)); if (writtenHistoryDetail?.session.id === sessionId) setWrittenHistoryDetail(undefined); store.setNotice("笔试记录已删除"); }} />;
 })();

  const pageTitle = page === "home" ? "工作台" : page === "interview" ? "实时面试" : page === "written-test" ? "笔试解题" : page === "speech-script" ? "上传演讲稿" : page === "preparation" ? "面试准备" : page === "profiles" ? "面试档案" : page === "project-library" ? "项目详情" : page === "knowledge" ? "资料库" : page === "personal-memory" ? "项目知识审核" : page === "question-bank" ? "通用题库" : page === "job-targets" ? "岗位要求" : page === "history" ? "面试历史" : page === "help" ? "帮助与快速开始" : "设置";
  const commandItems: Array<{ label: string; hint: string; action: () => void }> = [
    { label: "开始面试", hint: "打开面试设置", action: () => { setCommandPaletteOpen(false); openInterviewSetup(); } },
    { label: "新建对话", hint: "开始一段新的准备对话", action: () => { setCommandPaletteOpen(false); beginNewConversation(); } },
    { label: "打开面试档案", hint: "管理简历与个人信息", action: () => { setCommandPaletteOpen(false); setPage("profiles"); } },
    { label: "打开项目库", hint: "查看项目与结构化经验", action: () => { setCommandPaletteOpen(false); setPage("project-library"); } },
    { label: "查看面试历史", hint: "回顾过去的面试记录", action: () => { setCommandPaletteOpen(false); setPage("history"); } },
    { label: "打开设置", hint: "调整应用和服务配置", action: () => { setCommandPaletteOpen(false); setPage("settings"); } }
  ];
  const filteredCommands = commandItems.filter((item) => `${item.label}${item.hint}`.toLowerCase().includes(commandQuery.trim().toLowerCase()));
  if (isOverlay) return <OverlayRoot surface={overlaySurface} mic={store.mic} system={store.system} state={store.state} sessionState={store.sessionState} realtimeState={store.realtimeState} operationMode={store.operationMode} overlayMode={store.overlayMode} hudState={store.hudState} runtimePhases={store.runtimePhases} runtimeNotice={store.notice?.text} automationMode={store.automationMode} answerMode={store.answerMode} writtenTest={store.writtenTest} screenshot={store.screenshot} question={store.question} answerText={store.answerText} answerStreaming={store.answerStreaming} questionGroups={store.questionGroups} activeQuestionGroupId={store.activeQuestionGroupId} activeAnswerGroupId={store.activeAnswerGroupId} answerThreads={store.answerThreads} remoteTranscript={store.remoteTranscript} micTranscript={store.micTranscript} speechScript={speechScript} captureProtectionEnabled={captureProtection.requested} captureProtectionSupported={captureProtection.supported} captureProtectionOsFlagApplied={captureProtection.osFlagApplied} captureProtectionDisplayVerified={captureProtection.displayCaptureVerified} captureProtectionLastError={captureProtection.lastError} captureTest={captureTest} onToggleCaptureProtection={() => void toggleCaptureProtection(!captureProtection.requested)} onToggleMode={() => void window.interviewCopilot.overlay.setMode(store.overlayMode === "interactive" ? "passive" : "interactive")} onToggleAutomation={toggleAutomation} onAnswerLatest={() => window.interviewCopilot.interview.answerLatest().catch((error) => { store.setNotice(`回答最新问题失败：${userFacingError(error)}`); })} onAnswerScreenshot={async () => { try { await answerScreenshotForMode(store.operationMode); } catch (error) { store.setNotice(`截图失败：${userFacingError(error)}`); } }} onEndInterview={() => store.operationMode === "WRITTEN_TEST" ? window.interviewCopilot.writtenTest.stop().then(() => undefined) : window.interviewCopilot.interview.stop()} onHideAll={() => void window.interviewCopilot.overlay.hideAll()} onShowAll={() => void window.interviewCopilot.overlay.showAll()} onTogglePanels={() => void window.interviewCopilot.overlay.toggleAll()} onToggleTranscript={() => void window.interviewCopilot.overlay.toggleTranscript()} onToggleAnswer={() => void window.interviewCopilot.overlay.toggleAnswer()} onToggleScript={() => void window.interviewCopilot.overlay.toggleScript()} onToggleShortcuts={() => void window.interviewCopilot.overlay.toggleShortcuts()} onRequestEndInterview={() => void window.interviewCopilot.overlay.requestEndInterview()} onToggleShare={() => void window.interviewCopilot.overlay.toggleShareMode()} />;
  ...
  return (
    <main className="app-shell modern-shell">
      ...
      <section className={`content-shell page-${page}`}>
        <div className="modern-topbar"><div className="topbar-context"><span className="topbar-breadcrumb">{page === "project-library" ? "项目库" : "有招"}</span><span className="topbar-slash">/</span><strong>{page === "project-library" ? "项目详情" : pageTitle}</strong></div><div className="topbar-actions"><button className="topbar-search-button" aria-label="搜索或打开快捷操作" onClick={() => { setCommandQuery(""); setCommandSelection(0); setCommandPaletteOpen(true); }}><Search size={15} strokeWidth={1.8} /><span>搜索面试、项目或资料</span><kbd><Command size={12} />K</kbd></button><button className="topbar-icon-button" aria-label="通知"><Bell size={16} strokeWidth={1.8} /></button>{page === "project-library" && <button className="topbar-icon-button" aria-label="项目库设置" onClick={() => setPage("settings")}><Settings2 size={16} strokeWidth={1.8} /></button>}<span className="topbar-profile">{selectedProfile ? selectedProfile.name : "未选择档案"}</span><button className="dark-pill start-interview" onClick={openInterviewSetup}>开始面试 <span>↗</span></button></div></div>
      ...
      {commandPaletteOpen && <div className="command-palette-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setCommandPaletteOpen(false); }}><section className="command-palette" role="dialog" aria-modal="true" aria-label="快捷操作"><div className="command-palette-search"><Search size={17} strokeWidth={1.8} /><input autoFocus value={commandQuery} onChange={(event) => { setCommandQuery(event.target.value); setCommandSelection(0); }} onKeyDown={(event) => { if (event.key === "Escape") setCommandPaletteOpen(false); if (event.key === "ArrowDown") { event.preventDefault(); setCommandSelection((current) => filteredCommands.length ? (current + 1) % filteredCommands.length : 0); } if (event.key === "ArrowUp") { event.preventDefault(); setCommandSelection((current) => filteredCommands.length ? (current - 1 + filteredCommands.length) % filteredCommands.length : 0); } if (event.key === "Enter" && filteredCommands[commandSelection]) filteredCommands[commandSelection].action(); }} placeholder="搜索或运行操作" /><kbd>⌘ K</kbd><button aria-label="关闭快捷操作" onClick={() => setCommandPaletteOpen(false)}><X size={16} /></button></div><div className="command-palette-list">{filteredCommands.length === 0 ? <div className="command-palette-empty">没有匹配的操作</div> : filteredCommands.map((item, index) => <button className={`command-palette-item ${index === commandSelection ? "selected" : ""}`} key={item.label} onMouseEnter={() => setCommandSelection(index)} onClick={item.action}><span><strong>{item.label}</strong><small>{item.hint}</small></span>{index === commandSelection ? <Check size={15} /> : <ChevronRight size={15} />}</button>)}</div><footer><span>↑↓ 选择</span><span>Enter 执行</span><span>Esc 关闭</span></footer></section></div>}
