    ...
    <main className="app-shell">
      ...
      <section className="content-shell">
        ...
        <div className="content-scroll">
          {page === "settings" && <section className="panel settings-panel"><div className="panel-heading"><div><div className="eyebrow">PROVIDER CENTER</div><h3>Provider 配置</h3></div><span className="muted">API Key 仅进入系统安全存储</span></div><div className="dashboard-grid"><div><label className="device-field"><span>LLM Provider / Base URL</span><input value={llmBaseUrl} onChange={(event) => setLlmBaseUrl(event.target.value)} placeholder="https://api.openai.com" /></label><label className="device-field"><span>默认 LLM Model</span><input value={llmModel} onChange={(event) => setLlmModel(event.target.value)} placeholder="gpt-4o-mini" /></label><label className="device-field"><span>FAST Model</span><input value={fastModel} onChange={(event) => setFastModel(event.target.value)} placeholder="留空使用默认模型" /></label><label className="device-field"><span>NORMAL Model</span><input value={normalModel} onChange={(event) => setNormalModel(event.target.value)} placeholder="留空使用默认模型" /></label><label className="device-field"><span>DEEP Model</span><input value={deepModel} onChange={(event) => setDeepModel(event.target.value)} placeholder="留空使用默认模型" /></label><label className="device-field"><span>VISION Model</span><input value={visionModel} onChange={(event) => setVisionModel(event.target.value)} placeholder="留空使用默认模型" /></label><label className="device-field"><span>LLM API Key</span><input value={llmApiKey} onChange={(event) => setLlmApiKey(event.target.value)} type="password" placeholder={providerSettings?.llm.hasApiKey ? "已配置，留空表示不修改" : "粘贴 API Key"} /></label></div><div><label className="device-field"><span>ASR Provider</span><select value={asrProviderType} onChange={(event) => { const value = event.target.value as "deepgram" | "custom-gateway"; setAsrProviderType(value); setAsrProviderName(value === "deepgram" ? "Deepgram" : "Custom WebSocket ASR Gateway"); }}><option value="deepgram">Deepgram Direct</option><option value="custom-gateway">Custom Gateway（高级）</option></select></label><label className="device-field"><span>ASR Model</span><input value={asrModel} onChange={(event) => setAsrModel(event.target.value)} placeholder="nova-3" /></label><label className="device-field"><span>ASR Language</span><select value={asrLanguage} onChange={(event) => setAsrLanguage(event.target.value as "zh-CN" | "en-US" | "multi")}><option value="zh-CN">中文（简体）</option><option value="en-US">English</option><option value="multi">Multilingual</option></select></label><label className="device-field"><span>Deepgram API Key</span><input value={asrApiKey} onChange={(event) => setAsrApiKey(event.target.value)} type="password" placeholder={providerSettings?.asr.hasApiKey ? "已配置，留空表示不修改" : "粘贴 Deepgram API Key"} /></label>{asrProviderType === "deepgram" ? <p className="muted">Deepgram Direct 在 Electron Main 中连接，Key 不会进入 Renderer、URL 或 Gateway。</p> : <><label className="device-field"><span>Custom Gateway WebSocket URL</span><input value={asrBaseUrl} onChange={(event) => setAsrBaseUrl(event.target.value)} placeholder="ws://127.0.0.1:8787" /></label><label className="device-field"><span>短期 Gateway Token</span><input value={realtimeTicket} onChange={(event) => setRealtimeTicket(event.target.value)} type="password" placeholder="仅放入 WebSocket ticket 查询参数" /></label><p className="muted">Custom Gateway 仅用于高级场景；长期 Deepgram Key 不会通过该协议传递。</p></>}</div></div><button className="primary-button" onClick={() => void saveProviderSettings()}>保存 Provider 配置</button></section>}
          ...
          <section className="panel realtime-panel"><div className="panel-heading"><div><div className="eyebrow">REALTIME ASR</div><h3>MIC / REMOTE Transcript</h3></div><StatusPill state={store.realtimeState.toUpperCase()} /></div><div className="transcript-grid"><TranscriptColumn label="REMOTE / 面试官" snapshot={store.remoteTranscript} /><TranscriptColumn label="MIC / 用户" snapshot={store.micTranscript} /></div><div className="diagnostic-list"><div><span>Provider</span><small>{store.asrDiagnostics.provider} · {store.asrDiagnostics.model || "—"} · {store.asrDiagnostics.language || "—"}</small></div><div><span>MIC ASR</span><small>{store.asrDiagnostics.micState}</small></div><div><span>REMOTE ASR</span><small>{store.asrDiagnostics.remoteState}</small></div><div><span>Latency</span><small>partial {store.asrDiagnostics.lastPartialLatencyMs ?? "—"}ms · final {store.asrDiagnostics.lastFinalLatencyMs ?? "—"}ms</small></div><div><span>Recovery</span><small>reconnect {store.asrDiagnostics.reconnectCount} · dropped PCM {store.asrDiagnostics.droppedPcmPackets}</small></div></div><details className="advanced-settings"><summary>高级连接：Custom Gateway</summary><div className="realtime-connect"><input value={realtimeUrl} onChange={(event) => setRealtimeUrl(event.target.value)} placeholder="ws://127.0.0.1:8787" aria-label="Realtime WebSocket URL" /><button className="secondary-button" onClick={() => void connectRealtime()}>连接</button><button className="ghost-button" onClick={() => void disconnectRealtime()}>断开</button></div><p className="muted">正常模式使用 Deepgram Direct；Custom Gateway 只在设置中明确选择后使用。API Key 和 Gateway token 不显示在诊断中。</p></details><details className="advanced-settings"><summary>问题判断诊断（最近 {store.questionDiagnostics.length} 条）</summary><div className="diagnostic-list">{store.questionDiagnostics.slice(-8).map((item, index) => <div key={`${item.text}-${index}`}><span>{item.confirmed ? "confirmed" : item.ignoredReason ?? (item.candidate ? "candidate" : "ignored")}</span><small>score {item.questionScore.toFixed(2)}{item.dedupeScore === undefined ? "" : ` · dedupe ${item.dedupeScore.toFixed(2)}`} · {item.text}</small></div>)}</div></details></section>
