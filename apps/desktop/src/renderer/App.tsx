...
import { QUESTION_BANK_TYPE_LABELS, QUESTION_BANK_TYPES, isFactReviewRequired, isFactUserActionRequired, validateLlmModelConfiguration } from "@interview-copilot/shared";
...
function ProjectLibraryManager(props: ProjectLibraryManagerProps): JSX.Element {
  ...
  const projectFacts = factFilter === "stale" ? staleProjectFacts : allProjectFacts.filter((fact) => factFilter === "action" ? isFactUserActionRequired(fact) : factFilter === "review" ? isFactReviewRequired(fact) : factFilter === "conflict" ? fact.status === "conflicting" || fact.conflictStatus === "conflicting" : factFilter === "responsibility" ? fact.type === "responsibility" : true);
  ...
  return <section className="simple-page project-library-manager">
    ...
    <div className="project-library-summary"><span>项目 <strong>{props.stats.projects}</strong></span><span>可信事实 <strong>{props.stats.eligibleFacts}</strong></span><span>待本人确认 <strong>{props.stats.userActionRequiredFacts}</strong></span><span>冲突 <strong>{props.stats.conflictingFacts}</strong></span><span>项目题 <strong>{props.stats.interviewQuestions}</strong></span>{props.stats.staleFacts > 0 && <span>已失效 <strong>{props.stats.staleFacts}</strong></span>}</div>
    ...
    {selectedProject && <>
      ...
      {tab === "facts" && <div className="fact-review-grid"><div className="fact-review-toolbar" role="tablist" aria-label="事实筛选"><button className={factFilter === "action" ? "active" : ""} onClick={() => setFactFilter("action")}>需要本人确认 {detailStats?.userActionRequiredFacts ?? 0}</button><button className={factFilter === "review" ? "active" : ""} onClick={() => setFactFilter("review")}>系统待复核 {detailStats?.reviewRequiredFacts ?? 0}</button><button className={factFilter === "conflict" ? "active" : ""} onClick={() => setFactFilter("conflict")}>冲突 {detailStats?.conflictingFacts ?? 0}</button><button className={factFilter === "responsibility" ? "active" : ""} onClick={() => setFactFilter("responsibility")}>职责</button><button className={factFilter === "all" ? "active" : ""} onClick={() => setFactFilter("all")}>全部 {allProjectFacts.length}</button>{(detailStats?.staleFacts ?? 0) > 0 && <button className={factFilter === "stale" ? "active" : ""} onClick={() => setFactFilter("stale")}>已失效 {detailStats?.staleFacts}</button>}</div><div className="project-fact-cards">{[...projectFacts].sort((a, b) => (b.conflictStatus === "conflicting" ? 1 : 0) - (a.conflictStatus === "conflicting" ? 1 : 0) || (b.type === "responsibility" ? 1 : 0) - (a.type === "responsibility" ? 1 : 0)).map((fact) => <article className={`fact-review-card fact-review-${fact.status ?? "active"}`} key={fact.id}><header><div><span className="fact-type-label">{PROJECT_FACT_LABELS[fact.type]}</span><h3>{fact.title}</h3></div><strong>{statusText(fact)}</strong></header><p>{fact.content}</p><div className="fact-evidence-line"><span>来源：{fact.sourceIds.map(sourceTitle).join("、") || "未关联"}</span><span>{fact.evidenceLevel ?? "pending"} · {fact.ownership ?? "unknown"} · 置信度 {Math.round(fact.confidence * 100)}%</span></div>{fact.evidence?.[0] && <button className="evidence-link" onClick={() => setSelectedEvidence(fact)}>查看证据：“{fact.evidence[0].quote.slice(0, 80)}”</button>}{fact.conflictGroupId && <small className="page-note">冲突组：{fact.conflictGroupId}</small>}<footer>{!fact.stale && fact.status !== "rejected" && isFactUserActionRequired(fact) && <button className="dark-pill" onClick={() => void (fact.conflictGroupId ? props.onResolveConflict(fact.conflictGroupId, fact.id, false) : props.onReviewFact(fact.id, "active"))}>{fact.status === "conflicting" ? "采用此版本" : fact.type === "responsibility" ? "确认本人职责" : "确认事实"}</button>}{!fact.stale && fact.status !== "rejected" && <button className="outline-pill" onClick={() => void props.onReviewFact(fact.id, "rejected")}>{fact.status === "conflicting" ? "排除此版本" : "不正确"}</button>}{fact.status === "rejected" && <button className="outline-pill" onClick={() => void props.onReviewFact(fact.id, "pending_review")}>恢复待确认</button>}</footer></article>)}{projectFacts.length === 0 && <div className="knowledge-empty"><strong>暂无符合条件的事实</strong><span>{factFilter === "action" ? "当前没有需要你确认的事实。" : factFilter === "review" ? "当前没有系统待复核事实。" : "重新分析或补充项目资料。"}</span></div>}</div><aside className="fact-review-summary"><strong>风险优先</strong><span>职责必须本人确认</span><span>冲突事实需成组处理</span><span>结果 / 指标需要证据</span><span>当前筛选 {projectFacts.length} 条 · 项目事实共 {allProjectFacts.length} 条</span></aside></div>}
...
export function App(): JSX.Element {
  ...
  const [projectMemoryStats, setProjectMemoryStats] = useState<ProjectMemoryStats>({ projects: 0, modules: 0, technicalPoints: 0, problems: 0, interviewQuestions: 0, questions: 0, facts: 0, eligibleFacts: 0, reviewRequiredFacts: 0, userActionRequiredFacts: 0, conflictingFacts: 0, staleFacts: 0 });
  ...
  useEffect(() => {
    if (!profileId) {
      ...
      setProjectMemoryStats({ projects: 0, modules: 0, technicalPoints: 0, problems: 0, interviewQuestions: 0, questions: 0, facts: 0, eligibleFacts: 0, reviewRequiredFacts: 0, userActionRequiredFacts: 0, conflictingFacts: 0, staleFacts: 0 });
      setProjectFacts([]);
