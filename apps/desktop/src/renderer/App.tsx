  ...
  return <section className="simple-page project-library-manager">
    ...
    {selectedProject && <>
      ...
      <section className="project-agent-panel" aria-label="项目资料 AI Agent">
        ...
        <div className="project-agent-conversation">{visibleAgentMessages.length ? visibleAgentMessages.map((message) => <article className={`project-agent-message ${message.role}`} key={message.id}><strong>{message.role === "user" ? "我" : "项目 Agent"}</strong><MarkdownAnswer text={message.content || (message.status === "streaming" ? "正在分析…" : "")} /></article>) : <div className="knowledge-empty"><strong>从资料到可信项目库</strong><span>可以直接问“哪些地方冲突”“我的职责还缺什么证据”，也可以让 Agent 提议补充事实。它不会未经确认写入。</span></div>}</div>
