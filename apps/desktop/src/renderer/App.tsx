...
function DesktopPreview({ value, onChange }: { value: OverlayPreferences; onChange: (patch: OverlayPreferencesPatch) => void }): JSX.Element {
  ...
  const screenSize = value.layoutPreset === "dual_screen" ? { width: 2560, height: 1440 } : DESIGNER_CANVAS;
...
function OverlayPreferencesPanel({ value, onChange, onReset }: { value: OverlayPreferences; onChange: (patch: OverlayPreferencesPatch) => void; onReset: () => void }): JSX.Element {
  const lastLayout = useRef<OverlayPreferencesPatch | undefined>(undefined);
  const commit = (patch: OverlayPreferencesPatch) => { lastLayout.current = { layoutPreset: value.layoutPreset, questionWindow: value.questionWindow, answerWindow: value.answerWindow, controlBar: value.controlBar, appearance: value.appearance, behavior: value.behavior }; onChange(patch); };
  ...
  return <section className="settings-service-card overlay-preferences-card overlay-designer-card">
    ...
    <div className="overlay-designer-layout"><div className="overlay-designer-controls">
      ...
      <section className="designer-section"><div className="designer-section-heading"><div><span>05</span><div><h3>Appearance</h3><p>背景、文字、边框分离；Text-only 不隐藏文字。</p></div></div></div><div className="designer-appearance-mode-grid">{([ ["glass", "Glass", "70–85% background · blur 12–24"], ["translucent", "Translucent", "25–50% background · blur 4–12"], ["text_only", "Text-only", "0 background · text 100%"], ["custom", "Custom", "Use per-window values"] ] as const).map(([mode, label, description]) => <button key={mode} type="button" className={value.appearance.mode === mode ? "selected" : ""} onClick={() => onChange({ appearance: { ...value.appearance, mode } })}><strong>{label}</strong><small>{description}</small></button>)}</div><div className="designer-fields-grid"><DesignerNumberField label="Blur" value={value.appearance.blur} min={0} max={40} suffix="px" onChange={(next) => onChange({ appearance: { ...value.appearance, blur: next } })} /><label className="designer-select-field"><span>Text Shadow</span><select value={value.appearance.textShadow} onChange={(event) => onChange({ appearance: { ...value.appearance, textShadow: event.target.value as OverlayPreferences["appearance"]["textShadow"] } })}><option value="none">None</option><option value="soft">Soft · 推荐</option><option value="medium">Medium</option></select></label><label className="designer-select-field"><span>Text Outline</span><select value={value.appearance.textOutline} onChange={(event) => onChange({ appearance: { ...value.appearance, textOutline: Number(event.target.value) as 0 | 0.5 | 1 } })}><option value="0">0 px</option><option value="0.5">0.5 px</option><option value="1">1 px</option></select></label></div><div className="designer-toggle-grid"><label className="designer-check-row"><input type="checkbox" checked={value.appearance.border} onChange={(event) => onChange({ appearance: { ...value.appearance, border: event.target.checked } })} /><span><strong>Border</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.appearance.shadow} onChange={(event) => onChange({ appearance: { ...value.appearance, shadow: event.target.checked } })} /><span><strong>Shadow</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.liveApply} onChange={(event) => updateBehavior({ liveApply: event.target.checked })} /><span><strong>Live Apply</strong><small>默认开启；布局操作在释放后持久化</small></span></label></div></section>
