...
import type { LlmModelProfileInput, OverlayPreferencesPatch, ProviderCenterPublicConfig, PublicProviderSettings, TencentValidationState, TencentValidationStatus } from "../main/settings-store";
import { DEFAULT_OVERLAY_PREFERENCES, type OverlayControlBarOrientation, type OverlayControlBarPositionMode, type OverlayFontWeight, type OverlayPreferences } from "../shared/overlay-preferences";
...
import type { CaptureProtectionState, HUDState, OverlayDisplayInfo, OverlayMode } from "../main/overlay-manager";
...
function LegacyOverlayPreferencesPanel({ value, onChange, onReset }: { value: OverlayPreferences; onChange: (patch: Partial<OverlayPreferences>) => void; onReset: () => void }): JSX.Element {
  const toggles: Array<[keyof Pick<OverlayPreferences, "showToolbar" | "showTranscript" | "showAnswer" | "showTimestamps">, string, string]> = [
    ...
    ["showTranscript", "问题导航", "只显示问题组和追问，不显示完整转写"],
    ["showAnswer", "答案阅读器", "保留当前回答和历史答案栈"],
    ["showTimestamps", "转写时间", "在每段对话旁显示时间"]
  ];
  const updateQuestionWindow = (patch: Partial<OverlayPreferences["questionWindow"]>) => onChange({ layoutPreset: "custom", questionWindow: { ...value.questionWindow, ...patch } });
  const updateAnswerWindow = (patch: Partial<OverlayPreferences["answerWindow"]>) => onChange({ layoutPreset: "custom", answerWindow: { ...value.answerWindow, ...patch } });
  const updateBehavior = (patch: Partial<OverlayPreferences["behavior"]>) => onChange({ behavior: { ...value.behavior, ...patch } });
  const updateScreenshot = (patch: Partial<OverlayPreferences["screenshot"]>) => onChange({ screenshot: { ...value.screenshot, ...patch } });
  const fixedRegion = value.screenshot.fixedRegion ?? { x: 0, y: 0, width: 1280, height: 720 };
  const updateFixedRegion = (patch: Partial<typeof fixedRegion>) => updateScreenshot({ fixedRegion: { ...fixedRegion, ...patch } });
  return <section className="settings-service-card overlay-preferences-card">
    <header><div><span className="step-number">04</span><div><h2>面试悬浮窗</h2><p>问题导航和答案阅读器独立调整；修改后立即应用并保留到当前显示器。</p></div></div><button className="outline-pill" onClick={onReset}>恢复默认</button></header>
    <div className="overlay-preferences-layout"><div className="overlay-preferences-controls"><p className="page-note">滚轮只滚动鼠标所在阅读器；离开底部后不会被新回答强行拉回。鼠标中键可分别配置实时面试和笔试截图。</p>
      ...
      <div className="overlay-preset-row"><label className="clean-field"><span>布局预设</span><select value={value.layoutPreset} onChange={(event) => onChange({ layoutPreset: event.target.value as OverlayPreferences["layoutPreset"] })}><option value="compact">紧凑</option><option value="standard">标准</option><option value="wide">宽阅读</option><option value="dual_screen">双屏</option><option value="custom">自定义</option></select></label><small>自定义尺寸后自动切换为自定义。</small></div>
      <div className="overlay-window-settings"><fieldset><legend>问题导航窗口</legend><div className="overlay-window-grid"><label className="clean-field"><span>宽度 <small>{value.questionWindow.width}px</small></span><input type="range" min="310" max="900" value={value.questionWindow.width} onChange={(event) => updateQuestionWindow({ width: Number(event.target.value) })} /></label><label className="clean-field"><span>高度 <small>{value.questionWindow.height}px</small></span><input type="range" min="260" max="900" value={value.questionWindow.height} onChange={(event) => updateQuestionWindow({ height: Number(event.target.value) })} /></label><label className="clean-field"><span>字号 <small>{value.questionWindow.fontSize}px</small></span><input type="range" min="11" max="24" value={value.questionWindow.fontSize} onChange={(event) => updateQuestionWindow({ fontSize: Number(event.target.value) })} /></label></div></fieldset><fieldset><legend>答案阅读器窗口</legend><div className="overlay-window-grid"><label className="clean-field"><span>宽度 <small>{value.answerWindow.width}px</small></span><input type="range" min="340" max="1200" value={value.answerWindow.width} onChange={(event) => updateAnswerWindow({ width: Number(event.target.value) })} /></label><label className="clean-field"><span>高度 <small>{value.answerWindow.height}px</small></span><input type="range" min="260" max="900" value={value.answerWindow.height} onChange={(event) => updateAnswerWindow({ height: Number(event.target.value) })} /></label><label className="clean-field"><span>字号 <small>{value.answerWindow.fontSize}px</small></span><input type="range" min="11" max="24" value={value.answerWindow.fontSize} onChange={(event) => updateAnswerWindow({ fontSize: Number(event.target.value) })} /></label></div></fieldset></div>
      <div className="overlay-function-grid"><label className="overlay-function-toggle"><span><strong>问题跟随最新</strong><small>新问题到达时自动定位到底部</small></span><input type="checkbox" checked={value.behavior.followLatestQuestion} onChange={(event) => updateBehavior({ followLatestQuestion: event.target.checked })} /></label><label className="overlay-function-toggle"><span><strong>答案跟随最新</strong><small>手动滚动后保留位置并显示新内容数</small></span><input type="checkbox" checked={value.behavior.followLatestAnswer} onChange={(event) => updateBehavior({ followLatestAnswer: event.target.checked })} /></label><label className="overlay-function-toggle"><span><strong>锁定窗口位置</strong><small>禁止拖拽，仍可使用滚轮阅读</small></span><input type="checkbox" checked={value.behavior.lockPosition} onChange={(event) => updateBehavior({ lockPosition: event.target.checked })} /></label><label className="overlay-function-toggle"><span><strong>始终置顶</strong><small>同步到原生悬浮窗层级</small></span><input type="checkbox" checked={value.behavior.alwaysOnTop} onChange={(event) => updateBehavior({ alwaysOnTop: event.target.checked })} /></label><label className="overlay-function-toggle"><span><strong>鼠标穿透</strong><small>仅滚动区和控件自动接管鼠标</small></span><input type="checkbox" checked={value.behavior.mousePassthrough} onChange={(event) => updateBehavior({ mousePassthrough: event.target.checked })} /></label></div>
      <div className="overlay-screenshot-settings"><label className="overlay-function-toggle"><span><strong>鼠标中键截图</strong><small>统一支持手动面试和笔试模式</small></span><input type="checkbox" checked={value.screenshot.middleMouseEnabled} onChange={(event) => updateScreenshot({ middleMouseEnabled: event.target.checked })} /></label><label className="overlay-function-toggle"><span><strong>实时面试中启用</strong></span><input type="checkbox" checked={value.screenshot.enabledInManualInterview} onChange={(event) => updateScreenshot({ enabledInManualInterview: event.target.checked })} /></label><label className="overlay-function-toggle"><span><strong>笔试中启用</strong></span><input type="checkbox" checked={value.screenshot.enabledInExamMode} onChange={(event) => updateScreenshot({ enabledInExamMode: event.target.checked })} /></label><label className="clean-field"><span>截图区域</span><select value={value.screenshot.captureMode} onChange={(event) => updateScreenshot({ captureMode: event.target.value as OverlayPreferences["screenshot"]["captureMode"] })}><option value="current_display">当前显示器</option><option value="full_screen">全屏</option><option value="fixed_region">固定区域</option><option value="last_region">上次区域</option><option value="interactive">交互选择（回退全屏）</option></select></label>{value.screenshot.captureMode === "fixed_region" && <div className="overlay-region-grid"><label>左 <input type="number" min="0" value={fixedRegion.x} onChange={(event) => updateFixedRegion({ x: Number(event.target.value) })} /></label><label>上 <input type="number" min="0" value={fixedRegion.y} onChange={(event) => updateFixedRegion({ y: Number(event.target.value) })} /></label><label>宽 <input type="number" min="1" value={fixedRegion.width} onChange={(event) => updateFixedRegion({ width: Number(event.target.value) })} /></label><label>高 <input type="number" min="1" value={fixedRegion.height} onChange={(event) => updateFixedRegion({ height: Number(event.target.value) })} /></label></div>}</div>
    </div><div className="overlay-settings-preview" style={{ background: `color-mix(in srgb, ${value.backgroundColor} ${Math.round(value.backgroundOpacity * 100)}%, transparent)`, color: value.fontColor }}><small>实时预览 · 问题导航</small><strong style={{ fontSize: value.questionWindow.titleFontSize }}>简述一下 TCP 和 UDP 的核心区别。</strong><p style={{ fontSize: value.answerWindow.fontSize, lineHeight: value.answerWindow.lineHeight }}>TCP 可靠、有连接；UDP 更轻量、无连接。选择时主要看可靠性和实时性要求。</p><div className="overlay-preview-divider" /><small>答案阅读器</small><p style={{ fontSize: value.answerWindow.fontSize, lineHeight: value.answerWindow.lineHeight }}>回答会继续追加到答案栈；滚动离开底部时保留阅读位置。</p></div></div>
  </section>;
}

const DESIGNER_CANVAS: DesignerCanvas = { width: 1920, height: 1080 };

function designerLayoutFromPreferences(value: OverlayPreferences, canvas: DesignerCanvas = DESIGNER_CANVAS): DesignerLayout {
  ...
  return {
    question: resolved.questionWindow,
    answer: resolved.answerWindow,
    controlBar: resolved.controlBar
  };
}
...
function PreviewWindow({ panel, rect, canvas, selected, onSelect, onMove, onResize, onCommit, orientation, children }: { panel: DesignerPanel; rect: DesignerRect; canvas: DesignerCanvas; selected: boolean; onSelect: () => void; onMove: (rect: DesignerRect) => void; onResize: (rect: DesignerRect) => void; onCommit: () => void; orientation?: OverlayControlBarOrientation; children: JSX.Element }): JSX.Element {
  ...
  const beginDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const move = (next: PointerEvent) => { const frame = frameRef.current?.parentElement; if (!frame || !startRef.current) return; const dx = (next.clientX - startRef.current.clientX) * canvas.width / Math.max(1, frame.clientWidth); const dy = (next.clientY - startRef.current.clientY) * canvas.height / Math.max(1, frame.clientHeight); onMove(clampDesignerRect({ ...startRef.current.rect, x: startRef.current.rect.x + dx, y: startRef.current.rect.y + dy }, canvas, panel === "question" ? QUESTION_DESIGNER_BOUNDS : panel === "answer" ? ANSWER_DESIGNER_BOUNDS : { minimumWidth: 120, maximumWidth: 2_000, minimumHeight: 36, maximumHeight: 240 })); };
...
function DesktopPreview({ value, onChange, displays = [], activeDisplay }: { value: OverlayPreferences; onChange: (patch: OverlayPreferencesPatch) => void; displays?: OverlayDisplayInfo[]; activeDisplay?: OverlayDisplayInfo }): JSX.Element {
  const [background, setBackground] = useState(value.previewBackground);
  ...
  useEffect(() => { setBackground(value.previewBackground); }, [value.previewBackground]);
  useEffect(() => { if (!draggingRef.current) { const next = designerLayoutFromPreferences(value, screenSize); layoutRef.current = next; setLayout(next); } }, [screenSize.width, screenSize.height, value.layoutPreset, value.questionWindow.x, value.questionWindow.y, value.questionWindow.width, value.questionWindow.height, value.answerWindow.x, value.answerWindow.y, value.answerWindow.width, value.answerWindow.height, value.controlBar.x, value.controlBar.y, value.controlBar.width, value.controlBar.height, value.controlBar.orientation]);
  useEffect(() => {
    if (!activeDisplay) return;
    setSelectedDisplayId(activeDisplay.id);
    setScreenSize({ width: activeDisplay.workArea.width, height: activeDisplay.workArea.height });
  }, [activeDisplay?.id, activeDisplay?.workArea.width, activeDisplay?.workArea.height]);
  const updateLayout = (next: DesignerLayout) => { layoutRef.current = next; setLayout(next); draggingRef.current = true; };
  const commit = () => {
  ...
  const editSelectedGeometry = (patch: Partial<DesignerRect>) => { updateLayout({ ...layoutRef.current, [selected]: clampDesignerRect({ ...layoutRef.current[selected], ...patch }, screenSize, selectedBounds) }); commit(); };
  ...
  const renderCanvas = (large = false): JSX.Element => {
  ...
  return <div className="desktop-preview-shell">
    <div className="desktop-preview-heading"><div><span className="page-kicker">{OVERLAY_LABELS.currentDisplay}</span><strong>桌面效果预览</strong><small>拖动窗口或八向调整点，释放后同步设置；预览不会读取或修改真实面试数据。</small></div><div className="desktop-preview-heading-actions"><select aria-label="选择显示器" value={selectedDisplayId ?? displayOptions[0].id} onChange={(event) => { const next = displayOptions.find((display) => display.id === Number(event.target.value)); if (next) { setSelectedDisplayId(next.id); setScreenSize({ width: next.workArea.width, height: next.workArea.height }); } }}><option value={displayOptions[0].id}>{displays.length ? "选择显示器" : "模拟显示器"}</option>{displayOptions.filter((display) => display.id !== displayOptions[0].id || displays.length).map((display, index) => <option value={display.id} key={display.id}>显示器 {index + 1} · {display.workArea.width}×{display.workArea.height} · {Math.round(display.scaleFactor * 100)}%</option>)}</select><button type="button" className="dark-pill" onClick={() => setLargeEditorOpen(true)}>放大编辑</button></div></div>
    <div className="desktop-preview-background-switcher">{([ ["light_desktop", "浅色桌面"], ["dark_ide", "深色 IDE"], ["web_page", "网页"], ["custom_color", "自定义"] ] as const).map(([key, label]) => <button key={key} type="button" className={background === key ? "selected" : ""} onClick={() => { setBackground(key); onChange({ previewBackground: key }); }}>{label}</button>)}{background === "custom_color" && <input type="color" aria-label="预览自定义背景色" value={value.previewCustomColor} onChange={(event) => onChange({ previewCustomColor: event.target.value })} />}</div>
    <div className="desktop-preview-background-switcher"><span className="preview-tool-label">编辑对象</span>{([ ["controlBar", "控制栏"], ["question", "问题导航"], ["answer", "答案阅读器"] ] as const).map(([panel, label]) => <button key={panel} type="button" className={selected === panel ? "selected" : ""} onClick={() => setSelected(panel)}>{selected === panel ? "● " : "○ "}{label}</button>)}<span className="preview-tool-label">画布缩放</span>{([ [0.5, "50%"], [0.75, "75%"], [1, "100%"], [1.25, "125%"], ["fit", "适应窗口"] ] as const).map(([key, label]) => <button key={String(key)} type="button" className={zoom === key ? "selected" : ""} onClick={() => setZoom(key as typeof zoom)}>{label}</button>)}</div>
    {renderCanvas()}
    <div className="desktop-preview-geometry"><strong>{OVERLAY_LABELS.panel[selected]}尺寸</strong><DesignerNumberField label="X" value={selectedRect.x} min={-2000} max={4000} suffix="px" onChange={(next) => editSelectedGeometry({ x: next })} /><DesignerNumberField label="Y" value={selectedRect.y} min={-2000} max={3000} suffix="px" onChange={(next) => editSelectedGeometry({ y: next })} /><DesignerNumberField label="宽度" value={selectedRect.width} min={selectedBounds.minimumWidth} max={selectedBounds.maximumWidth} suffix="px" onChange={(next) => editSelectedGeometry({ width: next })} /><DesignerNumberField label="高度" value={selectedRect.height} min={selectedBounds.minimumHeight} max={selectedBounds.maximumHeight} suffix="px" onChange={(next) => editSelectedGeometry({ height: next })} /></div>
    <div className="desktop-preview-legend"><span><i className="preview-legend-question" />问题导航</span><span><i className="preview-legend-answer" />答案阅读器</span><span><i className="preview-legend-toolbar" />控制栏</span><small>预览 ↔ 设置双向同步 · 逻辑坐标 {screenSize.width}×{screenSize.height} · 拖动边缘或角点均可调整</small></div>
    {largeEditorOpen && <div className="desktop-preview-modal-backdrop" role="presentation"><section className="desktop-preview-modal" role="dialog" aria-modal="true" aria-labelledby="desktop-preview-modal-title"><header><div><span className="page-kicker">悬浮窗设计器</span><h3 id="desktop-preview-modal-title">放大编辑</h3></div><button type="button" className="outline-pill" onClick={() => setLargeEditorOpen(false)}>关闭</button></header>{renderCanvas(true)}</section></div>}
  </div>;
}

function DesignerNumberField({ label, value, min, max, step = 1, suffix = "", onChange }: { label: string; value: number; min: number; max: number; step?: number; suffix?: string; onChange: (value: number) => void }): JSX.Element {
  return <label className="designer-number-field"><span>{label}</span><div><input type="number" value={value} min={min} max={max} step={step} onChange={(event) => onChange(Number(event.target.value))} /><small>{suffix}</small></div></label>;
}

function DesignerOpacityField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }): JSX.Element {
  const percent = Math.round(value * 100);
  return <label className="designer-opacity-field"><span>{label}<strong>{percent}%</strong></span><div><input type="range" min="0" max="100" value={percent} onChange={(event) => onChange(Number(event.target.value) / 100)} /><input type="number" min="0" max="100" value={percent} onChange={(event) => onChange(Number(event.target.value) / 100)} /><small>%</small></div></label>;
}

function DesignerWindowSection({ kind, title, value, onChange, onReset }: { kind: "question" | "answer"; title: string; value: OverlayPreferences["questionWindow"] | OverlayPreferences["answerWindow"]; onChange: (patch: Partial<OverlayPreferences["questionWindow"]>) => void; onReset: () => void }): JSX.Element {
  const isQuestion = kind === "question";
  const minWidth = isQuestion ? 220 : 300;
  const maxWidth = isQuestion ? 1_000 : 1_600;
  const minHeight = isQuestion ? 120 : 150;
  return <div className="designer-section-card">
    <div className="designer-section-card-heading"><div><strong>{title}</strong><small>{isQuestion ? OVERLAY_LABELS.questionNavigator : OVERLAY_LABELS.answerReader} · 独立配置</small></div><button type="button" className="text-button" onClick={onReset}>重置本区</button></div>
    <div className="designer-fields-grid designer-geometry-fields"><DesignerNumberField label="X" value={value.x ?? 0} min={-2000} max={4000} suffix="px" onChange={(next) => onChange({ x: next })} /><DesignerNumberField label="Y" value={value.y ?? 0} min={-2000} max={3000} suffix="px" onChange={(next) => onChange({ y: next })} /><DesignerNumberField label="宽度" value={value.width} min={minWidth} max={maxWidth} suffix="px" onChange={(next) => onChange({ width: next })} /><DesignerNumberField label="高度" value={value.height} min={minHeight} max={1_200} suffix="px" onChange={(next) => onChange({ height: next })} /></div>
    <div className="designer-fields-grid"><DesignerNumberField label="字体大小" value={value.fontSize} min={10} max={isQuestion ? 32 : 40} suffix="px" onChange={(next) => onChange({ fontSize: next })} /><label className="designer-select-field"><span>字体粗细</span><select value={value.fontWeight} onChange={(event) => onChange({ fontWeight: Number(event.target.value) as OverlayFontWeight })}><option value="400">400 · 常规</option><option value="500">500 · 中等</option><option value="600">600 · 半粗</option></select></label><DesignerNumberField label="行高" value={value.lineHeight} min={1.1} max={2.2} step={0.05} onChange={(next) => onChange({ lineHeight: next })} /><DesignerNumberField label={isQuestion ? "项目间距" : "段落间距"} value={isQuestion ? value.itemGap : value.paragraphGap} min={isQuestion ? 2 : 0} max={isQuestion ? 24 : 32} suffix="px" onChange={(next) => onChange(isQuestion ? { itemGap: next } : { paragraphGap: next })} /></div>
    <div className="designer-fields-grid designer-appearance-fields"><DesignerOpacityField label="背景透明度" value={value.backgroundOpacity} onChange={(next) => onChange({ backgroundOpacity: next })} /><DesignerOpacityField label="文字透明度" value={value.textOpacity} onChange={(next) => onChange({ textOpacity: next })} /><DesignerOpacityField label="边框透明度" value={value.borderOpacity} onChange={(next) => onChange({ borderOpacity: next })} /><label className="designer-color-field"><span>背景 / 文字颜色</span><div><input type="color" value={value.backgroundColor} onChange={(event) => onChange({ backgroundColor: event.target.value })} /><input type="color" value={value.textColor} onChange={(event) => onChange({ textColor: event.target.value })} /></div></label></div>
  </div>;
}

function OverlayPreferencesPanel({ value: persistedValue, onChange, onReset }: { value: OverlayPreferences; onChange: (patch: OverlayPreferencesPatch) => void; onReset: () => void }): JSX.Element {
  ...
  useEffect(() => {
    if (!draftDirty || persistedValue.behavior.liveApply) {
      setDraft(persistedValue);
      setDraftDirty(false);
    }
  }, [draftDirty, persistedValue]);
  const value = persistedValue.behavior.liveApply ? persistedValue : draft;
  const mergePatch = (base: OverlayPreferences, patch: OverlayPreferencesPatch): OverlayPreferences => {
    const { questionWindow, answerWindow, controlBar, behavior, appearance, screenshot, ...topLevel } = patch;
    return {
      ...base,
      ...topLevel,
      ...(questionWindow ? { questionWindow: { ...base.questionWindow, ...questionWindow } } : {}),
      ...(answerWindow ? { answerWindow: { ...base.answerWindow, ...answerWindow } } : {}),
      ...(controlBar ? { controlBar: { ...base.controlBar, ...controlBar } } : {}),
      ...(behavior ? { behavior: { ...base.behavior, ...behavior } } : {}),
      ...(appearance ? { appearance: { ...base.appearance, ...appearance } } : {}),
      ...(screenshot ? { screenshot: { ...base.screenshot, ...screenshot } } : {})
    };
  };
  const applyEditorPatch = (patch: OverlayPreferencesPatch): void => {
    const next = mergePatch(value, patch);
    const liveApplyChanged = patch.behavior?.liveApply !== undefined && patch.behavior.liveApply !== persistedValue.behavior.liveApply;
    const shouldPersist = persistedValue.behavior.liveApply || liveApplyChanged || next.behavior.liveApply;
    setDraft(next);
    setDraftDirty(!shouldPersist);
    if (shouldPersist) onChange(persistedValue.behavior.liveApply || liveApplyChanged ? patch : next);
  };
  const lastLayout = useRef<OverlayPreferencesPatch | undefined>(undefined);
  const commit = (patch: OverlayPreferencesPatch) => { lastLayout.current = { layoutPreset: value.layoutPreset, questionWindow: value.questionWindow, answerWindow: value.answerWindow, controlBar: value.controlBar, appearance: value.appearance, behavior: value.behavior }; applyEditorPatch(patch); };
  const updateQuestion = (patch: Partial<OverlayPreferences["questionWindow"]>) => commit({ layoutPreset: "custom", questionWindow: patch });
  const updateAnswer = (patch: Partial<OverlayPreferences["answerWindow"]>) => commit({ layoutPreset: "custom", answerWindow: patch });
  const updateControl = (patch: Partial<OverlayPreferences["controlBar"]>) => commit({ layoutPreset: "custom", controlBar: patch });
  const updateBehavior = (patch: Partial<OverlayPreferences["behavior"]>) => commit({ behavior: patch });
  const resetQuestion = () => commit({ questionWindow: DEFAULT_OVERLAY_PREFERENCES.questionWindow });
  const resetAnswer = () => commit({ answerWindow: DEFAULT_OVERLAY_PREFERENCES.answerWindow });
  const resetLayout = () => { void window.interviewCopilot.overlay.resetLayout(); applyPreset("standard"); };
  const undoLayout = () => { if (lastLayout.current) applyEditorPatch(lastLayout.current); };
  const fixedRegion = value.screenshot.fixedRegion ?? { x: 0, y: 0, width: 1280, height: 720 };
  const updateScreenshot = (patch: Partial<OverlayPreferences["screenshot"]>) => commit({ screenshot: patch });
  const applyDraft = () => { onChange(draft); setDraftDirty(false); };
  const cancelDraft = () => { setDraft(persistedValue); setDraftDirty(false); };
  const activeDisplay = displays.find((display) => display.id === value.questionWindow.displayId) ?? displays[0];
  const presetDisplay = activeDisplay ?? { id: undefined, workArea: DESIGNER_CANVAS, bounds: { ...DESIGNER_CANVAS }, scaleFactor: 1 };
  const applyPreset = (preset: OverlayPreferences["layoutPreset"]) => {
    const resolved = applyLayoutPreset(preset, presetDisplay, {
      questionWindow: { x: value.questionWindow.x ?? 120, y: value.questionWindow.y ?? 180, width: value.questionWindow.width, height: value.questionWindow.height },
      answerWindow: { x: value.answerWindow.x ?? 570, y: value.answerWindow.y ?? 180, width: value.answerWindow.width, height: value.answerWindow.height },
      controlBar: { x: value.controlBar.x ?? 620, y: value.controlBar.y ?? 24, width: value.controlBar.width, height: value.controlBar.height },
      controlBarOrientation: value.controlBar.orientation,
    ...
    commit({ layoutPreset: preset, questionWindow: { ...resolved.questionWindow, ...(resolved.displayId !== undefined ? { displayId: resolved.displayId } : {}) }, answerWindow: { ...resolved.answerWindow, ...(resolved.displayId !== undefined ? { displayId: resolved.displayId } : {}) }, controlBar: { ...resolved.controlBar, positionMode: resolved.controlBarPositionMode, orientation: resolved.controlBarOrientation, ...(resolved.displayId !== undefined ? { displayId: resolved.displayId } : {}) }, ...(preset === "transparent" ? { appearance: { ...value.appearance, mode: "text_only" } } : {}) });
  };
  return <section className="settings-service-card overlay-preferences-card overlay-designer-card">
    <header className="overlay-designer-header"><div><span className="step-number">04</span><div><span className="page-kicker">悬浮窗设计器</span><h2>悬浮窗设计器</h2><p>右侧预览模拟真实桌面效果；拖动和调整大小会在释放后同步到真实悬浮窗。</p></div></div><div className="overlay-designer-header-actions"><button className="outline-pill" onClick={onReset}>恢复全部默认</button><button className="outline-pill" onClick={undoLayout} disabled={!lastLayout.current}>撤销布局</button>{!value.behavior.liveApply && <><button className="outline-pill" onClick={cancelDraft} disabled={!draftDirty}>取消修改</button><button className="dark-pill" onClick={applyDraft} disabled={!draftDirty}>应用到真实窗口</button></>}</div></header>
    <div className="overlay-designer-layout"><div className="overlay-designer-controls">
      <section className="designer-section"><div className="designer-section-heading"><div><span>01</span><div><h3>{OVERLAY_LABELS.layout}</h3><p>布局、窗口坐标、{OVERLAY_LABELS.controlBar}和吸附。</p></div></div><button className="text-button" onClick={resetLayout}>重置布局</button></div><label className="designer-select-field designer-preset-field"><span>布局预设</span><select value={value.layoutPreset} onChange={(event) => applyPreset(event.target.value as OverlayPreferences["layoutPreset"])}><option value="compact">{OVERLAY_LABELS.preset.compact}</option><option value="standard">{OVERLAY_LABELS.preset.standard}</option><option value="wide">{OVERLAY_LABELS.preset.wide}</option><option value="dual_screen">{OVERLAY_LABELS.preset.dual_screen}</option><option value="transparent">{OVERLAY_LABELS.preset.transparent}</option><option value="custom">{OVERLAY_LABELS.preset.custom}</option></select></label><DesignerWindowSection kind="question" title="问题导航窗口" value={value.questionWindow} onChange={updateQuestion} onReset={resetQuestion} /><DesignerWindowSection kind="answer" title="答案阅读器窗口" value={value.answerWindow} onChange={updateAnswer} onReset={resetAnswer} /><div className="designer-section-card"><div className="designer-section-card-heading"><div><strong>{OVERLAY_LABELS.controlBar}</strong><small>顶部 / 侧边控制栏自由定位</small></div><button type="button" className="text-button" onClick={() => updateControl({ positionMode: "top_center", orientation: "horizontal" })}>重置</button></div><div className="designer-fields-grid designer-geometry-fields"><DesignerNumberField label="X" value={value.controlBar.x ?? 620} min={-2000} max={4000} suffix="px" onChange={(next) => updateControl({ x: next, positionMode: "custom" })} /><DesignerNumberField label="Y" value={value.controlBar.y ?? 24} min={-2000} max={3000} suffix="px" onChange={(next) => updateControl({ y: next, positionMode: "custom" })} /><DesignerNumberField label="宽度" value={value.controlBar.width} min={120} max={2_000} suffix="px" onChange={(next) => updateControl({ width: next })} /><DesignerNumberField label="高度" value={value.controlBar.height} min={36} max={240} suffix="px" onChange={(next) => updateControl({ height: next })} /></div><div className="designer-fields-grid"><label className="designer-select-field"><span>位置</span><select value={value.controlBar.positionMode} onChange={(event) => updateControl({ positionMode: event.target.value as OverlayControlBarPositionMode })}><option value="top_left">左上</option><option value="top_center">顶部居中</option><option value="top_right">右上</option><option value="bottom_left">左下</option><option value="bottom_center">底部居中</option><option value="bottom_right">右下</option><option value="custom">自定义</option></select></label><label className="designer-select-field"><span>方向</span><select value={value.controlBar.orientation} onChange={(event) => updateControl({ orientation: event.target.value as OverlayControlBarOrientation })}><option value="horizontal">横向</option><option value="vertical">纵向</option></select></label></div><div className="designer-fields-grid"><DesignerNumberField label="字号" value={value.controlBar.fontSize} min={9} max={24} suffix="px" onChange={(next) => updateControl({ fontSize: next })} /><label className="designer-select-field"><span>字重</span><select value={value.controlBar.fontWeight} onChange={(event) => updateControl({ fontWeight: Number(event.target.value) as OverlayFontWeight })}><option value="400">400 · 常规</option><option value="500">500 · 中等</option><option value="600">600 · 半粗</option></select></label><DesignerOpacityField label="背景透明度" value={value.controlBar.backgroundOpacity} onChange={(next) => updateControl({ backgroundOpacity: next })} /><DesignerOpacityField label="文字透明度" value={value.controlBar.textOpacity} onChange={(next) => updateControl({ textOpacity: next })} /><DesignerOpacityField label="边框透明度" value={value.controlBar.borderOpacity} onChange={(next) => updateControl({ borderOpacity: next })} /></div><div className="designer-fields-grid"><label className="designer-color-field"><span>背景颜色</span><input type="color" value={value.controlBar.backgroundColor} onChange={(event) => updateControl({ backgroundColor: event.target.value })} /></label><label className="designer-color-field"><span>文字颜色</span><input type="color" value={value.controlBar.textColor} onChange={(event) => updateControl({ textColor: event.target.value })} /></label></div></div><label className="designer-check-row"><input type="checkbox" checked={value.behavior.snapEnabled} onChange={(event) => updateBehavior({ snapEnabled: event.target.checked })} /><span><strong>启用位置吸附</strong><small>屏幕边缘、窗口边缘和控制栏；按住 Alt 临时关闭，阈值 {value.behavior.snapThreshold}px。</small></span></label><DesignerNumberField label="吸附阈值" value={value.behavior.snapThreshold} min={8} max={16} suffix="px" onChange={(next) => updateBehavior({ snapThreshold: next })} /><button className="dark-pill designer-edit-real-button" onClick={() => void window.interviewCopilot.overlay.enterLayoutEditMode()}>编辑真实窗口</button></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>02</span><div><h3>{OVERLAY_LABELS.questionNavigator}</h3><p>只显示问题组和追问，不重新判断问题。</p></div></div><button className="text-button" onClick={resetQuestion}>重置本区</button></div><label className="designer-check-row"><input type="checkbox" checked={value.showTranscript} onChange={(event) => commit({ showTranscript: event.target.checked })} /><span><strong>启用问题导航</strong><small>保留问题组的只读视图</small></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.showQuestionStatus} onChange={(event) => updateBehavior({ showQuestionStatus: event.target.checked })} /><span><strong>显示状态</strong></span></label></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>03</span><div><h3>{OVERLAY_LABELS.answerReader}</h3><p>答案栈只改变显示方式，不重新生成或清空回答。</p></div></div><button className="text-button" onClick={resetAnswer}>重置本区</button></div><label className="designer-check-row"><input type="checkbox" checked={value.showAnswer} onChange={(event) => commit({ showAnswer: event.target.checked })} /><span><strong>启用答案阅读器</strong><small>保留当前回答与历史答案</small></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.showAnswerStatus} onChange={(event) => updateBehavior({ showAnswerStatus: event.target.checked })} /><span><strong>显示状态</strong></span></label></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>04</span><div><h3>{OVERLAY_LABELS.interaction}</h3><p>点击、穿透、滚轮和临时恢复交互。</p></div></div></div><div className="designer-fields-grid"><label className="designer-select-field"><span>{OVERLAY_LABELS.interactionMode}</span><select value={value.behavior.interactionMode} onChange={(event) => updateBehavior({ interactionMode: event.target.value as OverlayPreferences["behavior"]["interactionMode"] })}><option value="interactive">{OVERLAY_LABELS.interactive}</option><option value="click_through">{OVERLAY_LABELS.clickThrough}</option><option value="full_passthrough">{OVERLAY_LABELS.fullPassthrough}</option></select></label><label className="designer-select-field"><span>{OVERLAY_LABELS.wheelRouting}</span><select value={value.behavior.wheelRouting} onChange={(event) => updateBehavior({ wheelRouting: event.target.value as OverlayPreferences["behavior"]["wheelRouting"] })}><option value="overlay_under_cursor">{OVERLAY_LABELS.overlayUnderCursor}</option><option value="underlying_app">{OVERLAY_LABELS.underlyingApp}</option><option value="dual">{OVERLAY_LABELS.dual}</option></select></label><label className="designer-select-field"><span>{OVERLAY_LABELS.temporaryModifier}</span><select value={value.behavior.temporaryInteractionModifier} onChange={(event) => updateBehavior({ temporaryInteractionModifier: event.target.value as OverlayPreferences["behavior"]["temporaryInteractionModifier"] })}><option value="ctrl">Ctrl</option><option value="alt">Alt</option><option value="shift">Shift</option><option value="ctrl_shift">Ctrl + Shift</option></select></label></div><div className="designer-toggle-grid"><label className="designer-check-row"><input type="checkbox" checked={value.behavior.alwaysOnTop} onChange={(event) => updateBehavior({ alwaysOnTop: event.target.checked })} /><span><strong>{OVERLAY_LABELS.alwaysOnTop}</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.lockLayout} onChange={(event) => updateBehavior({ lockLayout: event.target.checked })} /><span><strong>{OVERLAY_LABELS.lockLayout}</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.followLatestQuestion} onChange={(event) => updateBehavior({ followLatestQuestion: event.target.checked })} /><span><strong>{OVERLAY_LABELS.followLatestQuestion}</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.followLatestAnswer} onChange={(event) => updateBehavior({ followLatestAnswer: event.target.checked })} /><span><strong>{OVERLAY_LABELS.followLatestAnswer}</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.rememberPosition} onChange={(event) => updateBehavior({ rememberPosition: event.target.checked })} /><span><strong>{OVERLAY_LABELS.rememberPosition}</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.rememberSize} onChange={(event) => updateBehavior({ rememberSize: event.target.checked })} /><span><strong>{OVERLAY_LABELS.rememberSize}</strong></span></label></div><details className="designer-advanced-details"><summary>截图与中键</summary><div className="designer-toggle-grid"><label className="designer-check-row"><input type="checkbox" checked={value.screenshot.middleMouseEnabled} onChange={(event) => updateScreenshot({ middleMouseEnabled: event.target.checked })} /><span><strong>鼠标中键截图</strong><small>手动面试和笔试均保持独立 Hook</small></span></label><label className="designer-check-row"><input type="checkbox" checked={value.screenshot.enabledInManualInterview} onChange={(event) => updateScreenshot({ enabledInManualInterview: event.target.checked })} /><span><strong>实时面试启用</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.screenshot.enabledInExamMode} onChange={(event) => updateScreenshot({ enabledInExamMode: event.target.checked })} /><span><strong>笔试启用</strong></span></label></div><label className="designer-select-field"><span>截图区域</span><select value={value.screenshot.captureMode} onChange={(event) => updateScreenshot({ captureMode: event.target.value as OverlayPreferences["screenshot"]["captureMode"] })}><option value="current_display">当前显示器</option><option value="full_screen">全屏</option><option value="fixed_region">固定区域</option><option value="last_region">上次区域</option><option value="interactive">交互选择</option></select></label>{value.screenshot.captureMode === "fixed_region" && <div className="designer-fields-grid designer-region-fields"><DesignerNumberField label="左" value={fixedRegion.x} min={0} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, x: next } })} /><DesignerNumberField label="上" value={fixedRegion.y} min={0} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, y: next } })} /><DesignerNumberField label="宽" value={fixedRegion.width} min={1} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, width: next } })} /><DesignerNumberField label="高" value={fixedRegion.height} min={1} max={20_000} suffix="px" onChange={(next) => updateScreenshot({ fixedRegion: { ...fixedRegion, height: next } })} /></div>}</details></section>
      <section className="designer-section"><div className="designer-section-heading"><div><span>05</span><div><h3>{OVERLAY_LABELS.appearance}</h3><p>背景、文字、边框分离；纯文字模式不会隐藏文字。</p></div></div></div><div className="designer-appearance-mode-grid">{([["glass", OVERLAY_LABELS.glass, "背景 70–85% · 模糊 12–24"], ["translucent", OVERLAY_LABELS.translucent, "背景 25–50% · 模糊 4–12"], ["text_only", OVERLAY_LABELS.textOnly, "背景 0 · 文字 100%"], ["custom", OVERLAY_LABELS.custom, "使用每个窗口的独立值"]] as const).map(([mode, label, description]) => <button key={mode} type="button" className={value.appearance.mode === mode ? "selected" : ""} onClick={() => commit({ appearance: { ...value.appearance, mode } })}><strong>{label}</strong><small>{description}</small></button>)}</div><div className="designer-fields-grid"><DesignerNumberField label={OVERLAY_LABELS.blur} value={value.appearance.blur} min={0} max={40} suffix="px" onChange={(next) => commit({ appearance: { ...value.appearance, blur: next } })} /><DesignerNumberField label={OVERLAY_LABELS.radius} value={value.appearance.radius} min={0} max={32} suffix="px" onChange={(next) => commit({ appearance: { ...value.appearance, radius: next } })} /><label className="designer-select-field"><span>{OVERLAY_LABELS.textShadow}</span><select value={value.appearance.textShadow} onChange={(event) => commit({ appearance: { ...value.appearance, textShadow: event.target.value as OverlayPreferences["appearance"]["textShadow"] } })}><option value="none">无</option><option value="soft">柔和 · 推荐</option><option value="medium">中等</option></select></label><label className="designer-select-field"><span>{OVERLAY_LABELS.textOutline}</span><select value={value.appearance.textOutline} onChange={(event) => commit({ appearance: { ...value.appearance, textOutline: Number(event.target.value) as 0 | 0.5 | 1 } })}><option value="0">0 px</option><option value="0.5">0.5 px</option><option value="1">1 px</option></select></label></div><div className="designer-toggle-grid"><label className="designer-check-row"><input type="checkbox" checked={value.appearance.border} onChange={(event) => commit({ appearance: { ...value.appearance, border: event.target.checked } })} /><span><strong>{OVERLAY_LABELS.border}</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.appearance.shadow} onChange={(event) => commit({ appearance: { ...value.appearance, shadow: event.target.checked } })} /><span><strong>{OVERLAY_LABELS.shadow}</strong></span></label><label className="designer-check-row"><input type="checkbox" checked={value.behavior.liveApply} onChange={(event) => updateBehavior({ liveApply: event.target.checked })} /><span><strong>{OVERLAY_LABELS.liveApply}</strong><small>关闭后预览继续实时更新，点击“应用到真实窗口”才持久化</small></span></label></div></section>
    </div><aside className="overlay-settings-preview-sticky"><DesktopPreview value={value} onChange={commit} displays={displays} activeDisplay={activeDisplay} /></aside></div>
  </section>;
}
...
export function App(): JSX.Element {
  ...
  const pageTitle = page === "home" ? "工作台" : page === "interview" ? "实时面试" : page === "preparation" ? "面试准备" : page === "profiles" ? "档案 / 简历" : page === "project-library" ? "项目详情" : page === "knowledge" ? "资料库" : page === "personal-memory" ? "项目知识审核" : page === "question-bank" ? "通用题库" : page === "job-targets" ? "岗位要求" : page === "history" ? "面试历史" : "设置";
