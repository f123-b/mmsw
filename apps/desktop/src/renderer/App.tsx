...
import { QUESTION_BANK_TYPE_LABELS, QUESTION_BANK_TYPES, formatProjectFactValue, isFactEligible, isFactReviewRequired, isFactUserActionRequired, listConflictGroups, normalizeProjectOwnershipMode, validateLlmModelConfiguration, type ProjectConflictGroup } from "@interview-copilot/shared";
...
interface ProjectLibraryManagerProps {
  profileId: string;
  ...
  rebuilding: boolean;
  onUploadProject: (file: File, projectId?: string, sourceRole?: string) => Promise<void>;
  onRebuild: (projectId?: string) => void;
  onVerifyFact: (factId: string, verified: boolean) => Promise<void>;
  onOpenSources: () => void;
  onCreateProject?: (input: { name: string; ownershipMode: "personal" | "team" | "partial" | "reference"; ownershipNote?: string }) => Promise<void>;
  onUpdateProject: (projectId: string, input: { ownershipMode?: "personal" | "team" | "partial" | "reference"; ownershipNote?: string }) => Promise<void>;
...
function ProjectConflictGroupCard(props: { group: ProjectConflictGroup; sourceTitle: (id: string) => string; onResolve: (groupId: string, factId: string, keepBoth?: boolean, contexts?: Record<string, string>) => Promise<void>; onEvidence: (fact: ProjectFact) => void }): JSX.Element {
  const keepBoth = (): void => {
    const contexts: Record<string, string> = {};
    for (const [index, fact] of props.group.facts.entries()) {
      const answer = window.prompt(`请说明“${fact.content}”的版本关系（例如：early-version / later-version）`, index === 0 ? "early-version" : "later-version");
      if (!answer?.trim()) return;
      contexts[fact.id] = answer.trim();
    }
    void props.onResolve(props.group.id, props.group.facts[0]?.id ?? "", true, contexts);
  };
  return <article className="fact-review-card conflict-group-card"><header><div><span className="fact-type-label">冲突组 · {props.group.canonicalKey}</span><h3>{props.group.label}</h3></div><strong>待处理</strong></header><div className="conflict-candidates">{props.group.facts.map((fact) => <div className="conflict-candidate" key={fact.id}><strong>○ {fact.content}</strong><span>来源：{fact.sourceIds.map(props.sourceTitle).join("、") || "未关联"}</span>{fact.evidence?.[0] && <button className="evidence-link" onClick={() => props.onEvidence(fact)}>证据：“{fact.evidence[0].quote.slice(0, 100)}”</button>}<button className="outline-pill" onClick={() => void props.onResolve(props.group.id, fact.id, false)}>采用此版本</button></div>)}</div><footer><button className="dark-pill" onClick={keepBoth}>两个都正确（填写版本关系）</button><button className="outline-pill" onClick={() => props.onEvidence(props.group.facts[0] as ProjectFact)}>暂不处理</button></footer></article>;
}
...
function ProjectLibraryManager(props: ProjectLibraryManagerProps): JSX.Element {
  ...
  const [tab, setTab] = useState<"overview" | "facts" | "problems" | "sources" | "questions">("overview");
  ...
  const [factFilter, setFactFilter] = useState<"action" | "review" | "conflict" | "responsibility" | "all" | "stale">("action");
  const [responsibilityInput, setResponsibilityInput] = useState("");
  const [createProjectOpen, setCreateProjectOpen] = useState(false);
  const [newProjectName, setNewProjectName] = useState("新项目");
  const [newOwnershipMode, setNewOwnershipMode] = useState<"personal" | "team" | "partial" | "reference">("personal");
  const [newOwnershipNote, setNewOwnershipNote] = useState("");
  const [detailStats, setDetailStats] = useState<ProjectMemoryStats>();
  const selectedProject = props.memory.projects.find((project) => project.id === selectedProjectId) ?? props.memory.projects[0];
  const ownershipMode = normalizeProjectOwnershipMode(selectedProject?.ownershipMode);
  useEffect(() => {
    ...
    void window.interviewCopilot.projectMemory.stats(props.profileId, selectedProject.id).then(setDetailStats).catch(() => setDetailStats(undefined));
  ...
  const conflictGroups = listConflictGroups(allProjectFacts, { projectId: selectedProject?.id });
  const isConflictFact = (fact: ProjectFact): boolean => fact.status === "conflicting" || fact.conflictStatus === "conflicting";
  const projectFacts = factFilter === "stale" ? staleProjectFacts : allProjectFacts.filter((fact) => !isConflictFact(fact) && (factFilter === "action" ? isFactUserActionRequired(fact, ownershipMode) : factFilter === "review" ? isFactReviewRequired(fact) : factFilter === "conflict" ? false : factFilter === "responsibility" ? fact.type === "responsibility" : true));
  const parameterFacts = allProjectFacts.filter((fact) => fact.type === "parameter" && isFactEligible(fact));
  const parameterHasConflict = (fact: ProjectFact): boolean => conflictGroups.some((group) => group.facts.some((candidate) => candidate.id === fact.id));
  const decisionFacts = allProjectFacts.filter((fact) => (fact.type === "technical_decision" || fact.type === "decision") && isFactEligible(fact));
  ...
  const responsibilityHeading = ownershipMode === "personal" ? "项目边界说明（可选）" : ownershipMode === "reference" ? "参考范围" : "我的职责范围";
  const responsibilityEmpty = ownershipMode === "personal" ? "可选：补充项目边界" : ownershipMode === "reference" ? "参考项目不生成个人职责" : "待补充本人负责范围";
  ...
  return <section className="simple-page project-library-manager">
    {createProjectOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="modal-card"><h2>新建项目</h2><p className="page-note">先定义项目归属，后续回答会按这个边界控制第一人称。</p><label className="clean-field"><span>项目名称</span><input value={newProjectName} onChange={(event) => setNewProjectName(event.target.value)} autoFocus /></label><label className="clean-field"><span>项目归属</span><select value={newOwnershipMode} onChange={(event) => setNewOwnershipMode(event.target.value as "personal" | "team" | "partial" | "reference")}><option value="personal">个人项目</option><option value="team">团队项目</option><option value="partial">我只负责部分</option><option value="reference">参考项目</option></select></label>{newOwnershipMode !== "personal" && <label className="clean-field"><span>边界说明</span><textarea rows={2} value={newOwnershipNote} onChange={(event) => setNewOwnershipNote(event.target.value)} placeholder="哪些部分由你负责，哪些是团队或参考内容" /></label>}<div className="detail-actions"><button className="dark-pill" disabled={!newProjectName.trim()} onClick={async () => { await props.onCreateProject?.({ name: newProjectName.trim(), ownershipMode: newOwnershipMode, ...(newOwnershipNote.trim() ? { ownershipNote: newOwnershipNote.trim() } : {}) }); setCreateProjectOpen(false); setNewProjectName("新项目"); setNewOwnershipMode("personal"); setNewOwnershipNote(""); }}>创建</button><button className="outline-pill" onClick={() => setCreateProjectOpen(false)}>取消</button></div></div></div>}
    {selectedProject && <section className="project-v4-overview" aria-label="项目技术记忆 V4"><header><div><span className="page-kicker">PROJECT TECHNICAL MEMORY V4</span><h2>技术熟悉度与项目边界</h2><p className="page-note">参数、决策和问题链都来自已有 Project Facts，并保留证据与冲突状态。</p></div><label className="clean-field"><span>项目归属</span><select value={ownershipMode} onChange={(event) => void props.onUpdateProject(selectedProject.id, { ownershipMode: event.target.value as "personal" | "team" | "partial" | "reference" })}><option value="personal">个人项目</option><option value="team">团队项目</option><option value="partial">部分负责</option><option value="reference">参考项目</option></select></label></header><div className="project-memory-metrics"><div><span>项目熟悉度</span><strong>{score("projectFamiliarityScore")}</strong></div><div><span>技术覆盖</span><strong>{score("technicalCoverageScore")}</strong></div><div><span>参数覆盖</span><strong>{score("parameterCoverageScore")}</strong></div><div><span>决策覆盖</span><strong>{score("decisionCoverageScore")}</strong></div><div><span>问题覆盖</span><strong>{score("problemCoverageScore")}</strong></div><div><span>面试准备度</span><strong>{score("interviewReadinessScore")}</strong></div></div>{(ownershipMode === "team" || ownershipMode === "partial") && <label className="clean-field project-scope-note"><span>{ownershipMode === "partial" ? "我的负责边界" : "我的范围"}</span><textarea rows={2} defaultValue={selectedProject.ownershipNote ?? ""} placeholder="说明哪些模块由你负责、哪些由团队负责" onBlur={(event) => void props.onUpdateProject(selectedProject.id, { ownershipNote: event.target.value })} /></label>}{ownershipMode === "reference" && <p className="page-note">参考项目只用于解释技术概念，不会生成第一人称项目经历。</p>}<div className="project-v4-columns"><article><h3>关键参数</h3>{parameterFacts.slice(0, 8).map((fact) => <p key={fact.id}><strong>{fact.title}</strong> {formatProjectFactValue(fact.value) || fact.content} {parameterHasConflict(fact) && <span className="fact-conflict-marker">⚠ 冲突</span>} {fact.evidence?.[0] && <button className="evidence-link" onClick={() => setSelectedEvidence(fact)}>查看证据</button>}</p>)}{parameterFacts.length === 0 && <small>暂无已确认参数</small>}</article><article><h3>技术决策</h3>{decisionFacts.slice(0, 5).map((fact) => <p key={fact.id}><strong>{fact.title}</strong> {fact.content}</p>)}{decisionFacts.length === 0 && <small>暂无已确认决策</small>}</article><article><h3>问题链</h3>{projectProblems.slice(0, 4).map((problem) => <p key={problem.id}><strong>{problem.problem}</strong><br />原因：{problem.cause}<br />解决：{problem.solution}{problem.result && <><br />结果：{problem.result}</>}</p>)}{projectProblems.length === 0 && <small>暂无问题链</small>}</article></div></section>}
    {selectedProject && ownershipMode !== "reference" && <div className="inline-responsibility-form"><label htmlFor="project-responsibility">{ownershipMode === "personal" ? "补充项目边界说明（可选）" : "补充我的负责范围"}</label><textarea id="project-responsibility" rows={2} value={responsibilityInput} onChange={(event) => setResponsibilityInput(event.target.value)} placeholder={ownershipMode === "personal" ? "可选：补充项目边界、关键模块或实现细节" : "补充你真实负责的模块、实现细节和边界"} /><button className="outline-pill" disabled={!responsibilityInput.trim()} onClick={async () => { await props.onAddResponsibility(selectedProject.id, responsibilityInput.trim()); setResponsibilityInput(""); }}>{ownershipMode === "personal" ? "保存边界" : "保存范围"}</button></div>}
    <div className="page-heading"><div><span className="page-kicker">PROJECT LIBRARY</span><h1>项目库</h1><p className="page-note">围绕项目背景、职责、证据和面试准备管理个人工程经验。每份资料必须绑定到已有项目，避免误建项目。</p></div><div className="detail-actions"><button className="outline-pill" onClick={() => setCreateProjectOpen(true)}>新建项目</button><button className="dark-pill" disabled={!selectedProject} onClick={() => setSourceDialog(true)}>添加项目资料</button><button className="outline-pill" disabled={props.rebuilding || !selectedProject} onClick={() => selectedProject && props.onRebuild(selectedProject.id)}>{props.rebuilding ? "分析中…" : "重新分析"}</button></div></div>
    <div className="project-library-summary"><span>项目 <strong>{props.stats.projects}</strong></span><span>可信事实 <strong>{props.stats.eligibleFacts}</strong></span><span>待处理事项 <strong>{props.stats.userActions}</strong></span><span>冲突组 <strong>{props.stats.conflictGroups}</strong></span><span>项目题 <strong>{props.stats.interviewQuestions}</strong></span>{props.stats.staleFacts > 0 && <span>已失效 <strong>{props.stats.staleFacts}</strong></span>}</div>
    ...
    {selectedProject && <>
      ...
      <div className="project-memory-metrics"><div><span>可信事实</span><strong>{detailStats?.eligibleFacts ?? 0}</strong></div><div><span>待处理事项</span><strong>{detailStats?.userActions ?? 0}</strong></div><div><span>冲突组</span><strong>{detailStats?.conflictGroups ?? 0}</strong></div><div><span>已失效</span><strong>{detailStats?.staleFacts ?? 0}</strong></div><div><span>资料覆盖</span><strong>{score("sourceCoverageScore")}</strong></div><div><span>面试准备</span><strong>{score("interviewReadinessScore")}</strong></div>{health?.needsReanalysis === true && <p className="page-note">项目资料需重新分析：{healthIssues.map((item) => String(item.message)).join("；")}</p>}</div>
      <nav className="project-detail-tabs" aria-label="项目详情"><button className={tab === "overview" ? "active" : ""} onClick={() => setTab("overview")}>项目概览</button><button className={tab === "facts" ? "active" : ""} onClick={() => setTab("facts")}>项目事实 <small>{allProjectFacts.length}</small></button><button className={tab === "problems" ? "active" : ""} onClick={() => setTab("problems")}>问题与复盘 <small>{projectProblems.length}</small></button><button className={tab === "sources" ? "active" : ""} onClick={() => setTab("sources")}>资料与证据 <small>{sources.length}</small></button><button className={tab === "questions" ? "active" : ""} onClick={() => setTab("questions")}>项目面试题 <small>{projectQuestions.length}</small></button></nav>
      {tab === "overview" && <div className="project-overview-v2"><article className="detail-sheet project-summary-card"><span className="page-kicker">PROJECT SUMMARY</span><p className="project-summary-text">{selectedProject.description || "一句项目简介待补充"}</p><h3>{responsibilityHeading}</h3>{selectedProject.role ? <ul className="project-bullet-list">{selectedProject.role.split(/[；;]/).filter(Boolean).slice(0, 5).map((item) => <li key={item}>{item}</li>)}</ul> : <p className="page-note">{responsibilityEmpty}</p>}<h3>项目时间</h3><p>{selectedProject.time || "待补充"}</p></article><article className="detail-sheet"><h3>核心技术</h3><div className="technology-taxonomy">{(selectedProject.technologyTaxonomy ?? []).map((group) => <section key={group.category}><strong>{group.label}</strong><div className="tag-list">{group.items.map((item) => <span key={item}>{item}</span>)}</div></section>)}{!selectedProject.technologyTaxonomy?.length && <p className="page-note">待补充</p>}</div><h3>核心模块</h3><ul className="project-bullet-list">{props.memory.modules.filter((item) => item.projectId === selectedProject.id).slice(0, 6).map((item) => <li key={item.id}><strong>{item.moduleName}</strong>{item.description ? ` · ${item.description}` : ""}</li>)}{props.memory.modules.filter((item) => item.projectId === selectedProject.id).length === 0 && <li>待补充</li>}</ul></article><article className="detail-sheet"><h3>典型问题</h3><ul className="project-bullet-list">{projectProblems.slice(0, 3).map((item) => <li key={item.id}>{item.problem}</li>)}{projectProblems.length === 0 && <li>问题与复盘待补充</li>}</ul><h3>项目成果</h3><ul className="project-bullet-list">{allProjectFacts.filter((fact) => isFactEligible(fact) && (fact.type === "result" || fact.type === "metric") && fact.evidenceLevel !== "not-measured").slice(0, 4).map((fact) => <li key={fact.id}>{fact.content}</li>)}{allProjectFacts.filter((fact) => isFactEligible(fact) && (fact.type === "result" || fact.type === "metric") && fact.evidenceLevel !== "not-measured").length === 0 && <li>结果 / 性能数据待补充</li>}</ul><h3>资料状态</h3>{dimensions.map((dimension) => <div className="completeness-row" key={String(dimension.key)}><span>{String(dimension.label)}</span><strong className={`completeness-${String(dimension.sourceStatus ?? dimension.status)}`}>{dimension.sourceStatus === "covered" ? "已覆盖" : dimension.sourceStatus === "weak" ? "证据较弱" : dimension.sourceStatus === "conflicting" ? "有冲突" : dimension.missingKind === "not_measured" ? "未测量" : "待补充"}</strong></div>)}</article></div>}
      {tab === "facts" && <div className="fact-review-grid"><div className="fact-review-toolbar" role="tablist" aria-label="事实筛选"><button className={factFilter === "action" ? "active" : ""} onClick={() => setFactFilter("action")}>待处理事项 {detailStats?.userActions ?? 0}</button><button className={factFilter === "conflict" ? "active" : ""} onClick={() => setFactFilter("conflict")}>冲突组 {detailStats?.conflictGroups ?? 0}</button><button className={factFilter === "responsibility" ? "active" : ""} onClick={() => setFactFilter("responsibility")}>职责</button><button className={factFilter === "review" ? "active" : ""} onClick={() => setFactFilter("review")}>系统待复核 {detailStats?.reviewRequiredFacts ?? 0}</button><button className={factFilter === "all" ? "active" : ""} onClick={() => setFactFilter("all")}>全部 {allProjectFacts.length}</button>{(detailStats?.staleFacts ?? 0) > 0 && <button className={factFilter === "stale" ? "active" : ""} onClick={() => setFactFilter("stale")}>已失效 {detailStats?.staleFacts}</button>}</div><div className="project-fact-cards">{(factFilter === "action" || factFilter === "conflict" || factFilter === "all") && conflictGroups.map((group) => <ProjectConflictGroupCard group={group} sourceTitle={sourceTitle} onResolve={props.onResolveConflict} onEvidence={setSelectedEvidence} key={group.id} />)}{projectFacts.map((fact) => <article className={`fact-review-card fact-review-${fact.status ?? "active"}`} key={fact.id}><header><div><span className="fact-type-label">{PROJECT_FACT_LABELS[fact.type]}</span><h3>{fact.title}</h3></div><strong>{statusText(fact)}</strong></header><p>{fact.content}</p><div className="fact-evidence-line"><span>来源：{fact.sourceIds.map(sourceTitle).join("、") || "未关联"}</span><span>{fact.evidenceLevel ?? "pending"} · {fact.ownership ?? "unknown"} · 置信度 {Math.round(fact.confidence * 100)}%</span></div>{fact.evidence?.[0] && <button className="evidence-link" onClick={() => setSelectedEvidence(fact)}>查看证据：“{fact.evidence[0].quote.slice(0, 80)}”</button>}<footer>{!fact.stale && fact.status !== "rejected" && isFactUserActionRequired(fact, ownershipMode) && <button className="dark-pill" onClick={() => void props.onReviewFact(fact.id, "active")}>{fact.type === "responsibility" ? "确认本人职责" : "确认事实"}</button>}{!fact.stale && fact.status !== "rejected" && <button className="outline-pill" onClick={() => void props.onReviewFact(fact.id, "rejected")}>不正确</button>}{fact.status === "rejected" && <button className="outline-pill" onClick={() => void props.onReviewFact(fact.id, "pending_review")}>恢复待确认</button>}</footer></article>)}{projectFacts.length === 0 && conflictGroups.length === 0 && <div className="knowledge-empty"><strong>暂无符合条件的事实</strong><span>{factFilter === "action" ? "当前没有待处理事项。" : factFilter === "review" ? "当前没有系统待复核事实。" : "重新分析或补充项目资料。"}</span></div>}</div><aside className="fact-review-summary"><strong>风险优先</strong><span>一个冲突组只对应一个用户决策</span><span>职责必须本人确认</span><span>结果 / 指标需要证据</span><span>当前筛选 {projectFacts.length + ((factFilter === "action" || factFilter === "conflict" || factFilter === "all") ? conflictGroups.length : 0)} 项 · 项目事实共 {allProjectFacts.length} 条</span></aside></div>}
    ...
    {sourceDialog && selectedProject && <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="modal-card"><h2>添加项目资料</h2><p className="page-note">资料会直接绑定到“{selectedProject.name}”，不会自动创建新项目。</p><label className="clean-field"><span>资料类型</span><select value={sourceRole} onChange={(event) => setSourceRole(event.target.value)}><option value="overview">项目说明 / README</option><option value="code">源码 / 代码压缩包</option><option value="resume">简历项目经历</option><option value="responsibility">个人职责说明</option><option value="debug">问题排查记录</option><option value="test">测试与指标</option><option value="architecture">架构设计</option><option value="reference">参考资料</option><option value="other">其他</option></select></label><label className="dark-pill upload-project-action">选择文件<input type="file" accept=".zip,.txt,.md,.pdf,.docx" onChange={(event) => { const file = event.target.files?.[0]; if (file) { void props.onUploadProject(file, selectedProject.id, sourceRole); setSourceDialog(false); } event.target.value = ""; }} /></label><button className="outline-pill" onClick={() => setSourceDialog(false)}>取消</button></div></div>}
  </section>;
}
...
export function App(): JSX.Element {
  ...
  const uploadKnowledgeFile = async (file: File, documentType: KnowledgeDocumentTypeOption = "auto", projectId?: string, sourceRole?: string) => {
