...
export class OverlayManager {
  ...
  private syncZOrderWindows(): void {
    ...
    this.enforceControlHitTest();
  }

  private enforceControlHitTest(): void {
    const window = this.currentControlWindow;
    if (!window || window.isDestroyed() || !window.isVisible()) return;
    // The control surface is the only native overlay that must receive input.
