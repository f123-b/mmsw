...
try {
  ...
  const questionGeometry = await overlay.evaluate("(() => { const rect = document.querySelector('.question-panel')?.getBoundingClientRect(); return { viewport: { width: window.innerWidth, height: window.innerHeight }, content: rect ? { width: rect.width, height: rect.height } : undefined }; })()");
  const answerGeometry = await overlayAnswer.evaluate("(() => { const rect = document.querySelector('.answer-panel')?.getBoundingClientRect(); return { viewport: { width: window.innerWidth, height: window.innerHeight }, content: rect ? { width: rect.width, height: rect.height } : undefined }; })()");
  if (controlSurface?.surface !== "control" || controlSurface.buttons < 3 || controlSurface.contentPanels !== 0 || contentSurface?.surface !== "question" || contentSurface.hasToolbar || !contentSurface.hasQuestionPanel || contentSurface.hasAnswerPanel || answerSurface?.surface !== "answer" || answerSurface.hasToolbar || answerSurface.hasQuestionPanel || !answerSurface.hasAnswerPanel || (questionGeometry?.viewport?.height ?? 0) < 88 || (questionGeometry?.viewport?.height ?? 0) > 280 || (answerGeometry?.viewport?.height ?? 0) < 132 || (answerGeometry?.viewport?.height ?? 0) > 440) throw new Error(`OVERLAY_SURFACE_SPLIT failed: ${JSON.stringify({ controlSurface, contentSurface, answerSurface, questionGeometry, answerGeometry })}`);
