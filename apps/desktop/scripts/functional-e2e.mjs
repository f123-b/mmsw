...
try {
  ...
  if ((controlGeometry?.viewport?.width ?? 0) > 560 || (controlGeometry?.viewport?.height ?? 0) > 72 || Math.round(controlGeometry?.toolbar?.width ?? 0) > 560 || Math.round(controlGeometry?.toolbar?.height ?? 0) > 72) throw new Error(`CONTROL_GEOMETRY_REGRESSION failed: ${JSON.stringify(controlGeometry)}`);
  const contentSurface = await overlay.evaluate("(() => ({ surface: document.querySelector('.overlay-root')?.dataset.overlaySurface, hasToolbar: Boolean(document.querySelector('.toolbar-panel')), hasQuestionPanel: Boolean(document.querySelector('[data-overlay-content=question]')), hasAnswerPanel: Boolean(document.querySelector('[data-overlay-content=answer]')), hasShortcut: Boolean(document.querySelector('[data-testid=shortcut-menu]')) }))()");
  const answerSurface = await overlayAnswer.evaluate("(() => ({ surface: document.querySelector('.overlay-root')?.dataset.overlaySurface, hasToolbar: Boolean(document.querySelector('.toolbar-panel')), hasQuestionPanel: Boolean(document.querySelector('[data-overlay-content=question]')), hasAnswerPanel: Boolean(document.querySelector('[data-overlay-content=answer]')), hasShortcut: Boolean(document.querySelector('[data-testid=shortcut-menu]')) }))()");
  const questionGeometry = await overlay.evaluate("(() => { const rect = document.querySelector('.question-panel')?.getBoundingClientRect(); const root = document.querySelector('.overlay-root'); return { viewport: { width: window.innerWidth, height: window.innerHeight }, content: rect ? { width: rect.width, height: rect.height } : undefined, rootPointerEvents: root ? getComputedStyle(root).pointerEvents : '' }; })()");
  const answerGeometry = await overlayAnswer.evaluate("(() => { const rect = document.querySelector('.answer-panel')?.getBoundingClientRect(); const root = document.querySelector('.overlay-root'); return { viewport: { width: window.innerWidth, height: window.innerHeight }, content: rect ? { width: rect.width, height: rect.height } : undefined, rootPointerEvents: root ? getComputedStyle(root).pointerEvents : '' }; })()");
  const questionScroll = await overlay.evaluate("(() => { const region = document.querySelector('.overlay-scroll-region'); return { overflowY: region ? getComputedStyle(region).overflowY : '', cardOverflow: document.querySelector('[data-overlay-content=question]') ? getComputedStyle(document.querySelector('[data-overlay-content=question]')).overflowY : '' }; })()");
  ...
  await waitFor(() => document.querySelector('.transient-overlay-root')?.dataset.transientLayer === "shortcut" && Boolean(document.querySelector("[data-testid='shortcut-menu']")), 5_000, overlayTransient);
  const shortcutState = await overlayTransient.evaluate("(() => ({ layer: document.querySelector('.transient-overlay-root')?.dataset.transientLayer, actionCount: document.querySelectorAll('.shortcut-actions button').length, questionHasShortcut: false }))()");
  const questionHasShortcut = await overlay.evaluate("Boolean(document.querySelector('[data-testid=shortcut-menu]'))");
  if (shortcutState?.layer !== "shortcut" || shortcutState.actionCount !== 5 || questionHasShortcut) throw new Error(`TRANSIENT_SHORTCUT_SURFACE failed: ${JSON.stringify({ shortcutState, questionHasShortcut })}`);
  await nativeClick(".toolbar-end-button", overlayControl);
  await waitFor(() => document.querySelector('.transient-overlay-root')?.dataset.transientLayer === "end_confirm" && !document.querySelector("[data-testid='shortcut-menu']"), 5_000, overlayTransient);
  await nativeClick("[data-testid='confirm-cancel']", overlayTransient);
  await waitFor(() => document.querySelector('.transient-overlay-root')?.dataset.transientLayer === "none" && !document.querySelector("[data-testid='confirm-end']"), 5_000, overlayTransient);
  await nativeClick(".toolbar-icon-action[title='打开快捷操作']", overlayControl);
  await waitFor(() => document.querySelector('.transient-overlay-root')?.dataset.transientLayer === "shortcut", 5_000, overlayTransient);
  await nativeClick(".toolbar-icon-action[title='打开快捷操作']", overlayControl);
  await waitFor(() => document.querySelector('.transient-overlay-root')?.dataset.transientLayer === "none", 5_000, overlayTransient);
  evidence.push("Overlay control surface: PASS; compact independent shortcut surface: PASS; one transient layer: PASS; content surface remains panel-only: PASS");
  await screenshot("10-interview-running.png", overlay);
   const questionStructure = await overlay.evaluate("(() => { const panel = document.querySelector('[data-overlay-content=question]'); const box = document.querySelector('.question-panel')?.getBoundingClientRect(); return { overflow: panel ? getComputedStyle(panel).overflowY : '', pointerEvents: panel ? getComputedStyle(panel).pointerEvents : '', hasFullTranscript: Boolean(document.querySelector('.transcript-scroll, .transcript-bubble, .timeline-scroll')), hasNavigator: Boolean(document.querySelector('.overlay-history')), hasResizeHandle: Boolean(document.querySelector('.resize-handle')), panelWidth: box?.width ?? 0 }; })()");
   const answerStructure = await overlayAnswer.evaluate("(() => { const panel = document.querySelector('[data-overlay-content=answer]'); const box = document.querySelector('.answer-panel')?.getBoundingClientRect(); return { overflow: panel ? getComputedStyle(panel).overflowY : '', pointerEvents: panel ? getComputedStyle(panel).pointerEvents : '', hasReader: Boolean(document.querySelector('.answer-core')), hasResizeHandle: Boolean(document.querySelector('.resize-handle')), panelWidth: box?.width ?? 0 }; })()");
   const overlayStructure = { questionOverflow: questionScroll?.overflowY, answerOverflow: answerScroll?.overflowY, questionPointerEvents: questionGeometry?.rootPointerEvents, answerPointerEvents: answerGeometry?.rootPointerEvents, hasFullTranscript: questionStructure?.hasFullTranscript || answerStructure?.hasFullTranscript, hasHistoryToggle: questionStructure?.hasNavigator, questionResizeHandle: questionStructure?.hasResizeHandle, answerResizeHandle: answerStructure?.hasResizeHandle, hasReader: answerStructure?.hasReader, independentPanels: Boolean((questionStructure?.panelWidth ?? 0) > 0 && (answerStructure?.panelWidth ?? 0) > 0) };
  if (!overlayStructure || !['auto', 'scroll'].includes(overlayStructure.questionOverflow) || !['auto', 'scroll'].includes(overlayStructure.answerOverflow) || overlayStructure.questionPointerEvents !== 'none' || overlayStructure.answerPointerEvents !== 'none' || overlayStructure.hasFullTranscript || overlayStructure.questionResizeHandle || overlayStructure.answerResizeHandle || !overlayStructure.hasReader || !overlayStructure.independentPanels) throw new Error(`OVERLAY_SCROLL_LAYOUT_REGRESSION failed: ${JSON.stringify(overlayStructure)}`);
  ...
  const answerHistory = await overlayAnswer.evaluate("(() => { const details = document.querySelector('.overlay-history'); return { present: Boolean(details), open: details?.open ?? false }; })()");
  if (!answerHistory?.present || answerHistory.open) throw new Error(`ANSWER_HISTORY_NOT_COLLAPSED failed: ${JSON.stringify(answerHistory)}`);
  ...
  const screenshotAnswerVisible = await overlayAnswer.evaluate("(() => { const answer = document.querySelector('[data-overlay-content=answer]'); if (!answer) return false; answer.dispatchEvent(new WheelEvent('wheel', { bubbles: true, deltaY: 240 })); return document.querySelector('.answer-core')?.textContent?.length > 0; })()");
  await sleep(300);
  ...
  await nativeClick(".toolbar-end-button", overlayControl);
  await waitFor(() => document.querySelector('.transient-overlay-root')?.dataset.transientLayer === "end_confirm" && Boolean(document.querySelector("[data-testid='confirm-end']")), 5_000, overlayTransient);
  const confirmGeometry = await overlayTransient.evaluate("(() => { const card = document.querySelector('.confirm-card')?.getBoundingClientRect(); return { layer: document.querySelector('.transient-overlay-root')?.dataset.transientLayer, viewport: { width: window.innerWidth, height: window.innerHeight }, card: card ? { width: card.width, height: card.height } : undefined, shortcut: Boolean(document.querySelector('[data-testid=shortcut-menu]')) }; })()");
  if (confirmGeometry?.layer !== "end_confirm" || confirmGeometry?.viewport?.width !== 420 || confirmGeometry?.viewport?.height !== 170 || (confirmGeometry?.card?.width ?? 0) < 390 || (confirmGeometry?.card?.height ?? 0) < 140 || confirmGeometry.shortcut) throw new Error(`CONFIRM_GEOMETRY_REGRESSION failed: ${JSON.stringify(confirmGeometry)}`);
  await nativeClick("[data-testid='confirm-end']", overlayTransient);
