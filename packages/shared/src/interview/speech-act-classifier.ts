...
const META_CONVERSATION = /(?:你能看到吗|看得到吗|你能听到吗|听得到吗|你在动鼠标吗|动我鼠标|鼠标|屏幕能看到吗|声音听得到吗|卡了吗|软件怎么了|网络卡吗|你怎么知道我这里|你操作我电脑了吗|你操作了吗|能看到我这边|能听到我这边)/i;
const META_REPAIR = /^(?:你觉得(?:呢)?|怎么(?:回答|答|说)|答案(?:是什么|呢))[。！？?！\s]*$/i;
const CODE_CONTEXT = /(?:现在|接下来|下面|来)?(?:考你|问你)?(?:一个)?代码题|算法题|编程题/i;
const CODE_REQUEST = /(?:写.{0,18}(?:代码|函数|程序)|手写|代码实现|实现一下|写一个|写个|给一段代码|用\s*C(?:\+\+|语言)?\s*写|用\s*C\+\+\s*写|伪代码|补全代码|输出代码|字符串.{0,12}(?:代码|实现)|链表.{0,12}(?:代码|函数|实现)|排序.{0,12}(?:代码|实现))/i;
const ANSWER_REQUEST = /^(?:请)?(?:口述|描述|介绍(?:一下)?|详细介绍|讲一下|讲讲|详细讲述|讲述|说一下|说说|说明(?:一下)?|解释(?:一下)?|展开讲一下|展开说|列举一下|总结一下|分析一下|完整讲一下|完整讲述|具体说)(?:[：:]|\s|$|(?=[\p{L}\p{N}]))/iu;
const TRAILING_ANSWER_REQUEST = /(?:^|[，,、\s])(?:好|好的|嗯+|明白了?)?[，,、\s]*(?:说说|讲讲|展开说|展开说说|具体说|具体讲|再说说|再讲讲)[。！？?！\s]*$/i;
...
const FOLLOW_UP_PREFIX = /^(?:那|然后|还有|具体|如果|再|这个|它|这里|其中|继续|接着)/;
const ELLIPTICAL_FOLLOW_UP = /^(?:为什么|为何|怎么|如何|具体(?:呢)?|还有(?:呢|吗)?|然后呢|用过哪些|用在什么地方|哪几个|哪种|那低速呢|再具体一点)[？?。！!\s]*$/i;
...
const ASSERTIVE_STATEMENT = /^(?:我|我们|系统|项目|当前项目|当前|这个方案|这次优化|它|该模块).*(?:是|为|通过|使用|采用|完成|下降|提升|增加|减少|切换|实现了|负责了)[^？?]*[。！!]$/;
...
function isFollowUp(text: string, context: SpeechActContext): boolean {
  ...
  const contextualCompleteFollowUp = /^(?:那|然后|如果|再|这个|它|这里|其中).*(?:如果|换成|换为|结果|低速|验证|设计|优化|实现|怎么|如何|呢|吗)/.test(text) && !completeStandaloneForm;
...
export class SpeechActClassifier {
  classify(text: string, context: SpeechActContext = {}): SpeechActClassification {
    ...
    if (ACKNOWLEDGEMENT.test(normalizedText)) return { speechAct: "ACKNOWLEDGEMENT", shouldAnswer: false, confidence: 0.99, normalizedText, reason: "acknowledgement", topic, entities };
