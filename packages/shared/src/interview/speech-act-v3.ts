...
const QUESTION = /(?:什么|为什么|为何|怎么|如何|怎样|哪些|哪种|哪个|哪里|是否|有没有|能不能|可不可以|多少|几个|多久|吗|呢|区别|原理|作用|原因|介绍|解释|说明|讲一下|说一下|说说|排查|定位|验证|设计|实现|优化|选择|选)/iu;
...
export function classifySpeechActV3(text: string, hasContext = false): SpeechActV3Result {
  ...
  if (QUESTION.test(value)) {
    const followUp = hasContext && (FOLLOW_UP.test(value) || value.length <= 18);
