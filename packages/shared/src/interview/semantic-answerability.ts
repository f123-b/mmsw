...
export interface SemanticAnswerabilityContext {
  ...
  /** A strong local classifier may rescue an acoustically fragmented question. */
...
const STYLE_ONLY = /^(?:请你?|请)?(?:(?:简单(?:地)?(?:说说|说一下|讲讲|讲一下)|简单说说(?:思路)?|大概(?:说说|说一下|讲讲|讲一下)|说重点|重点说(?:一下)?|不用展开|不要展开|简短(?:一点|说一下)?|控制在\s*\d+\s*(?:秒|分钟)|结合项目(?:说|讲)(?:一下)?|只讲(?:思路|重点)?|说说(?:思路)?就行|简单讲一下就行|简单说说就行)(?:[，,、\s]+(?:控制在\s*\d+\s*(?:秒|分钟)|结合项目(?:说|讲)(?:一下)?|说重点|不用展开|简短(?:一点|说一下)?))*|(?:简单|大概|重点|简短|只讲|结合项目|控制在)[^？?。！？!]{0,48}(?:就行|即可|不用展开|不要展开))[。！？?！\s，,、]*$/iu;
...
const CONTEXT_REFERENCE = /(?:这个|那个|这种|那种|这样|这么|它|这里|其中|然后|还有|具体|再|那|用过哪些|用在什么地方|哪一个|哪个|这两个|前者|后者|为什么这样|怎么这样|你会(?:更)?倾向于?用?哪(?:一个|个)|更倾向(?:于)?哪(?:一个|个))/iu;
...
export function decideSemanticAnswerability(text: string, context: SemanticAnswerabilityContext = {}): AnswerabilityDecision {
  ...
  const concreteSubject = MEANINGFUL_OBJECT.test(normalized) || EXPLICIT_TOPIC.test(normalized);
  ...
  const independent = concreteSubject
    || (structuralNucleus && compactText.length >= 6 && !contextualReference)
  ...
  if (!normalized || FILLER_ONLY.test(normalized)) {
    return { state: "FILLER", confidence: 0.99, reason: "filler-only", shouldAnswer: false, shouldBuffer: false, shouldAttachToPrevious: false, hasIndependentSubject: false, hasMeaningfulObject: false, isDangling: false };
  }
  if (isStyleOnly(normalized)) {
    return { state: "STYLE_ONLY", confidence: 0.98, reason: "style-only-modifier", shouldAnswer: false, shouldBuffer: false, shouldAttachToPrevious: recentQuestion, hasIndependentSubject: false, hasMeaningfulObject: false, isDangling: false };
  }
  if (BARE_ANSWER_REQUEST.test(compactText)) {
    ...
    return { state: "SETUP_ONLY", confidence: 0.97, reason: "bare-answer-request-without-subject", shouldAnswer: false, shouldBuffer: true, shouldAttachToPrevious: false, hasIndependentSubject: false, hasMeaningfulObject: false, isDangling: false };
  }
  ...
  if (CONDITIONAL_PREFIX.test(normalized) && !/(?:怎么|如何|怎样|怎么办|会不会|是否|能不能|可不可以|什么|哪里|哪儿|谁|多少|几个|吗|呢)/iu.test(normalized) && !strongLocalRescue) {
    return { state: "SETUP_ONLY", confidence: 0.94, reason: "conditional-setup-without-nucleus", shouldAnswer: false, shouldBuffer: true, shouldAttachToPrevious: false, hasIndependentSubject: meaningfulObject, hasMeaningfulObject: meaningfulObject, isDangling: false };
  }
  const dangling = isDanglingQuestionTail(normalized);
  if (dangling && !hasContextReference(normalized)) {
    return { state: "INCOMPLETE", confidence: 0.91, reason: "dangling-question-tail", shouldAnswer: false, shouldBuffer: true, shouldAttachToPrevious: false, hasIndependentSubject: independent, hasMeaningfulObject: meaningfulObject, isDangling: true };
  }
  ...
  if (QUESTION_NUCLEUS.test(normalized) || /[？?]$/.test(normalized)) {
    return { state: "INCOMPLETE", confidence: 0.78, reason: "question-signal-without-nucleus", shouldAnswer: false, shouldBuffer: true, shouldAttachToPrevious: false, hasIndependentSubject: false, hasMeaningfulObject: meaningfulObject, isDangling: false };
  }
  return { state: "INCOMPLETE", confidence: 0.72, reason: "no-question-nucleus", shouldAnswer: false, shouldBuffer: true, shouldAttachToPrevious: false, hasIndependentSubject: false, hasMeaningfulObject: meaningfulObject, isDangling: false };
}
