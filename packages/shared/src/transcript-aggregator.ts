...
const CONTINUATION_START = /^(关键字|关键点|作用|影响|导致|决定|以及|并且|而且|尤其|包括|比如|例如|具体|分别|常见|十五秒|只讲|简单说|先说|你会|你准备|用一句话|急速|抖动|当时|接着|最后|然后|隔离|硬件|软件|故障|可观测|验证|排查|定位|设计|实现|考虑|在|其中|同时|关于|针对|但|但是|否则|若|如果|有什么)/;
...
const STANDALONE_TRANSITION = /^(?:还有(?:一个)?问题|下一个(?:问题)?|再问(?:一个)?|接下来(?:问)?|继续问一个(?:系统设计|C\+\+基础|底层驱动|通信协议|项目经验|异常恢复)?问题?|下面聊(?:一下)?(?:RTOS|FreeRTOS)?|我换个(?:更底层的)?问题)[。！？?！\s，,、]*$/iu;
