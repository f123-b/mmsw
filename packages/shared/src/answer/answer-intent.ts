...
const OWNERSHIP_QUESTION = /你(?:在项目中|在这个项目里|在项目里面|实际)?(?:负责|主导|设计|实现|独立完成|做过|解决|优化|承担)|介绍一下你(?:的|负责|做过|参与)的项目/;
...
export function analyzeAnswerIntent(input: AnswerIntentInput | string, kind?: AnswerQuestionKind): AnswerIntent {
  ...
  const requiresPersonalMetric = !requiresPersonalIdentity && (METRIC_QUESTION.test(normalized) && (PROJECT_CUE.test(normalized) || asksBehavioralEpisode || resolvedKind === "follow-up" || /\d+(?:\.\d+)?\s*(?:%|ms|us|秒|分钟|小时|天|Hz|MHz|kHz|MB|KB)/.test(normalized)));
