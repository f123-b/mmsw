...
function removeReplacementArtifacts(text: string): string {
...
export class SpokenAnswerFormatter {
  ...
  instructions(mode: AnswerMode, kind: AnswerQuestionKind = "technical", plan?: AnswerPlan): string {
    ...
    const firstPerson = kind === "project" || kind === "behavioral" ? "使用第一人称并严格依据真实经历。" : "不必强行使用第一人称项目叙述；通用技术题只回答当前问题，不自动加入项目经历。";
