...
const RULES: readonly AccuracyRule[] = [
  ...
  , { id: "interrupt-task-priority-separation", question: /(?:中断|interrupt)[^。！？\n]{0,20}(?:任务|task)?[^。！？\n]{0,12}优先级|(?:任务|task)[^。！？\n]{0,20}(?:中断|interrupt)?[^。！？\n]{0,12}优先级/i, violation: /(?:中断优先级|interrupt\s+priority)[^。！？\n]{0,60}(?:就是|等于|决定|控制)[^。！？\n]{0,20}(?:任务优先级|调度|时间片)|(?:任务优先级|task\s+priority)[^。！？\n]{0,60}(?:就是|等于|决定)[^。！？\n]{0,20}(?:中断优先级|interrupt\s+priority)/i, negation: /(?:两套|分别|独立|不是同一|区别|不能混淆)/i, correction: "中断优先级和任务优先级属于不同调度层：前者决定中断嵌套/响应顺序，后者由 RTOS 调度器决定线程运行顺序，不能混为一谈。" }
  , { id: "mutex-semaphore-separation", question: /(?:mutex|互斥锁|semaphore|信号量)/i, violation: /(?:信号量|semaphore)[^。！？\n]{0,35}(?:就是|等同于|只能用于|表示)[^。！？\n]{0,16}(?:互斥锁|mutex)|(?:互斥锁|mutex)[^。！？\n]{0,35}(?:就是|等同于)[^。！？\n]{0,16}(?:信号量|semaphore)/i, negation: /(?:区别|不同|不能简单|不等同|互斥语义|计数)/i, correction: "互斥锁强调唯一所有权和解锁归属，信号量强调计数或同步；二者都能参与并发协调，但语义、所有权和优先级继承能力不同。" }
  , { id: "serial-bus-distinction", question: /(?:IIC|I2C|SPI|UART)[^。！？\n]{0,45}(?:区别|比较|对比)|(?:区别|比较|对比)[^。！？\n]{0,45}(?:IIC|I2C|SPI|UART)/i, violation: /(?:IIC|I2C)[^。！？\n]{0,25}(?:使用|有|需要)[^。！？\n]{0,20}(?:MOSI|MISO|片选|CS)|SPI[^。！？\n]{0,25}(?:只有|没有)[^。！？\n]{0,20}(?:片选|时钟|SCK)|UART[^。！？\n]{0,25}(?:有|使用)[^。！？\n]{0,20}(?:SDA|SCL|MOSI|MISO)/i, negation: /(?:不是|没有把|不能把|分别使用|I2C.*SDA.*SPI.*MOSI)/i, correction: "I2C 使用 SDA/SCL 的共享总线并通过地址寻址；SPI 使用时钟、MOSI/MISO 和片选，通常吞吐更高；UART 是异步点对点串口，使用 TX/RX 和波特率约定。" }
  , { id: "hardfault-watchdog-separation", question: /(?:HardFault|硬故障|看门狗|watchdog)[^。！？\n]{0,45}(?:区别|复位|原因)|(?:区别|原因)[^。！？\n]{0,45}(?:HardFault|硬故障|看门狗|watchdog)/i, violation: /(?:HardFault|硬故障)[^。！？\n]{0,35}(?:就是|属于|由)[^。！？\n]{0,16}(?:看门狗|watchdog)|(?:看门狗|watchdog)[^。！？\n]{0,35}(?:就是|属于|由)[^。！？\n]{0,16}(?:HardFault|硬故障)/i, negation: /(?:不同|不是|分别|可能导致|触发后进入)/i, correction: "HardFault 是处理器异常，通常来自非法访问、执行错误或其他 fault 升级；看门狗复位是独立的超时复位机制，二者需要分别查异常寄存器、复位原因和任务健康状态。" }
  , { id: "foc-current-speed-loop-separation", question: /(?:FOC|电流环|速度环)[^。！？\n]{0,45}(?:区别|作用|控制|分别|做什么)|(?:区别|作用)[^。！？\n]{0,45}(?:电流环|速度环)/i, violation: /(?:速度环|速度控制)[^。！？\n]{0,28}(?:直接控制|就是控制|负责控制)[^。！？\n]{0,12}(?:电流|Iq|Id)|(?:电流环|电流控制)[^。！？\n]{0,28}(?:直接控制|就是控制|负责控制)[^。！？\n]{0,12}(?:转速|速度)/i, negation: /(?:外环|内环|参考|给.*电流环|给.*速度环|分别)/i, correction: "FOC 中速度环通常根据速度误差生成转矩/电流给定，电流环再快速调节 d/q 轴电流；速度环和电流环是级联的不同控制环，不能把控制对象直接混为一谈。" }
  , { id: "dma-interrupt-separation", question: /(?:DMA|直接内存访问)[^。！？\n]{0,35}(?:中断|区别)|(?:中断)[^。！？\n]{0,35}(?:DMA|直接内存访问)/i, violation: /DMA[^。！？\n]{0,35}(?:就是|等于|本质是)[^。！？\n]{0,12}(?:中断)|(?:中断)[^。！？\n]{0,35}(?:就是|等于)[^。！？\n]{0,12}DMA/i, negation: /(?:不是|不等于|完成后.*中断|通过.*中断通知)/i, correction: "DMA 是由外设/内存控制器执行的数据搬运机制；传输完成或出错时可以触发中断通知 CPU，但 DMA 本身不是中断。" }
];
